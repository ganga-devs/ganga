...
public:
  ...
  void write( int ref, const char *outf ) {
    // write the info
    char buf[200];
	...
	for (it2 = InvCollNameforGuidMap.begin(); it2 != InvCollNameforGuidMap.end(); it2++)
	  {
	    ...
	    FILE *ofout = fopen(it2->first.c_str(), "wb");
	    fwrite( &(ref), sizeof(int), 1, ofout);
	    ...
	    //ofout << buf << " ";
	    guidOidMap_t::size_type num_guids = it2->second.size();
	    fwrite(&num_guids, sizeof(guidOidMap_t::size_type), 1, ofout);

	    for (int i = 0; i < it2->second.size(); i++)
	      {
		// write the guid + # of OIDs
		fwrite(it2->second[i].c_str(), sizeof(char), it2->second[i].length(), ofout);
		oidEvtVecMap_t::size_type num_oids = m_infoMap[ it2->second[i] ].size();
		fwrite(&num_oids, sizeof(oidEvtVecMap_t::size_type), 1, ofout);
		//std::cout << num_oids << std::endl;
		// now go over the OIDs
		oidEvtVecMap_t::iterator it3;
		for (it3 = m_infoMap[ it2->second[i] ].begin(); it3 != m_infoMap[ it2->second[i] ].end(); it3++)
		  {
		    int oid = it3->first;
		    fwrite(&oid, sizeof(int), 1, ofout);
		    std::vector<int>::size_type num_evts = it3->second.size();
		    fwrite(&num_evts, sizeof(std::vector<int>::size_type), 1, ofout);

		    for (int j = 0; j < it3->second.size(); j++)
		      {
			int evt = it3->second[j];
			//std::cout << evt << std::endl;
			fwrite(&evt, sizeof(int), 1, ofout);
		      }
		  }
	      }

	    // create a ref file for panda
	    fclose(ofout);
	    make_template(it2->first.c_str());

	  }
      }
    else
      {
	//std::ofstream ofout(outf, ios::out | ios::binary | ios::app | ios::trunc);
	FILE *ofout = fopen(outf, "wb");
	fwrite( &(ref), sizeof(int), 1, ofout);
	...
	//sprintf(buf, "%d", m_infoMap.size());
	//ofout << buf << " ";
	guidOidMap_t::size_type num_guids = m_infoMap.size();
	fwrite(&num_guids, sizeof(guidOidMap_t::size_type), 1, ofout);
	...
	for ( it = m_infoMap.begin(); it != m_infoMap.end(); it++)
	  {
	    ...
	    fwrite(it->first.c_str(), sizeof(char), it->first.length(), ofout);
	    oidEvtVecMap_t::size_type num_oids = it->second.size();
	    fwrite(&num_oids, sizeof(oidEvtVecMap_t::size_type), 1, ofout);
  ...
  void make_template(const char *fname)
  {
...
{
  string thisProgram("CollCompressEventInfo");
  ...
  try
  {
    ...
    for( unsigned int i=0; i<srcinfo.nSrc(); i++ )
    {
      ...
      std::cout << splitRefName << std::endl;
    ...
    size_t found = splitRef.find( std::string("AOD"));
    int ref = 0;
    if (found != string::npos)
      {
	ref = 1;
	std::cout << "Setting AOD reference\n";
      }
    else
      {
	ref = 2;
	std::cout << "Setting ESD reference\n";
      }

    outInfo.write(ref , "out.dat");
    return 0;
  }
