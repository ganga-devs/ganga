...
import shutil
import tempfile
from GangaLHCb.Lib.Mergers.LHCbFileMerger import LHCbFileMerger as dstmerger
from GangaTest.Framework.tests import GangaGPITestCase
from GangaTest.Framework.utils import sleep_until_completed,file_contains
from Ganga.GPIDev.Lib.File import  FileBuffer
from Ganga.GPIDev.Adapters.IPostProcessor import PostProcessException

class TestLHCbFileMerger(GangaGPITestCase):

    # add some coverage for the public interface
    def test_LHCbFileMerger_merge(self):
        d = LHCbFileMerger()
        d.merge([])

    # Since LHCbFileMerger::merge is just a thin wrapper around this method, there's
    # no real point to including a separate test.
    def test__LHCbFileMerger_mergefiles(self):
        ...
        tool = dstmerger()
        tool._impl.mergefiles(dst_files,outdst)
