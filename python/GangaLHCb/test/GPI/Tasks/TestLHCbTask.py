...
if doConfig:
    ...
    bkQueryList = [GPI.BKTestQuery(stripping15down), GPI.BKTestQuery(
        stripping16up), GPI.BKTestQuery(stripping16down)]
...
class TestLHCbTask(GangaGPITestCase):
    ...
    def test_appendTransform(self):
        ...
        assert len(
            t.transforms[-1]._impl.toProcess_dataset.files), 'Transform not updated properly after appending'
    ...
    def test_update(self):
        ...
        # transforms
        t.update()
        assert len(t.transforms[
                   0]._impl.toProcess_dataset.files), 'Update did not produce any datafiles to process in transform 0'
        assert len(t.transforms[
                   1]._impl.toProcess_dataset.files), 'Update did not produce any datafiles to process in transform 1'
