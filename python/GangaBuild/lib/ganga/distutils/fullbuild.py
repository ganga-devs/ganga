...
#        ('release=', None, 'name of the release branch being built')
    ...
    def _buildAllModules(self):
        ...
        if not os.path.exists('tags'):
            print "Tags directory not found, so checking out of SVN"
            procSt, output = commands.getstatusoutput("svn co --depth infinity %s/tags/%s" % (self.tagroot, self.build_version))
            #procSt, output = commands.getstatusoutput("svn co --depth immediates  %s/tags" % self.svnroot)

            print "procSt: " + str(procSt)
            print "output: " + str(output)
            print "self.tagroot: " + str(self.tagroot)
            print "self.build_version " + str(self.build_version)
        #get module names
        junk = os.listdir('./')
        print junk
        procSt, modules = commands.getstatusoutput("svn list ./%s/python" % (self.build_version))
        print "procSt: " + str(procSt)
#        print "output2: " + str(output2)
        print "modules: " + str(modules)
        done = {}
        for moduleName in modules.split('\n'):
...
#            else:
#                print "Real dir name " + realDirName
...
#            commands.getstatusoutput("svn up %s --set-depth empty" % moduleName)
    ...
    def run(self):
        ...
        print "build procSt: " + str(procSt)
        print "build output: " + str(output)
...
#        allCom = [['bdist_rpm', '-d %s/dist/' % self.workspace]]

        errors = ''
        for (c, options) in allCom:

            print os.listdir('./')
            print "python setup.py %s %s" % (c, options)
