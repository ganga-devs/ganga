...
def migrationRepositoryFactory(repositoryType='Local', root_dir='/users', **kwargs):
        ...
        try:
            ...
            def visitor(ida):
                ...
                for md in metadata:
                    try:
                        ...
                        msg = 'Dictionary of job %s cannot be evaluated because of the error: %s. The job is most likely corrupted and will not be not imported. To permanently delete the job use the command jobs._impl.repository.deleteJobs([%s]).' % (
                            md[0], str(e), md[0])
                        logger.error(msg)
                    else:
                        res.append([jdict, rr])
            ...
            for jdict in jdicts:
                try:
                    ...
                    msg = 'Exception: %s while constructing job object from dictionary %s' % (
                        str(e), repr(jdict))
                    logger.error(msg)
                else:
                    jobs.append(job)
    ...
    def registerJobs(self, jobs):
        ...
        try:
            self._commitJobs(
                jobs, forced_action=True, deep=True, register=True, get_ids=False)
        finally:
            self._rep_lock.release()
    ...
    if repositoryType == 'Local':
        ...
        while fn:
            ...
            logger.warning(
                'Non conventional root directory of the new repository. The old repository will not be migrated')
        ...
        if not os.path.isdir(old_path):
            ...
            if os.path.isdir(new_path):
                ...
                old_repository = repositoryFactory(
                    repositoryType=repositoryType, root_dir=old_root_dir, **kwargs)
                ...
                while ids:
                    ...
                    try:
                        ...
                        msg = (
                            "Error while getting %s from the old repository: " % rep_type) + str(e)
                        logger.error(msg)
                    else:
                        try:
                            # don't register jobs in incomplete state
                            jobs = filter(
                                lambda j: j.status != "incomplete", jobs)
                            registerJobs(new_repository, jobs)
                        except Exception as e:
                            msg = (
                                "Error while saving old %s in the new repository: " % rep_type) + str(e)
                            logger.error(msg)
                        else:
                            nn += len(jobs)

                # has to be in GPI before conversion
                import Ganga.GPIDev.Lib.JobTree.JobTree
                try:
                    job_tree = old_repository.getJobTree()
                    if job_tree:
                        new_repository.setJobTree(job_tree)
                except Exception as e:
                    msg = "Error while saving old jobtree in the new repository: " + \
                        str(e)
                    logger.error(msg)
                if nn > 0:
                    msg = '%d %s have been migrated to the new repository. To revert the migration use "revert_migration_v42tov43()" command' % (
                        nn, rep_type)
                    logger.warning(msg)
                return new_repository
    else:
        logger.debug(
            'Nothing to migrate. No support for the old remote repository')
        return standard_return()


# to support the trick with handlers order (see below atexit)
__revert_migration = False
...
# this is a trick: always register to revert handler to put it before any
# handlers in the repository
atexit.register(_revert_migration_v42tov43)
