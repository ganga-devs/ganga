...
switch_debug(arda_logger_name, Ganga.Utility.logging.getLevelName(
    arda_logger.getEffectiveLevel()))
...
##################################
all_configs = {}
all_configs['LocalAMGA'] = Ganga.Utility.Config.makeConfig(
    'LocalAMGA_Repository', 'Settings for the local AMGA job repository')
all_configs['LocalAMGA'].addOption(
    'blocklength', 1000, 'maximum number of jobs stored in a block of local repository')
all_configs['LocalAMGA'].addOption(
    'cache_size', 3, 'maximum size of memory (in blocks) that local repository can use for job caching')
all_configs['LocalAMGA'].addOption(
    'tries_limit', 200, 'maximum number of attempts to write/move file or to acquire the table lock in local repository')
all_configs['LocalAMGA'].addOption(
    'lock_timeout', 60, 'maximum time in seconds that limits lock validity for local repository')

all_configs['RemoteAMGA'] = Ganga.Utility.Config.makeConfig(
    'RemoteAMGA_Repository', 'Settings for the local AMGA job repository')
all_configs['RemoteAMGA'].addOption(
    'host', 'gangamd.cern.ch', 'location of the AMGA metadata server used by the remote repository')
all_configs['RemoteAMGA'].addOption(
    'port', 8822, 'port for secure connection to the remote repository')
all_configs['RemoteAMGA'].addOption(
    'reqSSL', 1, 'flag for secure connection to the remote repository')
all_configs['RemoteAMGA'].addOption(
    'login', '', 'login name to connect to the remote repository')
...
class ARDARepositoryMixIn(JobRepository):
    ...
    _id = ('id',         'int')          # job id
    # split counter (shows next id); if > 0 the job has been split
    _counter = ('counter',    'int')
    _subjobs = ('subjobs',    'text')         # list of subjob ids
    # flag showing that whether job is a subjob (for selections)
    _isasubjob = ('isasubjob',  'varchar(254)')
    # flag showing that blob has been compressed
    _compressed = ('compressed', 'varchar(254)')
    # internal state (for advanced locking)
    _istate = ('istate',     'varchar(254)')
    # time when the lock has been created
    ...
    def __init__(self, schema, role, streamer, tree_streamer, root_dir, init_schema):
        ...
        # init_schema is a switch used to control addition of default
                ...
                if self._isDirNotFoundError(e):  # directory not found
                    ...
                    if remove_sequence:
                        try:
                            ...
                            # Not a sequence
                            if not self._isNotASequenceError(e):
                                raise e
                    ...
                    try:
                        ...
                        # if directory is not found (other process may delete
                        # it) don't raise an exception
                        if not self._isDirNotFoundError(e):
                            raise e
                else:
                    raise e
        finally:
                ...
                try:
                    for a, t in schema:
                        if a not in attributes:
                            ...
                            if t != dbt:
                                logger.debug(
                                    "Attribute %s exists with the different type %s" % (a, dbt))
                finally:
                    self._finalizeCommand()
            except CommandException as e:
                if e.errorCode != 15:
            ...
            def extractMD(r, mfqid):
                ...
                for k, t in self.schema:
                    if k == self._blob[0]:
                        ...
                        if r[1]:
                            v = map(
                                lambda rr: rr[0]['data']['id']['data'], r[1])
                        else:
                            v = []
                        v = repr(v)
        ...
        try:
            ...
            def commit_visitor(md, path):
                try:
                    ...
                    if fqid not in self._job_cache:
                        if not register:
                            msg = "Job %s is not found in job cache. Commitment for this job will normally fail." % str(
                                fqid)
                            raise RepositoryError(msg=msg)
                    vals_dict = md[0]['metadata']
                    vals = map(lambda x: vals_dict[x], attrs)
                    updatecond = (not register) or (
                        deep and USE_COUNTERS_FOR_SUBJOBS and len(md[1]) > 0)
                    if updatecond:
                        self._generic_updateAttr(
                            fqid, attrs, vals, forced_action)
                    else:
                        self._generic_addEntry(fqid, attrs, vals)
                except (CommandException, RepositoryError) as e:
                    msg = "_commitJobs() command called while committing job %s raised an exception: %s" % (
                        str(fqid), str(e))
                    msgs.append(msg)
                    # logger.error(msg)
                    details[fqid] = e
            ...
            def register_visitor(j, path, reserve):
                ...
                try:
                    ...
                    msg = "sequenceNext() command called while registering jobs raised an exception: %s" % str(
                        e)
                    # logger.error(msg)
                ...
                try:
                    ...
                    try:
                        for j in jobs_to_commit:
                            commit_visitor(
                                self._getValues(j, timestamp, deep), path)
                    finally:
                        self._finalizeCommand()
                except Exception as e:
                    msg = str(e)
            ...
            def update_cache(fqid):
                ...
                if fqid in self._job_cache:
                    if self._job_cache[fqid] != job_lock:
                        logger.warning(
                            'Job %s has been modified outside of current session' % self._getJobFileName(fqid))
            ...
            if type(ids_or_attributes) in [types.TupleType, types.ListType]:
                ...
                for jid in ids_or_attributes:
                    ...
                    try:
                        ...
                        msg = "ARDA interface command getattr() called for job %s raised an exception: %s" % (
                            str(fqid), str(e))
                        logger.error(msg)
                    else:
                        while not self._generic_eot():
                            try:
                                ...
                                msg = "ARDA interface command getEntry() called for job %s raised an exception: %s" % (
                                    str(fqid), str(e))
                                logger.error(msg)
                            else:
                                md.insert(0, fqid)
                ...
                try:
                    self._generic_selectAttr(
                        selection, path, extended_attr_list)
                except CommandException as e:
                    msg = "ARDA interface command selectAttr() raised an exception: %s" % str(
                        e)
                    logger.error(msg)
                    # raise RepositoryError(e = e, msg = msg)
                else:
                    while not self._generic_eot():
                        try:
                            ...
                            msg = "ARDA interface command getSelectAttrEntry() raised an exception: %s" % str(
                                e)
                            logger.error(msg)
                        else:
                ...
                for jid in ids:
                    ...
                    try:
                        if new:
                            ...
                            self._generic_updateAttr(
                                fqid, attrs, values, forced_action)
                    except Exception as e:
                        msg = str(e)
                        # logger.error(msg)
    ...
    def _initCounter(self, fqid):
        self._rep_lock.acquire(1)
        try:
            attrs = [self._counter[0], self._lock[0],
                     self._istate[0], self._time[0]]
    ...
    def _counterNext(self, fqid, reserve=1):
        ...
        try:
            def next(fqid, reserve):
                metadata = self._getLockedMetaData(
                    [fqid], [self._counter[0]], istate='_counter')
                if metadata:
                    md = metadata[0]
                    assert(md[0] == fqid)
                    now = md[1]
                    ...
                    attrs = [
                        self._counter[0], self._lock[0], self._istate[0], self._time[0]]
                    values = [newval, self.guid, '_', repr(time.time())]
                    self._setMetaData([fqid], attrs, values)
                    return (now, newval)
                else:
                    raise RepositoryError(
                        msg='Can not lock job %s' % str(fqid))
            ...
            if reserve > 1:
                ...
                try:
                    ...
                    raise RepositoryError(
                        e=e, msg="getNextSubjobId error: " + str(e))
            else:
    ...
    def _generic_selectAttr(self, selection, path, attr_list):
        ...
        query = ' and '.join(
            map(lambda x: ':'.join([path, '%s="%s"' % x]), selection.items()))
        ...
        try:
            ...
            def visitor(md):
                try:
                    ...
                    msg = 'Dictionary of job %s cannot be evaluated because of the error: %s. The job is most likely corrupted and will not be not imported.' % (
                        str(e), str(md[0]))
                    raise RepositoryError(e=e, msg=msg)
                else:
                    return [jdict, rr]
            ...
            for md in metadata:
                try:
                    ...
                    msg = 'Exception: %s while constructing job object from a dictionary' % (
                        str(e))
                    logger.error(msg)
                else:
                    jobs.append(job)
    ...
    def deleteJobs(self, ids, forced_action=False):
        ...
        try:
            ...
            def getSubjobs(ids, deep=True):
                ...
                # if deep = False the format of the return value is [ [fqid, [
                # sfqid, ...] ],...]
                sjlist = self._getLockedMetaData(
                    ids, [self._subjobs[0]], '_deleting', forced_action)
            ...
            def rm(sjlist):
                ...
                for (fqid, ssjlist) in sjlist:
                    try:
                        ...
                        msg = "deleteJobs() command called while deleting job %s raised an exception: %s" % (
                            str(fqid), str(e))
                        details[fqid] = RepositoryError(e=e, msg=msg)
                        msgs.append(msg)
                    else:
            ...
            if USE_FOLDERS_FOR_SUBJOBS:
                ...
                try:
                    ...
                    msg = "Can not remove subjobs folder of the job %s because of the error: %s" % (
                        str(fqid), str(e))
                    # logger.error(msg)
                    details[fqid] = RepositoryError(e=e, msg=msg)
                    msgs.append(msg)
            ...
            # dictionary keys is master job fqids for which list of subjobs has
            # to be updated
            mfqids = {}
            ...
            # list of master jobs and all subjobs (to be updated)
            sjlist = getSubjobs(mfqids.keys(), deep=False)
            # updating ...
            self._initCommand()
            try:
                attrs_r = [self._subjobs[0], self._lock[0],
                           self._istate[0], self._time[0], self._counter[0]]
                ...
                for (mfqid, ssjlist) in sjlist:
                    try:
                        ...
                        for i in range(len(ssjlist)):
                            # reuse the same list to reduce memory consumption
                            ssjlist[i] = ssjlist[i][-1]
                        if len(ssjlist) == 0:
                            # reset counter
                            attrs = attrs_r
                            values = values_r
                            ...
                            attrs = attrs_s
                            values = values_s
                        values[0] = self._pstr2text(
                            repr(ssjlist))  # subjob ids
                        self._generic_updateAttr(
                            mfqid, attrs, values, forced_action)
                    except Exception as e:
                        msg = "Can not update master job %s of subjob %s because of the error: %s" % (
                            str(mfqid), str(fqid), str(e))
                        # logger.error(msg)
                        details[fqid] = RepositoryError(e=e, msg=msg)
                        msgs.append(msg)
            finally:
            ...
            if USE_FOLDERS_FOR_SUBJOBS:
                for (mfqid, ssjlist) in sjlist:
                    try:
                        ...
                        msg = "Can not unsplit master job %s after deleting the last subjob %s because of the error: %s" % (
                            str(mfqid), str(fqid), str(e))
                        # logger.error(msg)
                        details[fqid] = RepositoryError(e=e, msg=msg)
                        msgs.append(msg)
    ...
    def setJobsStatus(self, statusList, forced_action=False):
        ...
        try:
            ...
            attrs = ['status', self._compressed[0], self._blob[0],
                     self._lock[0], self._istate[0], self._time[0]]

            statusList = map(
                lambda x: (self._fqnConverter(x[0]), x[1]), statusList)
            ...
            try:
                for fqid, status in statusList:
                    if fqid in mdids:
                        ...
                        vals = [
                            status, compressed, blob, self.guid, istate, timestamp]
                        try:
                            self._generic_updateAttr(
                                fqid, attrs, vals, forced_action)
                        except (CommandException, RepositoryError) as e:
                            msg = "setJobsStatus() command called while committing job %s raised an exception: %s" % (
                                str(fqid), str(e))
                            msgs.append(msg)
                            # logger.error(msg)
                            details[fqid] = RepositoryError(e=e, msg=msg)
                        ...
                        msg = "Job %s is not found in the repository" % str(
                            fqid)
                        msgs.append(msg)
                        # logger.error(msg)
                        details[fqid] = RepositoryError(msg=msg)
    ...
    def getJobTree(self, tree_id=0):
        ...
        try:
            ...
            if self._tree_streamer:
                ...
                logger.warning(
                    "jobtree object can not be retrieved from repository")
        finally:
            self._rep_lock.release()
    ...
    def setJobTree(self, jobtree, tree_id=0):
        ...
        try:
            ...
            if self._tree_streamer:
                ...
                logger.warning("JobTree streamer has not been set.")
                logger.warning(
                    "jobtree object can not be saved in the repository")
                return
            try:
    ...
    def resetAll(self):
        ...
        try:
            try:
                ...
                jtreefldr = os.path.join(self.root_dir, self._jobsTreeFldr)
                self._forcedRemoveDir(
                    jtreefldr, [self._jobsTreeAttr], remove_sequence=False)
                # remove root_dir
...
class RemoteARDAJobRepository(ARDARepositoryMixIn, MDClient):
    ...
    def __init__(self,
        ...
        if reqSSL:
            ...
            try:
                ...
                msg = "Can not connect to the Repository because of the error: %s" % str(
                    e)
                logger.error(msg)
                raise RepositoryError(e=e, msg=msg)
    ...
    def removeAllLocks(self):
        logger.error(
            "method removeAllLocks should not be called for remote registry")
    ...
    def listAllLocks(self):
        logger.error(
            "method listAllLocks should not be called for remote registry")
        return []
...
class RemoteOracleARDAJobRepository(RemoteARDAJobRepository):
    ...
    def _initDir(self, path, schema=None, create_sequence=True):
        ...
        try:
            ...
            RemoteARDAJobRepository._initDir(
                self, path, schema, create_sequence)
    ...
    def _forcedRemoveDir(self, path, schema=None, remove_sequence=True):
        ...
        try:
            ...
            RemoteARDAJobRepository._forcedRemoveDir(self,
                                                     self._getBlobsFldrName(
                                                         path),
                                                     schema=[
                                                         self._blobsFldrAttr],
                                                     remove_sequence=False)
            # remove all the rest
            RemoteARDAJobRepository._forcedRemoveDir(
                self, path, schema, remove_sequence)
        finally:
            self._rep_lock.release()
    ...
    def _generic_updateAttr(self, fqid, attrs, values, forced_action):
        ...
        self._addBlob(fqid, attrs, values)
        # add all other metadata
        RemoteARDAJobRepository._generic_updateAttr(
            self, fqid, attrs, values, forced_action)
    ...
    def _generic_selectAttr(self, selection, path, attr_list):
        # always return filename as the first attribute
        command_buffer = []
        RemoteARDAJobRepository._generic_selectAttr(
            self, selection, path, attr_list)
        while not self.eot():
    ...
    def _generic_rm(self, fqid, forced_action):
        ...
        try:
            ...
            msg = "Can not delete blobs related to the job %s because of the error: %s" % (
                str(fqid), str(e))
            logger.debug(msg)
    ...
    def __init__(self,
        ...
        if not os.path.isdir(local_root):
            try:
                ...
                logger.debug(
                    "Root directory %s has been successfully created" % local_root)
        ...
        if lock_timeout < max_lock_time:
            logger.warning(
                "The 'lock_timeout' parameter is too small with respect to 'tries_limit' = %d" % tries_limit)
            logger.warning(
                "In order to avoid risk of deleting valid lock file the 'lock_timeout' parameter will be adjusted automatically")
        ...
        try:
            ...
            if old_locks:
                logger.warning(
                    "Lock files that are older than %d seconds are found in the repository" % self._lock_timeout)
                logger.warning(
                    "These locks most likely appear as a result of previous errors and will be removed")
                logger.warning(
                    "You can suppress this operation by increasing 'lock_timeout' parameter of the local repository constructor")
            ...
            if p_table != r_table:
                try:
                    ...
                    if r_table in self.loaded_tables:
                        ...
                        shutil.copy(
                            os.path.join(attr_dir, attr_name), path_dir)
                except Exception as e:
                    logger.debug(str(e))
            ...
            for n in self._get_iteration_list(mdtable):
                ...
                if e[0] == entry:
                    ...
                    if not (forced_action or e[lock_ind] == guid):
                        msg = "Job %s can not be commited because it is probably controlled by other client" % str(
                            fqid)
                        raise RepositoryError(msg=msg)
...
def repositoryFactory(**kwargs):
    ...
    info = 'AMGA Job Repository: type=%s ganga_user=%s' % (
        config['repositorytype'], config['user'])
    try:
        if repositoryType == 'LocalAMGA':
            if not kwargs.get('local_root'):
                # local_root is dynamically derived it is not configurable
                # parameter anymore
                kwargs['local_root'] = os.path.join(
                    expandfilename(config['gangadir']), 'repository', config['user'], 'LocalAMGA')
        ...
        if repositoryType == 'RemoteAMGA':
            if not kwargs.get('login'):
                ...
                info += ' login=%s host=%s port=%s reqSSL=%s' % (
                    kwargs['login'], kwargs['host'], kwargs['port'], kwargs['reqSSL'])
        ...
        if not kwargs.get('local_root'):
            ...
            # parameter anymore
            kwargs['local_root'] = os.path.join(
                expandfilename(config['gangadir']), 'repository', config['user'], 'Test')
