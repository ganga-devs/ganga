...
#------------------------ CONSUMERS -----------------------------------------
sys.path.insert(
    0, "/afs/cern.ch/user/m/mchamber/Ganga/install/Ganga-MSG-branch/python/Ganga/Lib/MonitoringServices/MSGPeek/")
sys.path.insert(0, '.')
processed = {}
k = 0
while len(processed) < num_pairs:
    ...
    if j_status == "running" and id not in processed:
        ...
        job_b.application = Executable(
            exe=File(JOB_B), args=['-I', str(start_job + k)])
        job_b.inputsandbox = [File(
            DIRECTORY + 'MSGPeekCollector.py', subdir='_python'), File(DIRECTORY + 'stomp.py', subdir='_python')]
        job_b.backend = LCG()
        job_b.submit()
        processed[id] = job_b.id
    elif (j_status == "completed" or j_status == "failed") and id not in processed:
        ferr.write(
            'The job %d is %s so no client can be launched in that state\n' % (id, j_status))
        processed[id] = -1
    else:
        sleep(1)
    k = k + 1
