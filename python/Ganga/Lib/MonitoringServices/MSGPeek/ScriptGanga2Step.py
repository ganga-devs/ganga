...
for k in d:
    ...
    #job_a.peek("stdout","tail -n 5")
        ...
        if not os.path.exists(a):
            f.write(
                "The output for the client %s doesn't exist, problems launching the client\n" % job_a.id)

        elif os.path.getsize(a) == 0:
            f.write(
                "The size of output of the client %s is 0, increase the number of the lines in the step 1\n" % job_a.id)
