...
class LCGRequirements(GangaObject):
   ...
   _schema = Schema(Version(1,1), { 
      'software'        : SimpleItem(defvalue=[],sequence=1,doc='Software Installations'),
   ...
   def convert(self):
      ...
      if self.cputime:        requirements += [ 'other.GlueCEPolicyMaxCPUTime >= %s' % str(self.cputime) ]
      if self.walltime:       requirements += [ 'other.GlueCEPolicyMaxWallClockTime >= %s' % str(self.walltime) ]
