...
#            try:
    ...
    def force_status(self,status,force=False):
        ...
        if not self.status in Job.allowed_force_states[status]:
            raise JobError('Only a job in one of %s may be forced into "failed" (job %s)'%(str(Job.allowed_force_states[status]),self.getFQID('.'))) #this gets called if arguement is 'completed' as well, so the message doesn't make complete sense. #Justin 12/8/09
