...
config = Ganga.Utility.Config.makeConfig('Display','control the printing style of the different registries ("jobs","box","tasks"...)')
...
class RegistrySlice(object):
    def __init__(self,name,display_prefix):
        ...
        self._display_columns_show_empty = config[display_prefix+"_columns_show_empty"]
    ...
    def select(self,minid=None,maxid=None,**attrs):
        ...
        slice = self.__class__("%s.select(minid=%s,maxid=%s%s)"%(self.name,r.repr(minid),r.repr(maxid),attrs_str))
        def append(id,obj):
    ...
    def do_select(self,callback,minid=None,maxid=None,**attrs):
        ...
        if is_container(minid):
            ...
            if maxid is None: maxid = sys.maxsize
