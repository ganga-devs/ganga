...
config.addOption('DEBUG_startup_profile', False,
    ...
    for n, r in zip(names, regs):
        ...
        if n == 'jobs' and config['DEBUG_startup_profile']:
            ...
            try:
                ...
                log_user_exception(logger)
                logger.critical(
                    'the repository cannot be loaded (dir=%s)', r.repository.dir)
                import tempfile
                import tarfile
                fn = tempfile.mktemp(
                    suffix='.tgz', prefix='Ganga-Repository-XML')
                tf = tarfile.open(fn, mode='w:gz')
                tf.add(
                    r.repository.dir, "Ganga-Repository-XML", recursive=True)
                tf.close()
                logger.critical(
                    'the repository was dumped to %s (%s) for debugging', fn, Ganga.Utility.util.hostname())
                raise x
