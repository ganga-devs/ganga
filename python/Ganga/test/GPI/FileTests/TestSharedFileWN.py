...
class TestSharedWN(GangaUnitTest):
    ...
    # Where on local storage we want to have our 'Shared solution'
    outputFilePath = '/tmp/SharedWN'

    # This sets up a SharedConfiguration which works by placing a file on local storage somewhere we can test using standard tools
    SharedTestConfig = {'defaultProtocol': 'file://',
                             'fileExtensions': [''],
    ...
    def setUp(self):
        """
        Configure the SharedFile for the test
        """
        extra_opts=[('PollThread', 'autostart', 'False'),
                    ('Local', 'remove_workdir', 'False'),
                    ('TestingFramework', 'AutoCleanup', 'False'),
                    ('Output', 'SharedFile', TestSharedWN.SharedTestConfig),
                    ('Output', 'FailJobIfNoOutputMatched', 'True')]
        super(TestSharedWN, self).setUp(extra_opts=extra_opts)
    ...
    def cleanUp():
        ...
        for file_ in TestSharedWN._managed_files:
            os.unlink(file_)
        TestSharedWN._managed_files = []

        shutil.rmtree(TestSharedWN.outputFilePath, ignore_errors=True)

    def test_a_Submit(self):
        """Test the ability to submit a job with some LocalFiles"""
        from Ganga.GPI import jobs, Job, LocalFile, SharedFile

        TestSharedWN.cleanUp()

        _ext = '.txt'

        file_1 = generate_unique_temp_file(_ext)
        TestSharedWN._managed_files.append(file_1)

        j = Job()
        j.inputfiles = [LocalFile(file_1)]
        j.outputfiles = [SharedFile(namePattern='*'+_ext, outputfilenameformat='{jid}/{fname}')]
        j.submit()
    ...
    def test_b_Completed(self):
        ...
        output_dir = os.path.join(TestSharedWN.outputFilePath, str(j.id))
    ...
    def test_c_SplitJob(self):
        ...
        from Ganga.GPI import Job, LocalFile, SharedFile, ArgSplitter
        ...
        j = Job()
        j.inputfiles = [LocalFile(file_1)]
        j.splitter = ArgSplitter(args = [[_] for _ in range(0, TestSharedWN.sj_len) ])
        j.outputfiles = [SharedFile(namePattern='*'+_ext, outputfilenameformat='{jid}/{sjid}/{fname}')]
        j.submit()
    ...
    def test_d_CompletedSJ(self):
        ...
        assert len(j.subjobs) == TestSharedWN.sj_len

        assert len(stripProxy(stripProxy(j.subjobs[0]).outputfiles[0]).subfiles) == 1
        assert len(j.subjobs[0].outputfiles) == 1

        for i in range(0, TestSharedWN.sj_len):
            output_dir = os.path.join(TestSharedWN.outputFilePath, str(j.id), str(i))
            assert os.path.isdir(output_dir)
    ...
    def test_e_MultipleFiles(self):
        ...
        from Ganga.GPI import LocalFile, SharedFile, Job, ArgSplitter
        ...
        TestSharedWN._managed_files.append(file_1)
        TestSharedWN._managed_files.append(file_2)
        TestSharedWN._managed_files.append(file_3)

        j = Job()
        j.inputfiles = [LocalFile(file_1), LocalFile(file_2), LocalFile(file_3)]
        j.splitter = ArgSplitter(args = [[_] for _ in range(0, TestSharedWN.sj_len) ])
        j.outputfiles = [SharedFile(namePattern='*'+_ext, outputfilenameformat='{jid}/{sjid}/{fname}'),
                         SharedFile(namePattern='*'+_ext2)]
        j.submit()
    ...
    def test_g_MultipleFiles(self):
        ...
        j.splitter = ArgSplitter(args = [[_] for _ in range(0, TestSharedWN.sj_len) ])
        j.outputfiles = [SharedFile(namePattern='*'+_ext, outputfilenameformat='{jid}_{sjid}_{fname}')]
        j.submit()
    ...
    def test_h_MultiUpload(self):
        ...
        for i in range(0, TestSharedWN.sj_len):
            ...
            file_prep = os.path.join(TestSharedWN.outputFilePath, str(j.id) + '_' + str(i) + '_')
