...
class TestSharedClient(GangaUnitTest):
    ...
    outputFilePath = '/tmp/SharedClient'
    ...
    def setUp(self):
        ...
        extra_opts=[('PollThread', 'autostart', 'False'),
                    ...
                    ('Output', 'SharedFile', TestSharedClient.SharedTestConfig),
                    ('Output', 'FailJobIfNoOutputMatched', 'True')]
        super(TestSharedClient, self).setUp(extra_opts=extra_opts)
    ...
    def cleanUp():
        ...
        for file_ in TestSharedClient._managed_files:
            os.unlink(file_)
        TestSharedClient._managed_files = []

        shutil.rmtree(TestSharedClient.outputFilePath, ignore_errors=True)
    ...
    def test_a_testClientSideSubmit(self):
        ...
        TestSharedClient.cleanUp()

        assert getConfig('Output')['SharedFile']['backendPostprocess']['Local'] == 'client'

        _ext = '.root'
        file_1 = generate_unique_temp_file(_ext)
        file_2 = generate_unique_temp_file(_ext)
        TestSharedClient._managed_files.append(file_1)
        TestSharedClient._managed_files.append(file_2)

        j = Job()
        j.inputfiles = [LocalFile(file_1), LocalFile(file_2)]
        j.splitter = ArgSplitter(args = [[_] for _ in range(0, TestSharedClient.sj_len) ])
        j.outputfiles = [SharedFile(namePattern='*'+_ext)]
        j.submit()
    ...
    def test_b_testClientSideComplete(self):
        ...
        for sj in j.subjobs:
            ...
            output_dir = os.path.join(TestSharedClient.outputFilePath, str(j.id), str(sj.id))
