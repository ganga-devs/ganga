...
class TestGangaList(GangaGPITestCase):
    ...
    def setUp(self):
        ...
        assert len(getProxyAttr(self.proxied1,'_list')) == len(self.plain1), 'Somthings wrong with construction'
        assert len(getProxyAttr(self.proxied2,'_list')) == len(self.plain2), 'Somthings wrong with construction'
    ...
    def testNonZero(self):
        """@ExpectedFailure"""
        assert not GangaList(), 'An empty GangaList should be false, just like a list'
    ...
    def testLE(self):
        ...
        assert (self.plain1 <= self.plain2) == (self.proxied1 <= self.proxied2), 'The lists should have the same le'
        assert (self.plain2 <= self.plain1) == (self.proxied2 <= self.proxied1), 'The lists should have the same le'

        assert (self.proxied1 <= self.proxied2) != (self.proxied2 <= self.proxied1), 'The le should invert correctly'
    ...
    def testSetSliceProxyList(self):
        ...
        assert self.plain1[3:7] == self.plain2[3:7], 'The lists should be equal'
        assert self.plain1[3:7] is not self.plain2[3:7], 'The lists should be copies'
        ...
        assert self.proxied1[3:7] == self.proxied2[3:7], 'The lists should be equal'
        assert self.proxied1[3:7] is not self.proxied2[3:7], 'The lists should be copies'
    ...
    def testCmp(self):
        ...
        assert cmp(self.proxied1,self.proxied2) == cmp(self.plain1,self.plain2)
