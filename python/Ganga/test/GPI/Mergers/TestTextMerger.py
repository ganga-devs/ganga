...
class TestTextMerger(GangaGPITestCase):
    ...
    def testOverWriteProtectionForce(self):
        ...
        tm = TextMerger()
        tm.files = ['out.txt']
        assert tm.merge(self.jobslice,tmpdir,overwrite = True), 'Merge should pass'
    ...
    def testMergeOnTransitionOneJob(self):
        ...
        for sj in j.subjobs:
            ...
            assert file_contains(output, out_txt), 'File must contain the output of each individual job'
    ...
    def testRecursiveMergeOnTransition(self):
        ...
        for o in out_list:
            assert file_contains(outfile,o), 'File must contain output from all subjobs'
    ...
    def testMergeWhenSubjobsHaveFailed(self):
        ...
        assert not file_contains(output, out_txt), 'The failed subjob must have been skipped'
    ...
    def testSingleJobWithNoSubjobs(self):
        ...
        assert not os.path.exists(os.path.join(tmpdir,'stdout')), 'No merge so no file'
    ...
    def testGZipFunction(self):
        ...
        for f in tm.files:
            assert os.path.exists(os.path.join(tmpdir,'%s.gz' % f)), 'GZip File Should exist'
