...
class CRABApp(IApplication):
            ...
            for key,item in cfp[section].schemadic.items():
                ...
                if attr != None:
                    if item.__class__.__name__ == 'SimpleItem' or self.force:
                        cfp[section].schemadic[key] = attr
                ...
                if cfp[section].schemadic[key].__class__.__name__ != 'SimpleItem':
                    cfile.append(section,'%s=%s'%(key,cfp[section].schemadic[key]))
                    setattr(job.inputdata,key,cfp[section].schemadic[key])
    ...
    def master_configure(self):
        ...
        job.application.writeCRABFile(job,cfgfile)
