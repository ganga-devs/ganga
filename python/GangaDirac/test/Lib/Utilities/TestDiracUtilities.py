...
class TestDiracUtilities(GangaGPITestCase):
    ...
    def test_execute_timeouts(self):
        ...
        ## Test timeouts
        self.assertEqual(execute('while true; do sleep 1; done', shell=True, timeout=10),
                         'Command timed out!')
        self.assertEqual(execute('while True: pass', timeout=10),
                         'Command timed out!')

        ## Test timeout doesn't hinder a normal command
        self.assertEqual(execute('import os\nprint os.getcwd()', timeout=10).strip(),
                         os.getcwd())
        self.assertEqual(execute('pwd',shell=True, timeout=10).strip(),
                         os.getcwd())
...
#                                      number=1) < 5)
    ...
    def test_execute_env(self):
        ...
        env = {'ALEX':'/hello/world', 'PATH':os.environ['PATH'], 'PYTHONPATH':os.environ['PYTHONPATH']}
    ...
    def test_execute_output(self):
        ...
        ## Test straight printout doesn't work without includes, stdout as str returned by default
