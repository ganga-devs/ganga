...
#        confirm = execute('execfile("%s")' % api_path_name)
        ...
        assert status[0][
            1] == 'Completed', 'job not completed properly: %s' % str(status)
...
#        os.remove(api_path_name)
    ...
    def test_getOutputDataInfo(self):
        ...
        self.assertEqual("%s" % type(
            confirm['InitTestFile.txt']), "<type 'dict'>", 'Command not executed successfully')
    ...
    def test_normCPUTime(self):
        ...
        self.assertEqual(
            "%s" % type(confirm), "<type 'str'>", 'Command not executed successfully')

    def test_getStateTime(self):
        confirm = execute('getStateTime("%s", "completed")' %
                          self.__class__._id)
        logging.info(str(confirm))
        self.assertEqual("%s" % type(
            confirm), "<type 'datetime.datetime'>", 'Command not executed successfully')

    def test_timedetails(self):
        confirm = execute('timedetails("%s")' % self.__class__._id)
        logging.info(str(confirm))
        self.assertEqual(
            "%s" % type(confirm), "<type 'dict'>", 'Command not executed successfully')
    ...
    def test_status(self):
        ...
        logging.info(str(confirm))
        self.assertEqual(
            "%s" % type(confirm), "<type 'list'>", 'Command not executed successfully')
    ...
    def test_replicateFile(self):
        ...
        confirm = execute('replicateFile("%s","%s","")' %
                          (self.__class__._getFileLFN, new_location))
        self.assertTrue(confirm['OK'], 'Command not executed successfully')
    ...
    def test_splitInputData(self):
        #lfn = self.lfn
        confirm = execute('splitInputData("%s","1")' %
    ...
    def test_uploadFile(self):
        #lfn = self.lfn
        new_lfn = '%s/upload_file' % os.path.dirname(
            self.__class__._getFileLFN)
        location = self.location
        ...
        add_file.close()
        confirm = execute(
            'uploadFile("%s","upload_file","%s")' % (new_lfn, location))
        self.assertEqual(
            "%s" % type(confirm), "<type 'dict'>", 'Command not executed successfully')
        confirm_remove = execute('removeFile("%s")' % new_lfn)
        self.assertTrue(
            confirm_remove['OK'], 'Command not executed successfully')
...
# Problematic tests
    ...
    def test_getInputDataCatalog(self):
        ...
        lfn = self.lfn
        #location = self.location
        confirm = execute('getInputDataCatalog("%s","","")' %
                          self.__class__._getFileLFN)
        self.assertEqual(confirm[
                         'Message'], 'Failed to access all of requested input data', 'Command not executed successfully')
        #self.assertTrue(confirm['OK'], 'Command not executed successfully')
        #print("REMOVEFILE:", execute('removeFile("%s")'%lfn))
    ...
    def test_getLHCbInputDataCatalog(self):
        ...
        #location = self.location
        confirm = execute(
            'getLHCbInputDataCatalog("%s",0,"","")' % (self.__class__._getFileLFN))
        self.assertEqual(confirm[
                         'Message'], 'Failed to access all of requested input data', 'Command not executed successfully')
        #self.assertTrue(confirm['OK'], 'Command not executed successfully')
        #print("REMOVEFILE:", execute('removeFile("%s")'%lfn))
