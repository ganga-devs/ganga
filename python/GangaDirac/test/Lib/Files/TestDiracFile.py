...
class TestDiracFile(GangaGPITestCase):
    ...
    def test__init__(self):
        self.assertEqual(
            self.df.namePattern, 'np',  'namePattern not initialised as np')
        self.assertEqual(
            self.df.lfn,         'lfn', 'lfn not initialised as lfn')
        self.assertEqual(
            self.df.localDir,    'ld',  'localDir not initialised as ld')
        ...
        self.assertEqual(
            d1.locations,   [], 'locations not initialised as empty list')

        d2 = DiracFile(namePattern='np', lfn='lfn', localDir='ld')
        self.assertEqual(
            d2.namePattern, 'np',  'namePattern not keyword initialised as np')
        self.assertEqual(
            d2.lfn,         'lfn', 'lfn not keyword initialised as lfn')
        self.assertEqual(
            d2.localDir,    'ld',  'localDir not keyword initialised as ld')

    def test__attribute_filter__set__(self):
        self.assertEqual(self.df._attribute_filter__set__(
            'dummyAttribute', 12), 12, 'Pass throught of non-specified attribute failed')
        self.assertEqual(self.df._attribute_filter__set__(
            'lfn', 'a/whole/newlfn'), 'a/whole/newlfn', "setting of lfn didn't return the lfn value")
        self.assertEqual(self.df.namePattern, 'newlfn',
                         "Setting the lfn didn't change the namePattern accordingly")
        self.assertEqual(self.df._attribute_filter__set__(
            'localDir', '~'), os.path.expanduser('~'), "Didn't fully expand the path")
    ...
    def test__on_attribute__set__(self):
        ...
        self.assertNotEqual(
            d2, self.df, "didn't modify properly when called with Job and outputfiles")
        self.assertEqual(
            d2.namePattern, self.df.namePattern, 'namePattern should be unchanged')
        self.assertEqual(d2.localDir, None, "localDir should be blanked")
        self.assertEqual(d2.lfn, '', "lfn should be blanked")

    def test__repr__(self):
        self.assertEqual(repr(self.df), "DiracFile(namePattern='%s', lfn='%s')" % (
            self.df.namePattern, self.df.lfn))
