...
class NasimRTHandler(IRuntimeHandler):
    ...
    def prepare(self,app,appsubconfig,appmasterconfig,jobmasterconfig):
        ...
        # Hack for splitting - should be done with proper datasets!
        if app.dataset_name == '':
            environment['NA48_DATASET_NAME']= 'users_run' + str(app.run_number) + '_beam' + str(app.beam) + '_trigs' + str(app.num_triggers) + '_' + time.strftime('%b_%a_%m_%d_%H_%M_%S')
        else:
            environment['NA48_DATASET_NAME']= app.dataset_name
