...
class TestProxy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.p = GangaCore.GPIDev.Base.Proxy.addProxy(new_object)
    ...
    def test_set_nonexistant(self):
        ...
        self.assertRaises(GangaCore.Core.exceptions.GangaAttributeError, _assign)
    ...
    def test_get_nonexistent(self):
        ...
        self.assertRaises(GangaCore.Core.exceptions.GangaAttributeError, _get)
    ...
    def test_set_wrong_type(self):
        ...
        self.assertRaises(GangaCore.Core.exceptions.TypeMismatchError, _assign)

    def test_stripProxy(self):
        """
        Check that stripping the proxy returns the original class
        """
        stripped = GangaCore.GPIDev.Base.Proxy.stripProxy(self.p)
        # TODO In Python >= 2.7 use
        #  self.assertIsInstance(stripped, TestGangaObject)
        self.assertEqual(type(stripped), SampleGangaObject)

    def test_isType(self):
        self.assertTrue(GangaCore.GPIDev.Base.Proxy.isType(self.p, SampleGangaObject))

    def test_isProxy(self):
        self.assertTrue(GangaCore.GPIDev.Base.Proxy.isProxy(self.p))

    def test_make_proxy_proxy(self):
        self.assertTrue(GangaCore.GPIDev.Base.Proxy.addProxy(self.p) is self.p)
    ...
    def test_proxy_layer_function(self):
        ...
        p2 = GangaCore.GPIDev.Base.Proxy.addProxy(SampleGangaObject())
        ret = self.p.check_not_proxy(p2)
        assert GangaCore.GPIDev.Base.Proxy.isProxy(ret), 'returned object should be proxy-wrapped'

    def test_call_nonproxied_method(self):
        def _call():
            self.p.not_proxied()

        self.assertRaises(GangaCore.Core.exceptions.GangaAttributeError, _call)
