"""Utilities for using MSG within GangaCore.
"""

# default stomp.py logging to CRITICAL

import GangaCore.Utility.Config as Config
...
def createPublisher(server, port, user='ganga', password='analysis', idle_timeout=None, exit_timeout=None):
    ...
    Usage::
        from GangaCore.Lib.MonitoringServices.MSG import MSGUtil
    ...
    from GangaCore.Utility.stomputil.publisher import IDLE_TIMEOUT, EXIT_TIMEOUT
    ...
    try:
        from GangaCore.Core.GangaThread import GangaThread as Thread
        managed_thread = True
    except ImportError:
    ...
    import GangaCore.Utility.stomputil
    publisher = GangaCore.Utility.stomputil.createPublisher(Thread, server, port, user, password, logger, idle_timeout)
    if managed_thread:
