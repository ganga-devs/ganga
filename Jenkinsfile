  ...
  stages {
    ...
    stage('Test') {
      parallel {
        stage('GangaCore') {
          steps {
            sh "docker run --name GangaCore${env.BRANCH_NAME} gangacoretest:${env.BRANCH_NAME} /root/ganga/python/GangaCore/test/Unit --cov-report term --cov-report xml:cov-GangaCore.xml --cov /root/ganga/python/GangaCore  --junitxml tests-GangaCore.xml || true"
