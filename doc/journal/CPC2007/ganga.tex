...
\begin{abstract}
  ...
  \qq{Scientific user communities often face a problem of usability of
the end-user interfaces for \grid access. In addition, to fully
exploit all available distributed computing resources users are
confronted with the problems of transition between different
distributed systems in the everyday work.  In this paper,} we present
the computational task-management tool \ganga, which allows for the
specification, submission, bookkeeping and post processing of
...
demonstrating how an analysis can be developed on a local system and
then transparently moved to a \grid system for processing of all
available data.  \ganga has an API that can be used via an interactive
interface, in scripts, or through a GUI. Specific knowledge about
types of tasks or computational resources is provided at run-time
through a plugin system, making new developments easy to integrate. We
give an overview of the \ganga architecture, give examples of current
use, and demonstrate how \ganga can be used in many different areas of
science.
\end{abstract}
...
\qq{
Scientific communities are using a growing number of distributed systems,
from local batch systems and community-specific services to generic,
global Grid infrastructures.  Users often debug applications using a
desktop computer, then perform small-scale application testing using
local resources and finally run at full-scale using globally
distributed Grids. Sometimes new resources are made available to the
users through systems previously unknown to them. Manual handling of
the diversity of the interfaces and intrinsically different system
architectures requires a lot of effort from the end-users and is
counter-productive. The cost of mastering application configuration,
...
gLite \cite{gLite}. Rather than being a replacement, \ganga provides an
interoperability layer for existing technologies . }
...
install \grid or batch submission tools nor the application
...
customisation and configuration of \ganga at a site, workgroup and user level.
}
...
collection mechanism is used transparently to the user. \qq{ Job
splitting functionality provides a flat list of subjobs suitable for
embarrassingly parallel style workloads. However, certain backends
allow users to specify special job types, e.g.  Message Passing Interface
(MPI)~\cite{MPI} jobs. In this case \ganga may be used to manage
collections of subjobs which correspond to MPI processes.}
...
\qq{
\ganga supports multiple security models which are defined by each backend implementation separately.
...
security infrastructure (GSI) \cite{GSI} provides security across
organizational boundaries for the \grid backends. The \ganga framework
allows for encapsulating different security models in pluggable components
which may be simultaneously used in the same \ganga session.
}
...
\begin{description}
\item[\code{queue~~~~~~:}] name of queue to which job should be submitted\footnote{\qq{If the queue is not specified a default queue which is configured by the batch system is used.}},
\end{description}
...
work. \qq{Plugins which are developed and maintained by the \ganga
team are included in the main \ganga distribution and upgraded
...
specific to \atlas and \lhcb. Plugins specific to a other user
communities should be installed separately but could also be easily
integrated in the main \ganga distribution if needed.  }
...
\qq{
Most of the plugins interact to with the underlying backends using
shell commands.  This {\em down-to-earth} approach is particularly
useful for the encapsulation of environments of different subsystems
and resolution of environment clashes. In the verbose mode \ganga
prints the executed commands so a user may reproduce the commands
externally if needed. Higher-level abstractions such as
JSDL \cite{JSDL}, OGSA-BES \cite{OGSA-BES} or SAGA API \cite{SAGA} are
not currently used for the implementation but specific backends which
support these standards could readily be added if needed.  }
...
system where user jobs are queued in the WMS server. Generic pilot
scripts are submitted to the \grid by WMS in the background.  A pilot
...
available, it is pulled from the WMS and started.  Otherwise, the
pilot terminates and the WMS sends a new pilot to the \grid.  This
system improves the reliability of the \grid system as seen by the
user. \ganga provides a \dirac backend component that supports
submission of jobs to the \dirac WMS, making use internally
of \dirac's \python API~\cite{DIRACAPI}.
}
...
\qq{The open plugin architecture of \ganga allows to integrate other
\grid middleware easily.} An example of the \ARC (Advanced Resource Connector)
...
e.g. by researchers in bioinformatics, genomics, meteorology, in
...
\qq{ Existing command line submission interfaces such as gLite do not have enough usability designed 
into them. Some higher level tools such as GridWay\cite{GridWay}
present jobs as if they were Unix processes and corresponding command
line utilities. Interfaces based on Condor job-submission scripts have
also been developed \cite{MCS}. A distinctive feature of \ganga is
that it may be easily adapted to different working styles of the users
who may simultaneously use three different interfaces. \ganga also
provides a higher abstraction level than most of the other tools and
allows a user to focus on solving the domain-specific problems rather
than changing their way of working each time they switch to a new interface.
}
