...
\author[a:CERN]{Jakub T. Mo{\'s}cicki\corauthref{cor1}}
...
according to user-defined criteria and the output merged at a later stage. Each
...
the values set for the component properties, and may require that
these values be used to derive secondary information.  For example,
one of the properties might specify the path to a file that needs to
be read.  The post-processing step can be useful for validation tasks
such as determination of the validity of the application output.
...
certificates {\bf TODO(explain about classic and VOMS proxies, maybe later...)} . The performance tests of both the local and remote repositories
...
multiple locations in a way that is quite similar to that provided by  IMAP for email systems.
...
for interacting with the \grid, and Kerberos \cite{Kerberos} tickets for interacting with a
...
\vspace{-2ex}
Finally we provide a view of the graphical user interface {\bf TODO:MISSING!}. The overview of jobs
