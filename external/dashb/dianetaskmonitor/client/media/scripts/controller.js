...
function Controller() {
    ...
    this.Data = new Data($('#ajaxAnimation'), this.Settings.Application);
    ...
    this.viewUpdater = function() {
        ...
        if (this.Data.user || !_Settings.userSelection) {
            if (this.Data.tid) {
                ...
                if (this.Settings.Subs.charts !== undefined) $("#siteTabs").tabs("enable",1); // Enable charts tab
                ...
                if (this.Settings.Mains.charts !== undefined) $("#siteTabs").tabs("enable",1); // Enable charts tab
                else $("#siteTabs").tabs("disable",1); // Disable charts tab
                $("#siteTabs").tabs("select",0); // Select data table tab
    ...
    this.drawUsers = function() {
        ...
        // Hide filters panel
        if (thisRef.Settings.Application.filters !== undefined) $('#dataFilters').hide();
    ...
    this.drawMainsTable = function() {
        ...
        var draw = function(data) {
            thisRef.mainsTable = $('#tableContent').lkfw_dataTable({
                ...
                fnERContentPostProcess:function(expandedID){ return thisRef.expand_click_postprocess(expandedID) },
        ...
        var getData = function(data) {
            ...
            // Hide filters panel
            if (thisRef.Settings.Application.filters !== undefined) $('#dataFilters').show();
        ...
        try {
            ...
            for (var i=0;i<_charts.length;i++) {
                ...
                if (_charts[i].dataURL) {
                    if (_charts[i].dataURL_params === undefined) _charts[i].dataURL_params = function() { return {}; };
    ...
    this.setupURL = function() {
        ...
        if (_Settings.filters !== undefined) {
            for (var i=0;i<_Settings.filters.length;i++) {
                urlHash[_Settings.filters[i].urlVariable] = this.Data.filters[_Settings.filters[i].urlVariable];
            }
        }
