...
function Data(ajaxAnimation, _Settings, jsonp) {
    var settings = _Settings.modelDefaults();
    var jsonp = _Settings.jsonp
    // general values
    ...
    if (_Settings.filters !== undefined) {
        for (var i=0;i<_Settings.filters.length;i++) {
            this.filters[_Settings.filters[i].urlVariable] = this.copyVal(_Settings.filters[i].value);
        }
    }
    ...
    this.quickSetup = function(params, ts2iso) {
        ...
        if (_Settings.filters !== undefined) {
            for (var i=0;i<_Settings.filters.length;i++) {
                this.filters[_Settings.filters[i].urlVariable] = (params[_Settings.filters[i].urlVariable] || this.copyVal(_Settings.filters[i].value));
            }
        }
    };
