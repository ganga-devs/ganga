function Model() {
    ...
    this.ajax_getTableData = function(f, user, query, from, till) {
        ...
        $.ajax({
            type: "GET",
            url: settings_HOST+settings_PORT+"/dianeruns?user="+user+from_url+till_url+base64query,
            dataType: "json",
    ...
    this.ajax_getSubjobs = function(f, job_uuid) {
        ...
        $.ajax({
            type: "GET",
            url: settings_HOST+settings_PORT+"/dianedetails?master_uuid="+job_uuid,
            dataType: "json",
            success: function(data) {
                _sub_cache.add(key, data, 0);
                $('#ajaxLoading').hide();
                f(data); 
            },
            error: thisRef.ajax_error
