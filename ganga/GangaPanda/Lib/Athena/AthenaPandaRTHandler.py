...
def createContainer(name):
    ...
    if not configPanda['processingType'].startswith('gangarobot') and not configPanda['processingType'].startswith(
            'hammercloud') and not configPanda['processingType'].startswith('rucio_test'):
        try:
...
class AthenaPandaRTHandler(IRuntimeHandler):
    ...
    def master_prepare(self, app, appconfig):
        ...
        # create a random number for this submission to allow multiple use of
        # containers
        self.rndSubNum = random.randint(1111, 9999)

        job = app._getParent()
        logger.debug(
            'AthenaPandaRTHandler master_prepare called for %s',
            job.getFQID('.'))
        ...
        if self.dbrelease != '' and self.dbrelease.find(':') == -1:
            raise ApplicationConfigurationError(
                "ERROR : invalid argument for DB Release. Must be 'DatasetName:FileName'")
        ...
        if app.atlas_exetype not in [
            ...
            'EXE',
                'TRF']:
            raise ApplicationConfigurationError(
                "Panda backend supports only application.atlas_exetype in ['ATHENA','PYARA','ARES','ROOT','EXE', 'TRF']")
        ...
        if app.is_prepared is not True:
            ...
            tmp_user_area_name = os.path.join(
                os.path.join(
                    shared_path, app.is_prepared.name), os.path.basename(
                    app.user_area.name))
        ...
        if job.inputdata:
            if job.inputdata._name == 'DQ2Dataset':
                ...
                raise ApplicationConfigurationError(
                    'Panda backend supports only inputdata=DQ2Dataset()')
        ...
        if app.atlas_exetype == 'TRF':
            self.job_options += ' '.join([os.path.basename(fopt.name)
                                          for fopt in app.option_file])

            if not job.outputdata.outputdata:
                raise ApplicationConfigurationError(
                    "job.outputdata.outputdata is required for atlas_exetype in ['PYARA','ARES','TRF','ROOT','EXE' ] and Panda backend")
            ...
            if len(app.atlas_environment) > 0 and app.atlas_environment[0].find(
                    'DBRELEASE_OVERRIDE') == -1:
                logger.warning(
        ...
        if self.inputdatatype == 'DQ2':
            if not job.splitter:
                ...
                raise ApplicationConfigurationError(
                    'Splitting with Panda+DQ2Dataset requires DQ2JobSplitter')
        elif self.inputdatatype == 'Tier3':
            if job.splitter and job.splitter._name != 'ATLASTier3Splitter':
                raise ApplicationConfigurationError(
                    'Splitting with Panda+ATLASTier3Dataset requires ATLASTier3Splitter')
            if job.backend.site == 'AUTO':
                raise ApplicationConfigurationError(
                    'Panda+ATLASTier3Dataset requires a specified backend.site')
        ...
        if len(job.subjobs) == 0 and job.backend.site == 'AUTO':
            raise ApplicationConfigurationError(
                'Error: backend.site=AUTO after brokerage. Report to DA Help Forum')
        ...
        try:
            ...
            raise BackendError(
                'Panda', 'Exception in Client.getDatasets %s: %s %s' %
                (job.outputdata.datasetname, sys.exc_info()[0], sys.exc_info()[1]))
        if job.outputdata.datasetname not in res.keys():
            # create the container
            createContainer(job.outputdata.datasetname)

        else:
            logger.info(
                'Adding datasets to already existing container %s' %
                job.outputdata.datasetname)
        ...
        for site in bjsites:
            ...
            try:
                tmpDsExist = False
                if (configPanda['processingType'].startswith('gangarobot') or configPanda['processingType'].startswith(
                        'hammercloud') or configPanda['processingType'].startswith('rucio_test')):
                    # RUCIO patch
                ...
                raise BackendError(
                    ...
                    (tmpDSName, sys.exc_info()[0], sys.exc_info()[1]))
                ...
                if not job.backend.nobuild:
                    try:
                        if not configPanda['processingType'].startswith('gangarobot') and not configPanda['processingType'].startswith(
                                'hammercloud') and not configPanda['processingType'].startswith('rucio_test'):
                            Client.addDataset(
                                self.libDatasets[site], False, location=self.outDsLocation)
                            logger.info(
                                'Lib dataset %s registered at %s' %
                                (self.libDatasets[site], self.outDsLocation))
                        dq2_set_dataset_lifetime(
                            self.libDatasets[site], self.outDsLocation)
                    except exceptions.SystemExit:
                        raise BackendError(
                            'Panda', 'Exception in Client.addDataset %s: %s %s' %
                            (self.libDatasets[site], sys.exc_info()[0], sys.exc_info()[1]))
        ...
        if job.inputsandbox:
            ...
            if tmp_user_area_name:
                rc, output = commands.getstatusoutput(
                    'cp %s %s.gz' %
                    (tmp_user_area_name, self.inputsandbox))
                if rc:
                ...
                rc, output = commands.getstatusoutput(
                    'gunzip %s.gz' % (self.inputsandbox))
                if rc:
                    logger.error(
                        'Unzipping user_area failed with status %d', rc)
                    logger.error(output)
                    raise ApplicationConfigurationError(
                        'Packing inputsandbox failed.')
            ...
            for fname in [os.path.abspath(f.name) for f in job.inputsandbox]:
                ...
                if not rc:
                    ...
                    # as we don't have to be in the run dir now, create a copy
                    # of the run_dir directory structure and use that
                    ...
                    if not rc:
                        ...
                        if not rc:
                            ...
                            raise ApplicationConfigurationError(
                                "Couldn't copy file %s to recreate run_dir for input sandbox" % fname)
                    else:
                        raise ApplicationConfigurationError(
                    ...
                    # strip the path from the filename if present in the
                    # userarea
                ...
                rc, output = commands.getstatusoutput(
                    'tar -h -r -f %s -C %s %s' %
                    (self.inputsandbox, path, fn))
                if rc:
                    logger.error(
                        'Packing inputsandbox failed with status %d', rc)
                    logger.error(output)
                    raise ApplicationConfigurationError(
                        'Packing inputsandbox failed.')
        ...
        if self.inputsandbox and not job.backend.libds:
            uploadSources(
                os.path.dirname(
                    self.inputsandbox), os.path.basename(
                    self.inputsandbox))

            if not self.inputsandbox == tmp_user_area_name:
                logger.info(
                    'Removing source tarball %s ...' %
                    self.inputsandbox)
        ...
        if app.athena_compile:
            ...
            for bjsite in bjsites:
                ...
                if app.useMana:
                    jspec.cmtConfig = AthenaUtils.getCmtConfig(
                    ...
                    # set the home package if we can otherwise the pilot won't
                ...
                if (job.backend.bexec != '') or (job.backend.requirements.rootver !=
                                                 '') or app.useRootCore or app.useMana or app.atlas_exetype in ['PYARA', 'ARES', 'ROOT', 'EXE']:
                ...
                if Client.isDQ2free(bjsite):
                    jspec.destinationDBlock = '%s/%s' % (
                        job.outputdata.datasetname, self.libDatasets[bjsite])
                ...
                if self.inputsandbox:
                    jspec.jobParameters += ' -i %s' % (
                        os.path.basename(self.inputsandbox))
                matchURL = re.search(
                    '(http.*://[^/]+)/', Client.baseURLCSRVSSL)
                if matchURL:
                    jspec.jobParameters += ' --sourceURL %s' % matchURL.group(
                        1)
...
#       if no outputdata are given
        ...
        if not configPanda['processingType'].startswith('gangarobot') and not configPanda['processingType'].startswith(
                'hammercloud') and not configPanda['processingType'].startswith('rucio_test'):
            job.outputdata.datasetname += '.%d.%s' % (
                self.rndSubNum, job.backend.site)

        if job.inputdata and self.inputdatatype == 'DQ2':
            if len(job.inputdata.dataset) > 1:
                raise ApplicationConfigurationError(
                    'Multiple input datasets per subjob not supported. Use a container dataset?')
        ...
        if app.useMana:
            ...
            # set the home package if we can otherwise the pilot won't set
            # things up properly
            if self.cacheVer:
                jspec.homepackage = 'AnalysisTransforms' + self.cacheVer
        ...
        if job.inputdata and self.inputdatatype == 'DQ2' and (
            not job.inputdata.tag_info or app.atlas_exetype in [
                'PYARA', 'ARES', 'ROOT', 'EXE']):
        ...
        if Client.isDQ2free(job.backend.site):
            jspec.destinationSE = 'local'
        # and (not job._getRoot().subjobs or job.id == 0):
...
#       output files
        outMap = {}
        AthenaUtils.convertConfToOutputOld(
            self.runConfig,
            jspec,
            outMap,
            job.backend.individualOutDS,
            self.extOutFile,
            masterjob.outputdata.datasetname)
        ...
        if job.backend.individualOutDS:
            for f in jspec.Files:
                if f.type in ['output', 'log']:
                    if f.dataset not in self.indivOutContList:
                        logger.info('Creating output container %s' % f.dataset)
                        createContainer(f.dataset)
                        self.indivOutContList.append(f.dataset)
                    if f.destinationDBlock not in self.indivOutDsList:
                        try:
                            logger.info(
                                'Creating dataset %s and adding to %s' %
                                (f.destinationDBlock, f.dataset))
                            if not configPanda['processingType'].startswith('gangarobot') and not configPanda['processingType'].startswith(
                                    'hammercloud') and not configPanda['processingType'].startswith('rucio_test'):
                                Client.addDataset(
                                    f.destinationDBlock, False, location=subjobOutputLocation)
                            dq2_set_dataset_lifetime(
                                f.destinationDBlock, subjobOutputLocation)
                            self.indivOutDsList.append(f.destinationDBlock)
                            addDatasetsToContainer(
                                f.dataset, [f.destinationDBlock])
                        except exceptions.SystemExit:
                            raise BackendError(
                                'Panda', 'Exception in Client.addDataset %s: %s %s' %
                                (f.dataset, sys.exc_info()[0], sys.exc_info()[1]))
...
#       job parameters
        ...
        if app.atlas_exetype in ['PYARA', 'ARES', 'ROOT', 'EXE']:
            ...
            if job.inputdata and job.inputdata._name == 'DQ2Dataset':
                self.job_options = self.job_options.replace(
                    "%INDS", job.inputdata.dataset[0].strip('/'))
        ...
        if app.atlas_exetype == 'ARES' or (
            app.atlas_exetype in [
                'PYARA',
                'ROOT',
                'EXE'] and app.useAthenaPackages):
            param += '--useAthenaPackages '

        if app.atlas_exetype in [
            'PYARA',
            'ROOT',
                'EXE'] and job.backend.requirements.rootver != '':
        ...
        # DBRelease
        if self.dbrelease != '' and (
            app.atlas_exetype not in ['TRF'] or (
                job.inputdata and self.inputdatatype == 'DQ2' and (
                    job.inputdata.tag_info or len(
                job.inputdata.tagdataset) != 0))):
        ...
        if job.inputdata:
            ...
            if self.inputdatatype == 'DQ2' and job.inputdata.tag_info:
                ...
                if job.inputdata.use_cvmfs_tag:
                    ...
                    for tag in job.inputdata.tag_info.keys():
                        tmpTagList.append(
                            "/cvmfs/atlas-condb.cern.ch/repo/tag/%s/%s" %
                            (job.inputdata.tag_info[tag]['dataset'], tag))
                    param += '--tagFileList %s ' % ','.join(tmpTagList)
                else:
                    input_files += job.inputdata.tag_info.keys()
                    param += '--tagFileList %s ' % ','.join(
                        job.inputdata.tag_info.keys())
                ...
                # sort out TAG use for exe types other than just athena - TRF
                # dealt with below
                if app.atlas_exetype in ['PYARA', 'ARES', 'ROOT', 'EXE']:
            ...
            if app.atlas_exetype not in ['TRF']:
                param += '-i "%s" ' % input_files
        else:
            param += '-i "[]" '
        ...
        if app.atlas_exetype in ['TRF']:
            ...
            if app.atlas_exetype in [
                    'TRF'] and job.backend.accessmode == 'DIRECT':
                param += ' --directIn '

            inPattList = [('%IN', input_files), ('%MININ', minList),
                          ('%CAVIN', cavList), ('%BHIN', bhaloList), ('%BGIN', bgasList)]
            for tmpPatt, tmpInList in inPattList:
                if tmpJobO.find(tmpPatt) != -1 and len(tmpInList) > 0:
                    tmpJobO = AthenaUtils.replaceParam(
                        tmpPatt, tmpInList, tmpJobO, useNewTRF)
           ...
           # DBRelease
            ...
            if self.runConfig.input and self.runConfig.input.inBS:
                inBS = True
            isDirectAccess = PsubUtils.isDirectAccess(
                job.backend.site, inBS, inTRF, inARA)
            ...
            if not isDirectAccess and (
                job.backend.accessmode != 'DIRECT') and (
                (self.inputdatatype != 'DQ2') or (
                    len(
                    job.inputdata.tagdataset) == 0 and not job.inputdata.tag_info)):
                param += ' --useLocalIO '
        ...
        if (self.runConfig.input and self.runConfig.input.inColl) or param.find(
                '--collRefName') != -1:
            param += '-c '
        if self.runConfig.input and self.runConfig.input.inBS:
            param += '-b '
        ...
        if app.atlas_exetype == 'ATHENA' and app.max_events > 0:
            param += '-f "theApp.EvtMax=%d;EventSelector.SkipEvents=%s" ' % (
                app.max_events, nEventsToSkip)
        ...
        if app.atlas_release.split('.')[0].isdigit() and int(
                app.atlas_release.split('.')[0]) > 20:
            param += '--useCMake '
...
#            param += '--ara '
        if job.backend.accessmode == 'FILE_STAGER':
            ...
            param += '--accessmode=copy '
        # and not app.atlas_exetype in ['PYARA','ARES','ROOT','EXE']:
