    ...
    _schema = Schema(
            ...
            0),
        {
            'exe': SimpleItem(
                ...
                typelist=['GangaCore.GPIDev.Lib.File.File.File'],
                doc='File object containing the pilot startup script. Do not modify unless you know what you are doing.'),
            'arg_pattern': SimpleItem(
                defvalue='-s %%QUEUE%% -h %%QUEUE%% -j false -u self',
                typelist=['str'],
                doc="Pilot argument pattern. Do not modify unless you know what you are doing."),
            'queue': SimpleItem(
                defvalue='',
                typelist=['str'],
                doc='The analysis queue to register the pilot on e.g. ANALY_TRIUMF')})
...
def queueToCE(queue):
    ...
    return Client.PandaSites[queue]['queue'] + \
        '-' + Client.PandaSites[queue]['localqueue']
    ...
    def prepare(self, app, appconfig, appmasterconfig, jobmasterconfig):
        ...
        if not app.queue:
            raise ApplicationConfigurationError(
                'queue not specified for PandaPilot')
        ...
        if not job.backend.requirements.sites:
            ...
            if not job.backend.requirements.sites:
                raise ApplicationConfigurationError(
                    'Could not map queue name to LCG site')
        logger.info('Sending pilot for %s to %s' %
                    (app.queue, job.backend.requirements.sites))
        return LCGJobConfig(app.exe, app._getParent().inputsandbox, [
                            args], app._getParent().outputsandbox, None)
