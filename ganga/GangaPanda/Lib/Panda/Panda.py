...
try:
    ...
    logger.error(
        "Failed to correctly configure Client.baseURL or Client.baseURLSSL ")
...
try:
    agisinfos = simplejson.load(urllib2.urlopen(
        "http://atlas-agis-api.cern.ch/request/site/query/list/?json&state=ACTIVE&rc_site_state=ACTIVE"))
except BaseException:
    agisinfos = []
    logger.error(
        "Failed to load AGIS info from http://atlas-agis-api.cern.ch/request/site/query/list/?json&state=ACTI\
...
def setChirpVariables():
    ...
    if configPanda['chirpserver'] and not configPanda['chirpconfig']:
        tempchirpconfig = 'chirp^%s^/%s^-d chirp' % (
            configPanda['chirpserver'], nickname)
        GangaCore.Utility.Config.setConfigOption(
            'Panda', 'chirpconfig', tempchirpconfig)
    return
    ...
    for location in locations:
        ...
        for queue, queueinfo in Client.PandaSites.iteritems():
            ...
            for queuelocation in queuelocations:
                if Client.convSrmV2ID(location) == Client.convSrmV2ID(
                        queuelocation) and queue not in sites:
                    sites.append(queue)
        info[location] = sites
    return info
...
def convertQueueNameToDQ2Names(queue):
    ...
    if not convertQueueNameToDQ2Names.pandaresources:
        if not os.path.exists(config['AGISJSONFile']):
            logger.warning(
                "Cannot find AGIS file '%s' - falling back to using Panda Client" %
                config['AGISJSONFile'])
        else:
            with open(config['AGISJSONFile'], 'r') as agis_file:
                convertQueueNameToDQ2Names.pandaresources = dict(
                    [(q['name'], q) for q in json.loads(agis_file.read())])
    ...
    if convertQueueNameToDQ2Names.pandaresources:
        ...
        if not ddm_names:
            logger.error(
                "Could not find queue name '%s' in AGIS file or ddm info not present. "
                "Returning no sites" %
                queue)
            return []
...
def queueToAllowedSites(queue):
    ...
    try:
        ...
        raise BackendError(
            'Panda',
            'Queue %s has no ddm field in SchedConfig' %
            queue)
    allowed_sites = []
    alternate_names = []
    for site in ToACache.sites:
            ...
            try:
                if ddm == site:
                    ...
                    for alternate_name in alternate_names:
                        if (
                                alternate_name in ToACache.sites[site]['alternateName']):
                            allowed_sites.append(site)
...
def runPandaBrokerage(job):
    ...
    if job.backend.site == "AUTO":
        ...
        if job.backend.libds:
            try:
                ...
                raise BackendError(
                    'Panda', 'Error in Client.getLocations for libDS')
            if not libdslocation:
                raise ApplicationConfigurationError(
                    'Could not locate libDS %s' % job.backend.libds)
            else:
                ...
                try:
                    ...
                    raise BackendError(
                        'Panda',
                        'Could not map libds site %s to a cloud' %
                        libdslocation)
        ...
        if job.inputdata:
            try:
                ...
                try:
                    ...
                    raise ApplicationConfigurationError(
                        'Could not determine input datasetname for Panda brokerage')
            if not dataset:
                raise ApplicationConfigurationError(
                    'Could not determine input datasetname for Panda brokerage')
            ...
            try:
                ...
                raise BackendError(
                    'Panda', 'Error in Client.queryFilesInDataset')
            try:
                ...
                if not dsLocationMap:
                    logger.info(
                        'Dataset not found in cloud %s, searching all clouds...' %
                        job.backend.requirements.cloud)
                    # RUCIO patch
            ...
            if dsLocationMap == {}:
                raise BackendError(
                    'Panda', "ERROR : could not find supported locations in the %s cloud for %s" %
                    (job.backend.requirements.cloud, dataset))
            # run brokerage
            ...
            for site, spec in Client.PandaSites.iteritems():
                if spec['cloud'] == job.backend.requirements.cloud and spec['status'] == 'online' and not Client.isExcudedSite(
                        site):
    ...
    if not tmpSites:
        raise BackendError(
            'Panda', "ERROR : could not find supported locations in the %s cloud for %s, %s" %
    ...
    try:
        if job.application.atlas_production == '':
            ...
            tag = '%s-%s' % (job.application.atlas_project,
    ...
    try:
        status, out = Client.runBrokerage(
            tmpSites, tag, verbose=False, trustIS=config['trustIS'], processingType=config['processingType'])
    except exceptions.SystemExit:
        job.backend.reason = 'Exception in Client.runBrokerage: %s %s' % (
            sys.exc_info()[0], sys.exc_info()[1])
        raise BackendError(
            'Panda', 'Exception in Client.runBrokerage: %s %s' %
            (sys.exc_info()[0], sys.exc_info()[1]))
    if status != 0:
        job.backend.reason = 'Non-zero to run brokerage for automatic assignment: %s' % out
        raise BackendError(
            'Panda',
            'Non-zero to run brokerage for automatic assignment: %s' %
            out)
    if out not in Client.PandaSites:
        job.backend.reason = 'brokerage gave wrong PandaSiteID:%s' % out
        raise BackendError(
            'Panda',
            'brokerage gave wrong PandaSiteID:%s' %
            out)
    # set site
    ...
    logger.info('Panda brokerage results: cloud %s, site %s' %
                (job.backend.requirements.cloud, job.backend.site))
...
def selectPandaSite(job, sites):
    ...
    try:
        status, out = Client.runBrokerage(
            pandaSites, tag, verbose=False, trustIS=config['trustIS'], processingType=config['processingType'])
    except exceptions.SystemExit:
        job.backend.reason = 'Exception in Client.runBrokerage: %s %s' % (
            sys.exc_info()[0], sys.exc_info()[1])
        raise BackendError(
...
def uploadSources(path, sources):
    ...
    logger.info(
        'Uploading source tarball %s in %s to Panda...' %
        (sources, path))
    try:
        cwd = os.getcwd()
        os.chdir(path)
        ...
        if output != 'True':
            if 'Cannot overwrite file' not in output:
                logger.error(
                    'Uploading sources %s/%s failed. Status = %d',
                    path,
                    sources,
                    rc)
                logger.error(output)
                raise BackendError(
                    'Panda', 'Uploading sources to Panda failed')
            else:
                logger.warning(
                    'Uploading sources %s/%s failed. Status = %d',
                    path,
        ...
        raise BackendError(
            'Panda', 'Exception while uploading archive: %s %s' %
            (sys.exc_info()[0], sys.exc_info()[1]))
...
def getLibFileSpecFromLibDS(libDS):
    ...
    if len(tmpFileList) != 1:
        raise BackendError(
            'Panda', "dataset %s contains multiple lib.tgz files : %s" %
            (libDS, tmpFileList))
...
def checkMergeJobStatus(job, pandaJobDefId):
    ...
    if ec == 0:
        ...
        logger.error(
            'checkMergeGenerationStatus returns non-zero exit code: %d' %
            ec)
                ...
                for id in mergeJobDefIds:
                    ...
                    if ec2 == 0:
                        ...
                        for jid, jinfo in mjs.items():
                            ...
                            if mjobj.id not in [
                                    mj2.id for mj2 in job.backend.mergejobs]:
...
def checkForRebrokerage(string):
    import re
    matchObj = re.match(
        r'reassigned to another site by rebrokerage. new PandaID=(\d+) JobsetID=(\d+) JobID=(\d+)',
        string)
    if matchObj:
        ...
        Version(
            ...
            'id': SimpleItem(
                defvalue=None, typelist=[
                    'type(None)', 'int'], protected=0, copyable=0, doc='Panda Job id'), 'status': SimpleItem(
                        defvalue=None, typelist=[
                            'type(None)', 'str'], protected=0, copyable=0, doc='Panda Job status'), 'jobSpec': SimpleItem(
                                defvalue={}, optional=1, protected=1, copyable=0, doc='Panda JobSpec'), 'url': SimpleItem(
                                    defvalue=None, typelist=[
                                        'type(None)', 'str'], protected=1, copyable=0, doc='Web URL for monitoring the job.')})
...
class Panda(IBackend):
    ...
    def master_submit(self, rjobs, subjobspecs, buildjobspec):
        ...
        if job.backend.requirements.express:
            ...
            if isinstance(buildjobspec, type([])):
        ...
        if job.backend.requirements.enableMerge:
            ...
            for js in subjobspecs:
                ...
                # set merging type and user executable if the configurations
                # are given
                merge_type = ''
                merge_exec = ''
                try:
                ...
                if not merge_type:
                    ...
                    if not merge_exec:
                        logger.error(
                            "user merging executable not set for merging type 'user': set PandaRequirements.configMerge['user_exec']")
                        return False
        ...
        if multiSiteJob:
            ...
            for bjspec in buildjobspec:
                ...
                if len(jobspecs) > config['serverMaxJobs']:
                    raise BackendError(
                        'Panda', 'Cannot submit %d subjobs. Server limits to %d.' %
                        (len(jobspecs), config['serverMaxJobs']))
                ...
                if "NULL" in [jobid[0] for jobid in jobids]:
                    logger.error(
                        'Panda could not assign job id to some jobs. Dataset name too long?')
                    return False
                ...
                logger.info(
                    "Added jobdefinitionID %d (%d subjobs at %s) to jobsetID %d" %
                    (jobdefID, njobs, bjspec.computingSite, jobsetID))

                if njobs < len(jobspecs):
                    logger.error(
                        'Panda server accepted only %d of your %d jobs. Confirm serverMaxJobs=%d is correct.' %
                        (njobs, len(jobspecs), config['serverMaxJobs']))
...
#       subjobs cannot have buildjobs
    ...
    def resplit(
        ...
        if self._getParent()._getParent():  # if has a parent then this is a subjob
            raise BackendError(
                'Panda',
                'Resplit on subjobs is not supported for Panda backend. \nUse j.backend.resplit() (i.e. rebroker the master job) and your failed (by default) subjobs \nwill be automatically selected and split again.')

        if self._getParent().splitter and self._getParent().splitter.numevtsperjob > 0:
            raise BackendError(
                'Panda',
                'Resplit while using numevtsperjob currently not supported. Please supply a new splitter.')

        if not newDS and (
            ('running' in sj_status) or (
                'submitted' in sj_status)):
            raise BackendError(
                'Panda',
                'Cannot resplit running jobs without specifying a new DS. Either specify a new DS or kill the active subjobs (j.kill())')
        ...
        if not os.path.exists(mj.application.user_area.name):
            logger.warning(
                "Previous user area not available. Re-running prepare()...(note this will pick up the current install area, etc.)")
        ...
        if self._getParent().subjobs:
            ...
            for sj in self._getParent().subjobs:
                ...
                # sites
                if sj.backend.site not in exc_sites:
                    exc_sites.append(sj.backend.site)

            if len(inDS) == 0:
                raise BackendError(
                    'Panda',
                    'No subjobs in state %s to resplit!' %
                    sj_status)
    ...
    def rebroker(self, cloud=''):
        ...
        if rc:
            logger.error(
                'Return code %d retrieving job status information.', rc)
            raise BackendError(
                'Panda',
        ...
        brokeredjobs = []
        for job in jspecs:
            if job.jobDefinitionID not in brokeredjobs:
                ...
                with inject_proxy(self.credential_requirements):
                    status, output = Client.runReBrokerage(
                        job.jobDefinitionID, libds, cloud, False)
                brokeredjobs.append(job.jobDefinitionID)
                if status != 0:
                    logger.error(output)
                    logger.error(
                        "Failed to rebroker job with Panda jobDefinitionID=%s" %
                        job.jobDefinitionID)
                    continue
                # done

                logger.info(
                    'Rebrokerage done for job with Panda jobDefinitionID=%s' %
                    job.jobDefinitionID)
        return
        ...
        for job in jspecs:
            if job.jobStatus in ['failed', 'killed', 'cancelled']:
                ...
                for attr in job._attributes:
                    if attr.endswith(
                            'ErrorCode') or attr.endswith('ErrorDiag'):
                ...
                for file in job.Files:
                    ...
                    if file.type == 'input':
                        if file.lfn.endswith(
                            ...
                            raise BackendError(
                                'Panda',
                                'GUID for %s is unknown. Cannot retry when corresponding buildJob failed' %
                                file.lfn)
                        file.status = 'ready'
                    elif file.type in ('output', 'log'):
                        file.destinationSE = retryDestSE
                        file.destinationDBlock = re.sub(
                            r'_sub\d+$', '', file.destinationDBlock)
                        # add retry num
                        if file.dataset.endswith('/'):
                            retryMatch = re.search(
                                r'_r(\d+)$', file.destinationDBlock)
                            if retryMatch is None:
                                file.destinationDBlock += '_r1'
                            else:
                                tmpDestinationDBlock = re.sub(
                                    r'_r(\d+)$', '', file.destinationDBlock)
                                file.destinationDBlock = tmpDestinationDBlock + \
                                    '_r%d' % (1 + int(retryMatch.group(1)))
                            jobIDs[oldID].outputdata.datasetname = file.destinationDBlock

                        # add attempt nr
                        oldName = file.lfn
                        file.lfn = re.sub(r"\.\d+$", "", file.lfn)
                        ...
                        job.jobParameters = re.sub(
                            ...
                            newName,
                            job.jobParameters)
                        # The previous line does not work on AthenaMC jobs, so
                        # the next lines are for them
        ...
        for rj in retryJobs:
            for tmpFile in rj.Files:
                if tmpFile.type in ['output',
                                    'log'] and tmpFile.dataset.endswith('/'):
                    # add datasets
                    if tmpFile.destinationDBlock not in addedDataset:
                        with inject_proxy(self.credential_requirements):
                        ...
                        if tmpFile.destinationDBlock not in res.keys():
                            # DS doesn't exist - create it
                            try:
                                with inject_proxy(self.credential_requirements):
                                    Client.addDataset(
                                        tmpFile.destinationDBlock, False, location=tmpOutDsLocation)
                            except exceptions.SystemExit:
                                raise BackendError(
                                    'Panda', 'Exception in Client.addDataset %s: %s %s' %
                                    (tmpFile.destinationDBlock, sys.exc_info()[0], sys.exc_info()[1]))
                        ...
                        if tmpFile.destinationDBlock not in res:
                            try:
                                # add to container
                                with inject_proxy(self.credential_requirements):
                                    Client.addDatasetsToContainer(
                                        tmpFile.dataset, [tmpFile.destinationDBlock], False)
                                logger.warning(
                                    'Created dataset %s and added to container %s.' %
                                    (tmpFile.destinationDBlock, tmpFile.dataset))
                            except exceptions.SystemExit:
                                raise BackendError(
                                    'Panda', 'Exception in Client.addDatasetsToContainer %s, %s: %s %s' %
                                    (tmpFile.destinationDBlock, tmpFile.dataset, sys.exc_info()[0], sys.exc_info()[1]))
                        # append
    ...
    def master_updateMonitoringInformation(jobs):
        ...
        for job in jobs:
            ...
            for mj in job.backend.mergejobs:
                logger.debug(
                    'adding merging job %s for status checking' %
                    mj.id)
                jobdict[mj.id] = job
            ...
            for subjob in job.subjobs:
                if subjob.backend.status in active_status or subjob.status in [
                        'running', 'submitted', 'submitting']:
                    jobdict[subjob.backend.id] = subjob
                    if not subjob.backend.id:
                        logger.debug(
                            "WARNING!! Job: %d - Subjob: %d - Status: %s - Backend ID: %d " %
                            (job.id, subjob.id, subjob.status, subjob.backend.id))
                else:
                    logger.debug(
                        'Skipping monitoring of Jobid: %d - Subjob id: %d - Status: %s - Backend ID: %d' %
                        (job.id, subjob.id, subjob.status, subjob.backend.id))

            if job.status in ['running',
                              'submitted'] and job.backend.domergeretrieve:
        ...
        jIDPieces = [allJobIDs[i:i + 2000]
                     for i in range(0, len(allJobIDs), 2000)]
            ...
            for status in jobsStatus:
                ...
                if job.backend.id == status.PandaID:
                    ...
                    if job.backend.status != status.jobStatus:
                        job.backend.jobSpec = dict(
                            zip(status._attributes, status.values()))

                        for k in job.backend.jobSpec.keys():
                            if type(
                                    job.backend.jobSpec[k]) not in [
                                    type(''), type(1)]:
                                job.backend.jobSpec[k] = str(
                                    job.backend.jobSpec[k])

                        logger.debug(
                            'Job %s has changed status from %s to %s',
                            job.getFQID('.'),
                            job.backend.status,
                            status.jobStatus)
                        ...
                        for k in job.backend.jobSpec.keys():
                            if k.endswith(
                                    'ErrorDiag') and job.backend.jobSpec[k] != 'NULL':
                                job.backend.reason += '%s: %s, ' % (
                                    k, str(job.backend.jobSpec[k]))
                        # if job.backend.jobSpec['transExitCode'] != 'NULL':
                        job.backend.reason += 'transExitCode: %s' % job.backend.jobSpec['transExitCode']

                    # can have odd race condition that updates panda info but
                    # NOT job status - force check every time
                    if status.jobStatus in [
                        ...
                        if job.status == 'submitting':  # Fix for HammerCloud in case jobs are left in submitting state
                            ...
                            if not job.backend._name == 'PandaBuildJob' and job.status != "completed":
                                ...
                                if config['enableDownloadLogs']:
                                    job.backend.getLogFiles(
                                        job.getOutputWorkspace().getPath(), status)

                            if job.master and job.master.backend.requirements.enableMerge:
                                job.updateStatus(
                                    'completed', update_master=False)
                            else:
                        ...
                        if job.status == 'submitting':  # Fix for HammerCloud in case jobs are left in submitting state
                            ...
                            if 'taskBufferErrorDiag' in job.backend.jobSpec and job.backend.jobSpec['taskBufferErrorDiag'].find(
                                    "PandaID=") != -1:
                                # grab the new panda ID
                                newPandaID = long(
                                    job.backend.jobSpec['taskBufferErrorDiag'].split("=")[1])
                    ...
                    # bug 67716
                        ...
                        if job.status == 'submitting':  # Fix for HammerCloud in case jobs are left in submitting state
                            ...
                            if 'taskBufferErrorDiag' in job.backend.jobSpec and "rebrokerage" in job.backend.jobSpec[
                                    'taskBufferErrorDiag']:
                                newPandaID = checkForRebrokerage(
                                    job.backend.jobSpec['taskBufferErrorDiag'])
                                logger.warning(
                        ...
                        logger.warning(
                            'Unexpected job status %s', status.jobStatus)

                    # Fix for HammerCloud in case jobs are left in submitting
                    # state
                    if job.status == 'submitting' and status.jobStatus in [
                        'registered',
                        'defined',
                        'assigning',
                        'ready',
                    ...
                    if job.backend.buildjob.status != status.jobStatus:
                        job.backend.buildjob.jobSpec = dict(
                            zip(status._attributes, status.values()))
                        for k in job.backend.buildjob.jobSpec.keys():
                            if type(
                                    job.backend.buildjob.jobSpec[k]) not in [
                                    type(''), type(1)]:
                                job.backend.buildjob.jobSpec[k] = str(
                                    job.backend.buildjob.jobSpec[k])

                        logger.debug(
                            'Buildjob %s has changed status from %s to %s',
                            job.getFQID('.'),
                            job.backend.buildjob.status,
                            status.jobStatus)
                        if config['enableDownloadLogs'] and not job.backend._name == 'PandaBuildJob' and status.jobStatus == "finished" and job.backend.buildjob.status != "finished":
                            job.backend.getLogFiles(
                                job.getOutputWorkspace().getPath("buildJob"), status)
                        ...
                        if status.jobStatus in [
                            ...
                            if 'taskBufferErrorDiag' in job.backend.buildjob.jobSpec and job.backend.buildjob.jobSpec['taskBufferErrorDiag'].find(
                                    "PandaID=") != -1:
                                # grab the new panda ID
                                newPandaID = long(
                                    job.backend.buildjob.jobSpec['taskBufferErrorDiag'].split("=")[1])
...
#                                job.backend.getLogFiles(job.getOutputWorkspace().getPath("buildJob"), status)
                            ...
                            try:
                                if len(
                                        job.backend.buildjobs) == 1 and status.jobStatus == 'finished':
                                    job.backend.libds = job.backend.buildjobs[0].jobSpec['destinationDBlock']
                            except KeyError:
                                pass

                            # update the status of the master job based on what
                            # all build jobs are doing
                            bjstats = [
                                bj2.status for bj2 in job.backend.buildjobs]
                            new_stat = None
                            ...
                            if new_stat in [
                                ...
                                if 'taskBufferErrorDiag' in bj.jobSpec and bj.jobSpec['taskBufferErrorDiag'].find(
                                        "PandaID=") != -1:
                                    # grab the new panda ID
                                    newPandaID = long(
                                        bj.jobSpec['taskBufferErrorDiag'].split("=")[1])
                                    bj.id = newPandaID
                                    bj.status = None
                    ...
                    mj = job.backend.mergejobs[job.backend.mergejobs.index(
                        tmp_mj)]
                    ...
                    if mj.status != status.jobStatus:
                        logger.debug(
                            'Mergejob %s has changed status from %s to %s',
                            job.getFQID('.'),
                            mj.status,
                            status.jobStatus)
                    ...
                    if mj.status == 'failed':
                        if 'taskBufferErrorDiag' in mj.jobSpec and mj.jobSpec['taskBufferErrorDiag'].find(
                                "PandaID=") != -1:
                            logger.info(
                                "Merge job failed for job %s. Panda has resubmitted" %
                                job.fqid)

                            # clear the merge jobs and force it to grab them
                            # again
        ...
        for job in update_merge_master_status:
            ...
            if merge_finished:
                # merge jobs finished, update the master job status respecting
                # the status of subjobs
        ...
        # going through all jobs to find those with merging jobs to be
        # retrieved
            ...
            if job.backend.requirements.enableMerge:
                ...
                if job.backend.domergeretrieve:
                    ...
                    for sj in job.subjobs:
                        # skip merging job retrieval if there are still subjobs
                        # not in the final state
                        if sj.backend.status not in [
                    ...
                    if do_merge_retrieve:
                        ...
                        jdefids = list(
                            set([bj.backend.jobSpec['jobDefinitionID'] for bj in job.subjobs]))
                        ...
                        for jdefid in jdefids:
                            ...
                            if status not in [
                                    'standby', 'generating', 'generated']:
                                # no merging jobs are expected in this case
                                # skip merging jobs checking by updating the
                                # master job status
                        ...
                        if retrieve_merge_jobs:
                            ...
                            for jdefid in jdefids:
                                ...
                                ick, status, num_mjobs = retrieveMergeJobs(
                                    job, jdefid)

                                logger.debug(
                                    'retrieveMergeJobs returns: %s %s' %
                                    (repr(ick), status))

                                if not ick:
                                    logger.warning(
                                        'merging job retrival failure')

                                tot_num_mjobs += num_mjobs

                        # set the flag to prevent further retrievals unless a
                        # resub is called
                        if tot_num_mjobs > 0:
                            job.backend.domergeretrieve = False
                            logger.info(
                                'job %s retrieved %d merging jobs' %
                        ...
                        # need to update the master job status when there are
                        # still subjobs not in final state
                        jlist_for_masterjob_update.append(job)
        ...
        for job in list(
                set(jobs) -
                set(jlist_for_masterjob_update) -
                set(jlist_merge_finished)):
...
#                    pass
    ...
    def list_ddm_sites(self, allowTape=False):
        ...
        if self.site == 'AUTO':
            if self.libds:
                ...
                if not libdslocation:
                    raise ApplicationConfigurationError(
                        'Could not locate libDS %s' % self.libds)
                else:
                    libdslocation = libdslocation.values()[0]
                    try:
                        ...
                        raise BackendError(
                            'Panda', 'Could not map libds site %s to a cloud' %
                            libdslocation)
                sites = libdslocation
                logger.info(
                    'LibDS is at %s. Run jobs will execute there.' %
                    sites)
            else:
                sites = Client.PandaSites.keys()

            for s in sites:
                if Client.PandaSites[s]['status'] not in ['online'] or s in self.requirements.excluded_sites or Client.PandaSites[s]['cloud'] in self.requirements.excluded_clouds or (
                        not self.requirements.anyCloud and Client.PandaSites[s]['cloud'] != self.requirements.cloud):
                    continue
                tokens = convertQueueNameToDQ2Names(s)
                for t in tokens:
                    if (allowTape or not Client.isTapeSite(
                            t)) and t.find("TZERO") == -1:
            ...
            try:
                ...
                raise BackendError(
                    'Panda',
                    'Site %s not in a known Panda Sites' %
                    self.site)
            if s['status'] not in ['online', 'brokeroff'] and not (
                    s['status'] == 'test' and config['processingType'].startswith('gangarobot')):
                raise BackendError(
                    'Panda', 'Cannot submit to %s in status %s' %
                    (self.site, s['status']))
            if self.site in self.requirements.excluded_sites:
                raise BackendError(
                    'Panda',
                    'Cannot submit to %s because it is in your requirements.excluded_sites list' %
                    self.site)
    ...
    def getLogFiles(self, workspace, status):
        for lf in [f for f in status.Files if f.type == "log"]:
            untar = ""
            if "tgz" in lf.lfn:
                untar = "tar xzf %s; mv tarball_PandaJob*/* .; rm tarball_PandaJob* -rf; rm %s;" % (
                    lf.lfn, lf.lfn)
            cmd = "pushd .; mkdir -p %s; cd %s; dq2-get -D -f %s %s; %s popd;" % (
                workspace, workspace, lf.lfn, lf.dataset, untar)
            Download.download_dq2(cmd).run()

    def fillOutputData(self, job, status):
        # format for outputdata is:
        # dataset,lfn,guid,size,md5sum,siteID\ndataset...
        outputdata = []
        locations = {}
        ...
        if len(locations.keys()) > 1:
            logger.warning(
                "Outputfiles of job %s saved at different locations! (%s)" %
                (job.fqid, locations.keys()))
...
# jobSpec = {'startTime': '2011-11-01 09:37:51', 'modificationTime': '2011-11-01 10:03:22', 'creationTime': '2011-11-01 09:37:40', 'endTime': '2011-11-01 09:47:16', } ,
    ...
    def getStateTime(self, status):
        ...
        try:
            t = datetime.datetime(
                *(time.strptime(self.jobSpec[checkstr], "%Y-%m-%d %H:%M:%S")[0:6]))
        except BaseException:
            return None
        return t
