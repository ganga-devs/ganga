...
class Jedi(IBackend):
    ...
    def master_submit(self, rjobs, subjobspecs, buildjobspec):
        ...
        if not tmpOut[0]:
            ...
            return False
        logger.info(
            "Task submission to Jedi suceeded with new jediTaskID=%s" %
            tmpOut[1])
    ...
    def master_kill(self):
        ...
        if self.id and self.status in active_status:
            ...
            if status:
                ...
                logger.info(
                    'Killing Jedi task %s, Server returned: %s' %
                    (self.id, output))
        else:
            logger.error(
    ...
    def master_updateMonitoringInformation(jobs):
        ...
        jobdict = {}
        for job in jobs:
            # add a delay as Panda can be a little slow in sorting out a new
            # Task
            if job.backend.id and job.backend.status in active_status and (
                    (datetime.datetime.utcnow() - job.time.timestamps["submitted"]).seconds > 120):
        ...
        for jID in allJobIDs:
            ...
            if job.backend.pandajobs:
                ...
                pandaJobIDs[jediTaskDict['jediTaskID']
                            ] = jediTaskDict['PandaID']
            ...
            if 'outDS' in jediTaskDict and jediTaskDict['outDS'] != '':
                for ds in jediTaskDict['outDS'].split(','):
                    if ds not in job.outputdata.datasetList:
                ...
                for status in jobsStatus:
                    ...
                    for pjob in job.backend.pandajobs:
                        if pjob.id == status.PandaID:
                            ...
                            for k in pjob.jobSpec.keys():
                                if k.endswith(
                                        'ErrorDiag') and pjob.jobSpec[k] != 'NULL':
                                    pjob.reason += '%s: %s, ' % (
                                        k, str(pjob.jobSpec[k]))
                            # if job.backend.jobSpec['transExitCode'] !=
                            # 'NULL':
                            pjob.reason += 'transExitCode: %s' % pjob.jobSpec['transExitCode']

                            if status.jobStatus in [
                                    'defined', 'unknown', 'assigned', 'waiting', 'activated', 'sent']:
                                logger.debug(
                                    'Panda job %s %s' %
                                ...
                                if 'taskBufferErrorDiag' in pjob.jobSpec and pjob.jobSpec['taskBufferErrorDiag'].find(
                                        "PandaID=") != -1:
                                    # grab the new panda ID
                                    newPandaID = long(
                                        pjob.jobSpec['taskBufferErrorDiag'].split("=")[1])
                                    pjob.id = newPandaID
                                    pjob.status = None
                                    pjob.url = 'http://panda.cern.ch/?job=%d' % newPandaID
                            # bug 67716
                            elif status.jobStatus == 'cancelled' and pjob.status not in ['completed', 'failed']:
                                logger.debug(
                                    'Panda job %s cancelled' % pjob.id)
    ...
    def master_resubmit(self, jobs):
        ...
        for jID in allJobIDs:
            ...
            #    pass
                ...
                logger.warning(
                    "Cannot resubmit. Jedi task %s is status %s." %
