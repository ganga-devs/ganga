...
dataTable = JobInfoParser.getJobInfoTable(
    jlist, ['backend', 'backend.actualCE', 'application', 'status', 'backend.status'])
...
plotter.piechart(
    jobs['DIANE_4'],
    attr='backend.CE',
    title='Job distribution by queue',
    output='test.png')
