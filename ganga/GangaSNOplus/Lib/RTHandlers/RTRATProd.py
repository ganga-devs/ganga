...
# Assume that the applications should come from the
...
class RTHandler(IRuntimeHandler):
    ...
    def prepare(self, app, appconfig, appmasterconfig, jobmasterconfig):
        ...
        for i, var in enumerate(app.inputFiles):
            finList += '%s,' % var
        if len(foutList) != 1:
            # remove final comma, add close bracket
            foutList = '%s]' % foutList[:-1]
        if len(finList) != 1:
            # remove final comma, add close bracket
            finList = '%s]' % finList[:-1]

        if app.environment is None:
            args = []
...
class LCGRTHandler(IRuntimeHandler):
    ...
    def prepare(self, app, appconfig, appmasterconfig, jobmasterconfig):
        ...
        if app.softwareEnvironment is None:
            ...
            # wrapper
            app.softwareEnvironment = r'\$SNOPLUS_CVMFS_DIR/sw/%s/env_rat-%s.sh' % (
