...
# Assume that the applications should come from the same
# GangaSNOplus/Lib/Applications directory
_app_directory = os.path.abspath(
    os.path.join(
...
class UserLCGRTHandler(IRuntimeHandler):
    ...
    def prepare(self, app, appconfig, appmasterconfig, jobmasterconfig):
        ...
        if app.outputDir is None:
            if app.config['grid_outputDir'] is None:
        ...
        if app.softwareEnvironment is None:
            ...
            # Note the extra \ to escape the dollar in the initial python
            # wrapper
            rrArgs += r'-e \$SNOPLUS_CVMFS_DIR/sw/%s/env_rat-%s.sh ' % (
                app.ratBaseVersion, app.ratBaseVersion)
        else:
            rrArgs += '-e %s ' % app.softwareEnvironment

        if app.ratVersion is not None:
        ...
        app._getParent().inputsandbox.append(
            '%s/check_root_output.py' %
            _app_directory)
...
''')
        ...
        for i in app._getParent().inputsandbox:
            # not sure why these are File objects...
            inputsandbox.append(i.name)
...
class UserRTHandler(IRuntimeHandler):
    ...
    def prepare(self, app, appconfig, appmasterconfig, jobmasterconfig):
        ...
        if app.outputDir is None:
            if app.config['local_outputDir'] is None:
                logger.error('Output directory not defined')
                raise Exception
            else:
        ...
        if app.environment == []:
            ...
            app._getParent().inputsandbox.append('%s/job_tools.py' % _app_directory)
            app._getParent().inputsandbox.append(
                '%s/check_root_output.py' % _app_directory)
            ...
            # can either use a specific file or a list of strings.  the latter
            # needs to be converted to a temp file and shipped.
            envFile = None
            rrArgs = ''
            spArgs = []
            ...
            if isinstance(app.environment, list):
