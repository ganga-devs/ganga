...
config = GangaCore.Utility.Config.makeConfig(
    'WestGrid', 'internal WestGrid command line interface')
...
class WestGrid(Batch):
    ...
    '''
    # same schema as a Batch, but with the added option of where the user
    # proxy is stored
    _schema = Schema(
        Version(
            1,
            0),
        {
            'queue': SimpleItem(
                defvalue='',
                doc='queue name as defomed in your local Batch installation'),
            'extraopts': SimpleItem(
                defvalue='',
                doc='extra options for Batch. See help(Batch) for more details'),
            'id': SimpleItem(
                defvalue='',
                protected=1,
                copyable=0,
                doc='Batch id of the job'),
            'exitcode': SimpleItem(
                defvalue=None,
                typelist=[
                    'int',
            ...
            'status': SimpleItem(
                ...
                hidden=1,
                copyable=0,
                doc='Batch status of the job'),
            'actualqueue': SimpleItem(
                defvalue='',
                protected=1,
                copyable=0,
                doc='queue name where the job was submitted.'),
            'actualCE': SimpleItem(
                defvalue='',
                protected=1,
                copyable=0,
                doc='hostname where the job is/was running.'),
            'voproxy': SimpleItem(
                defvalue=None,
                typelist=[
                    'str',
                    'type(None)'],
                doc='select specific proxy location')})
    _category = 'backends'
    _name = 'WestGrid'
