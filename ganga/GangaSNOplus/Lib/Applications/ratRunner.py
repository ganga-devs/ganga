...
def download_rat(token, run_version, install_path):
    ...
    tar_name = 'rat.tar.gz'
    job_tools.download_file(
        url,
        token,
        filename=tar_name,
        cache_path=cache_path)
    job_tools.untar_file(
        tarName,
        installPath,
        'rat',
        cache_path,
...
def untar_local(tarname):
    ...
    # nothing to strip, but do need to move
    job_tools.untar_file(tarname, installPath, 'rat', os.getcwd(), 0)
...
def check_outputs(options):
    ...
    '''
    # Can only check the outputs for files with output file specified in the
    # command options
    ...
    if options.grid_mode is None:
        ...
        return None
    # For output files, check if there is an output root and output ntuple
    # processor in the macro
    ...
    # If only some (i.e. any) exist, but not all, then we need to raise this
    # problem by failing the job!
    dump_out = {}
    if all_exist:
        ...
        for s in suffixes:
            try:
                ...
                # Note that the actual se and replica information may be
                # different to the local information
                se_name = replica
...
def check_output_file(options):
    ...
    # Can only check the outputs for files with number of events specified in
    # the command options
    ...
    if root or soc:
        rtc, out, err = job_tools.execute_ratenv(
            'python', [
                'check_root_output.py', '-f', '%s.root' %
                options.output_file, '-n', options.n_events, '-v', options.base_version])
        if rtc == 0:
    ...
    if ntuple:
        rtc, out, err = job_tools.execute_ratenv(
            'python', [
                'check_root_output.py', '-f', '%s.ntuple.root' %
                options.output_file, '-n', options.n_events, '-v', options.base_version])
        if rtc == 0:
            # this must be true (exception raised otherwise)
            print "Outputs file checks successful"
...
if __name__ == '__main__':
    ...
    parser.add_option(
        "-t",
        dest="token",
        help="Token to use, if in token mode")
    parser.add_option("-b", dest="base_version", help="RAT base version")
    parser.add_option("-v", dest="run_version", help="RAT version to run")
    parser.add_option(
        "-e",
        dest="env_file",
        help="Software environment file to source")
    parser.add_option("-m", dest="rat_macro", help="RAT macro to use")
    parser.add_option(
        "-d",
        dest="output_dir",
        help="LFC/SURL relative directory if in grid mode, full dir path if not in grid mode")
    parser.add_option("-g", dest="grid_mode", default=None,
                      help="Grid mode (allowed modes are srm and lcg)")
    parser.add_option(
        "-f",
        dest="zip_filename",
        help="Zip filename for output file")
    parser.add_option(
        "-i",
        dest="input_file",
        default=None,
    ...
    parser.add_option("-N", dest="n_events", default=None,
                      help="Number of events (must not be in macro)")
    parser.add_option("-T", dest="run_time", default=None,
                      help="Duration of run (cannot use with n_events)")
    parser.add_option(
        "--dbuser",
        dest="dbuser",
        default=None,
    ...
    parser.add_option(
        ...
        dest="dbprotocol",
        default=None,
        help="Database protocol (http or https)")
    parser.add_option("--dburl", dest="dburl", default=None,
                      help="Database URL (sans protocol)")
    parser.add_option(
        "--nostore",
        dest="nostore",
        action="store_true",
        help="Don't copy the outputs at the end")
    parser.add_option(
        "--voproxy",
        dest="voproxy",
        default=None,
        help="VO proxy location, MUST be used with grid srm mode")
    ...
    if options.run_version:
        ...
        # all args have to be strings anyway (from Ganga) - force base version
        # into a string
        install_local(options.run_version, options.env_file)
        job_helper.add_environment(
            os.path.join(
                os.getcwd(),
                "installerEnv.sh"))
    else:
        # Still need to ensure that the JobHelper singleton knows about the
        # environment
    ...
    if dump_out is None:
        ...
        if not options.nostore:
             # the nostore option is only applied when running in testing mode
             # for production
            dump_out = copy_data(options.output_dir, options.grid_mode)
        else:
            # return a dummy output dump - no storage information but the
            # production scripts expect it
        ...
        return_card.close()
        raise Exception(
            "RATRUNNER: requested outputs already exist, see return_card.js for details")
