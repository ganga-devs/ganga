...
config = GangaCore.Utility.Config.makeConfig(
    'defaults_RATUser', 'Defaults for the RATUser application')

config.addOption(
    'local_softwareEnvironment',
    None,
    'Local snoing-install directory (or directory with env_rat-x.y.sh files)')
config.addOption(
    'local_environment',
    [],
    'Environment options required to run on local or batch system')
config.addOption(
    'local_outputDir',
    None,
    '*Default* output directory if running on a batch or local system (can override)')
config.addOption(
    'grid_outputDir',
    None,
    '*Defult* output directory if running on system with grid storage (can override)')
config.addOption(
    'cacheDir',
    '~/gaspCache',
    'Directory to store RAT snaphots (if required)')
...
class RATUser(IApplication):
    ...
    # Add any options that are required, but try to set sensible default
    # values to minimise effort required of user
    ...
    def configure(self, masterappconfig):
        ...
        if self.ratBaseVersion == '':
            logger.error(
                'Error: must give a rat base (fixed release) version number')
            raise Exception
        if not self.outputFile:
            # checks if output file set in command line
            logger.error('No output file defined!')
            raise Exception
        if RATUtil.check_command(self.ratMacro, ['/rat/procset', 'file']):
            # checks if output file si set in the macro
            logger.error(
                'Output file should only be defined via the command line and not in the macro!')
            raise Exception
        if self.outputFile:
            if not RATUtil.check_command(self.ratMacro,
                                         ['/rat/proclast',
                                          'outroot']) and not RATUtil.check_command(self.ratMacro,
                                                                                    ['/rat/proclast',
                                                                                     'outntuple']) and not RATUtil.check_command(self.ratMacro,
                                                                                                                                 ['/rat/proclast',
                                                                                                                                  'outsoc']):
                # checks for putroot processor (needs to be there regardless
                # where the output file is defined)
                logger.error(
                    'Have specified an output file, but no root, ntuple or soc processor present in macro')
                raise Exception
        if not self.nEvents and not self.tRun:
            # checks if number of events or run duration is set in command line
            logger.error(
                'Need to specifiy either the number of events or the duration of run! No number of events or run duration defined!')
            raise Exception
        if self.nEvents and self.tRun:
            logger.error(
                'Cannot specify number of events and the duration of run!')
            raise Exception
        if not RATUtil.check_command(self.ratMacro, ['/rat/run/start', '']):
            # check if the /rat/run/start command is set
            logger.error(
                '/rat/run/start must be set in the macro but no number of events should be specified! Number of events should only be defined via the command line!')
            raise Exception
        if self.inputFile:
            if RATUtil.checkText(self.ratMacro, ['inroot/read']):
                logger.error(
                    'Cannot specify inputFile in Ganga job if "/rat/inroot/read" line is present in macro')
                raise Exception
        if self.useDB:
        ...
        if self.ratVersion is not None:
            # download the code locally
            # only uses the main SNO+ rat branch for now
            # need to add pkl object to inform which branch we have and add
            # others when required
            self.zipFileName = RATUtil.make_rat_snapshot(
                self.ratFork,
                self.ratVersion,
                self.versionUpdate,
                os.path.expanduser(
    ...
    _schema = Schema(
            ...
            0),
        {
            'ratMacro': SimpleItem(
                ...
                typelist=['str'],
                sequence=1,
                doc='A list of lists for specifying rat macros files'),
            'outputFile': SimpleItem(
                defvalue=[],
                typelist=[
                    'str',
                    'type(None)'],
                sequence=1,
                doc='A list of lists for specifying rat output files'),
            'inputFile': SimpleItem(
                defvalue=[],
                typelist=[
                    'str',
                    'type(None)'],
                sequence=1,
                doc='A list of lists for specifying rat input files'),
            'nEvents': SimpleItem(
                defvalue=[],
                typelist=[
                    'int',
                    'type(None)'],
                sequence=1,
                doc='A list of the number of events for each sub job')})
    ...
    def split(self, job):
        ...
        if self.outputFile != []:
            if len(self.outputFile) != len(self.ratMacro):
                logger.error(
                    'Must have same number of macros, outputs and inputs for the splitter')
                raise Exception
        if self.inputFile != []:
        ...
        if self.nEvents != []:
            if len(self.nEvents) != len(self.ratMacro):
                logger.error(
                    'Must have same number of nEvents as macros for the splitter')
                raise Exception
