...
if __name__ == '__main__':
    ...
    parser.add_option(
        "-m",
        dest="rat_macro",
        help="RAT macro to use (or script if in shell mode)")
    parser.add_option(
        "-d",
        dest="output_dir",
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="shell_mode",
        help="Shell mode: use if passing a shell script to run")
    parser.add_option(
        "-i",
        dest="input_files",
        help="list of input files, must be in [braces] and comma delimited")
    parser.add_option(
        "-x",
        dest="input_dir",
        help="LFC/SURL relative directory if in grid mode, full dir path if not in grid mode")
    parser.add_option(
        "-o",
        dest="output_files",
        help="list of output files, must be in [braces] and comma delimited")
    parser.add_option(
        "--dbuser",
        dest="dbuser",
        default=None,
    ...
    if not options.rat_macro or not options.output_dir or not options.env_file and not options.output_files:
        ...
        if options.grid_mode == 'srm':
            if not options.voproxy:
                parser.print_help()
                raise Exception(
                    'Grid %s mode, must define voproxy' %
                    options.grid_mode)
            elif not os.path.exists(options.voproxy):
                parser.print_help()
                raise Exception(
                    'Grid %s mode, must define valid voproxy' %
                    options.grid_mode)
