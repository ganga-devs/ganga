...
config = GangaCore.Utility.Config.makeConfig(
    'defaults_RATProd', 'Defaults for the RATProd application')
...
class RATProd(IApplication):
    ...
    def configure(self, masterappconfig):
        ...
        if self.ratMacro != '':
            ...
            # macFile=decimated[len(decimated)-1]#for when I thought we needed
            # args to go with the script
        ...
        # we want a list of files - if we only have one (i.e. a string) then
        # just force into a list
        if isinstance(self.inputFiles, str):
            self.inputFiles = [self.inputFiles]
        if isinstance(self.outputFiles, str):
...
class RATSplitter(ISplitter):
    ...
    _schema = Schema(
        Version(
            ...
            0),
        {
            'outputFiles': SimpleItem(
                defvalue=[],
                typelist=[
                    'list',
                    'str'],
                sequence=1,
                doc='A list of lists for specifying output files.  Only works with the RATProd application.'),
            'inputFiles': SimpleItem(
                defvalue=[],
                typelist=[
                    'list',
                    'str'],
                sequence=1,
                doc='A list of lists for specifying intput files.  Only works with the RATProd application.'),
            'prodScript': SimpleItem(
                defvalue=[],
                typelist=['str'],
                sequence=1,
                doc='A list specifying the scripts (if used).'),
            'ratMacro': SimpleItem(
                defvalue=[],
                typelist=['str'],
                sequence=1,
                doc='A list specifying the macros (if used)'),
        })
    ...
    def split(self, job):
        ...
        if nScript != 0 and nMacro != 0:
            logger.error(
                'cannot provide both scripts and macros, must be either or')
            raise Exception
        if nScript == 0 and nMacro == 0:
        ...
        for i in range(nFiles):
            # nFiles should have the same number as either (or both) #input or
            # #output
