...
def postprocess(self, logger):
    ...
    if j.subjobs:
        ...
        if 'XMLSUMMARYBASEROOT' not in env:
            logger.warning('"XMLSUMMARYBASEROOT" env var not defined so summary.xml files not'
                           ' merged for subjobs of job %s' % j.fqid)
            return
        ...
        for sj in j.subjobs:
            ...
            if not os.path.exists(outputxml):
                logger.warning("XMLSummary for job %s subjobs will not be merged as 'summary.xml' "
                               "not present in job %s outputdir" % (j.fqid, sj.fqid))
                return
            elif os.path.getsize(outputxml) == 0 or os.stat(outputxml).st_size == 0:
                logger.warning("XMLSummary for job %s subjobs will not be merged as %s appears to "
                               "be an empty file" % (j.fqid, outputxml))
                logger.warning("Please try to recreate this file by either resubmitting your job "
                               "or re-downloading the data from the backend")
                return
            summaries.append(outputxml)

        # Not needed now that we dont merge if ANY of subjobs have missing summary.xml
        # if not summaries:
        #    logger.debug('None of the subjobs of job %s produced the output XML summary file '
        #                 '"summary.xml". Merging will therefore not happen' % j.fqid)
        #    return
    ...
    if j.subjobs:
        ...
        for sj in j.subjobs:
            ...
            if not os.path.exists(outputxml):
                ...
                logger.warning("XMLSummary fro job %s subjobs will not be merged as %s appears to "
                               "be an empty file" % (j.fqid, outputxml))
                logger.warning("Please try to recreate this file by either resubmitting your job "
                               "or re-downloading the data from the backend")
                return
