...
def addTimestampFile(given_path, fileName='__timestamp__'):
    """
    This creates a file in this directory given called __timestamp__ which contains the time so
    that the final file is unique
...
def getGaudiExecInputData(optsfiles, app):
    ...
    opt = "~/cmtuser/DaVinci_v22r0p2/Tutorial/Analysis/options/Bs2JpsiPhi2008.py"
    j.inputdata = getGaudiExecInputData([opt], j.application)

    This is also called behind the scenes for
    j.readInputData([\"~/cmtuser/DaVinci_v22r0p2/Tutorial/Analysis/options/Bs2JpsiPhi2008.py\"])
...
def prepare_cmake_app(
        ...
        myBranch='master'):
    """
        Short helper function for setting up minimal application environments on disk for job
        submission
        ...
        Args:
            ...
            myBranch (str): This is the branch used for the lb-checkout. Master branch assumed if
                            not specified
    """
    ...
    verStat, verOut, verErr = _exec_cmd('lb-sdb-query listPlatforms %s %s'\
                                        % (myApp, myVer), full_path)
    ...
    if not path.exists(full_path + '/' + myApp + 'Dev_' + myVer):
        devStat, devOut, devErr = _exec_cmd(
            'export CMTCONFIG=%s && source LbLogin.sh --cmtconfig=%s && lb-dev %s/%s' %
            (platformToUse, platformToUse, myApp, myVer), full_path)
        ...
        raise GangaException(
            "Path %s already exists. Not checking out application. Try a different location." % str(
    ...
    if myUse:
        lbUse, lbUseOut, lbUseErr = _exec_cmd(
            'export CMTCONFIG=%s && source LbLogin.sh --cmtconfig=%s && git lb-use %s' %
            (platformToUse, platformToUse, myUse), dev_dir)
        logger.info("Running git lb-use %s" % myUse)
        if lbUse != 0:
            logger.error("git lb-use %s failed!" % myUse)
            raise ApplicationPrepareError(lbUseErr)
        if myFolder:
            chk, chkOut, chkErr = _exec_cmd('export CMTCONFIG=%s && source LbLogin.sh '
                                            '--cmtconfig=%s && git lb-checkout %s/%s %s' %
                                            (platformToUse, platformToUse, myUse,
                                             myBranch, myFolder), dev_dir)
...
def prepareGaudiExec(
    ...
    """
        ...
        Args:
            ...
            myBranch (str) : This is what is appended to the lb-checkout myFolder/myBranch , master
