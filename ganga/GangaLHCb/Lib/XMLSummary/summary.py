...
try:
    ...
    __schema__ = __imp__.machinery.SourceFileLoader('schema', __os__.sep.join(
        __file__.split(__os__.sep)[:-1] + ['schema.py'])).load_module()
...
class Summary(__schema__.VTree):
    ...
    def __init__(self, schemafile=__default_schema__, construct_default=True):
        ...
        module path optionally imports the schema module from a different path if it isn't already
        here
        """
        # print 'crap'
    ...
    def __file_merger__(
        ...
        destination.__element__.text = str(
            int(float(destination.__element__.text)) + int(addevents))
        return True
    ...
    def fill_VTree_counter(self, counter, isLumi=False):
        ...
        if name not in self.__count_dict__[mother]:
            counters = __schema__.VTree([bt for bt in self.__element__.getchildren()
                                         if mother in bt.tag][0], self.__schema__, self, False)
        ...
        for i in range(len(cnt.attrib('format'))):
            for j in range(len(counter.attrib('format'))):
                if cnt.attrib('format')[i] == counter.attrib('format')[j]:
                    ...
                    break
        cnt.value(val)
        if cnt.attrib('max') < counter.attrib('max'):
            cnt.attrib('max', counter.attrib('max'))
        if cnt.attrib('min') > counter.attrib('min'):
            cnt.attrib('min', counter.attrib('min'))
        ...
        if not astat or len(astat) == 0:
            ...
            if astat.attrib('unit') != unit:
                raise AttributeError(
                    'I cannot compare two MemoryMaxima when they have different units! ' +
                    astat.attrib('unit') +
                    " " +
                    unit)
...
def Merge(summaries, schemafile=__default_schema__):
    ...
    if isinstance("", type(schemafile)):
        schemafile = __schema__.Schema(schemafile)
    if not isinstance([], type(summaries)):
        raise TypeError('you should send a list into the merger, I got a ' +
                        str(type(summaries)) + ' object instead')
    sum_objects = []
    ...
    if isinstance(
        "", type(
            ...
            type(
                Summary(
                    schemafile=schemafile))) == str(
                        type(
                            summaries[0])):
        raise TypeError('you should send strings or Summaries into the merger, I got a ' +
                        str(type(summaries[0])) + ' object instead')
    ...
    try:
        ...
        if isinstance("", type(summaries[0])):
        ...
        flag = ''
        for asummary in sum_objects:
            if flag == asummary.children('step')[0].__element__.text:
                continue
            for step in steps:
