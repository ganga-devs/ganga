...
class VTree(object):
    ...
    def __insert_element__(self, child, index):
        ...
        if real_index != element_size - 1:
            self.__element__.getchildren()[real_index].tail = self.__element__.getchildren()[
                real_index + 1].tail
        else:
            self.__element__.getchildren()[real_index].tail = self.__element__.getchildren()[
                real_index + 1].tail[:-1]
    ...
    def __remove_element__(self, child):
        ...
        if element_size == 1:
            ...
            raise TypeError('This should never happen since child is supposed to belong to the '
    ...
    def add(self, child, index=None):
        ...
        if 'VTree' in child.__str__():
            ...
            try:
                ...
                raise TypeError(
                    'you can only add VTree objects, or use a string to add the default object')
                return False
    ...
    def value(self, val=None):
        ...
        if val is None:
            ...
            if self.__schema__.Tag_whitespace(self.tag()) == 'collapse':
                return self.__schema__.__cast_from_tag__(
                    self.tag(), self.__element__.text.rstrip().lstrip())
...
class Schema(object):
    ...
    def __check__(self, element):
            ...
            if len(element.text.rstrip().lstrip()):
                if not self.Tag_canHaveValue(element.tag, element.text):
                    raise ValueError(
                        'element ' +
                        element.tag +
                        ' has the wrong entry type for the schema')
                    return False
        # check attribs
        for att, val in element.items():
            ...
            #           print 'the root attribute', att, 'must have the value',\
            #                 self.__rootattribs__[att]
            #            return False
