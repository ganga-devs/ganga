...
RecoToDST-07/90000000/DST" ,
    ...
    docstr = 'Selection criteria: Runs, ProcessedRuns, NotProcessed (only works for ' \
             'type="RunsByDate")'
    schema['selection'] = SimpleItem(defvalue='', doc=docstr)
    schema['credential_requirements'] = ComponentItem(
        'CredentialRequirement', defvalue='DiracProxy')
    ...
    def getDatasetMetadata(self):
        ...
        if self.type is not 'RunsByDate':
            if self.startDate:
