...
def _getPathVersion(base_path, version):
    ...
    if found_path is None:
        logger.error(
            "Cannot find ROOT version: %s from manipulating PATH: %s" %
            (str(version), str(base_path)))
...
def _getPathArch(base_path, arch):
    ...
    if found_path is None:
        logger.error(
            "Cannot find ROOT for arch: %s from manipulating PATH: %s" %
            (str(arch), str(base_path)))
