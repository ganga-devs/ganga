...
class Schema(object):
    ...
    def createDefaultConfig(self):
        ...
        config = GangaCore.Utility.Config.makeConfig(
            defaultConfigSectionName(_self_name),
            "default attribute values for %s objects" %
            _self_name)
    ...
    def _getDefaultValueInternal(self, attr, val=None, check=False):
                ...
                if attr in eff_ops:
                    ...
                    if isProxy(defvalue):
                        raise GangaException(
                            "(1)Proxy found where it shouldn't be in the Config: %s" %
                            stored_attr_key)
        ...
        if useDefVal:
            ...
            if isProxy(defvalue):
                raise GangaException(
                    "(2)Proxy found where is shouldn't be in the Config" %
                    stored_attr_key)
            # Just in case a developer puts the proxied object into the default value!
            _found_attrs[stored_attr_key] = defvalue
                ...
                if defvalue is None:
                    if not item['load_default']:
                        try:
                            ...
                            raise SchemaError(
                                "This item '%s' is not a sequence, doesn't have a load_default and is not optional. This is unsupported!" %
                                type(item))
                        return None
        ...
        if forced:
            ...
            if len(forbidden) > 0:
                raise GangaTypeError(
                    '%s received forbidden (forced) keyword arguments %s' %
    ...
    def __check(isAllowedType, name, validTypes, input_val):
        if not isAllowedType:
            ...
            raise TypeMismatchError(
                'Attribute "%s" expects a value of the following types: %s\nfound: "%s" of type: %s' %
    ...
    def _check_type(self, val, name, enableGangaList=True):
        ...
        if self._meta['sequence']:
            ...
            if not isType(self._meta['defvalue'], (list, tuple, GangaList)):
                raise SchemaError(
                    'Attribute "%s" defined as a sequence but defvalue is not a list.' %
                    name)
            ...
            if self._meta['sequence']:
                ...
                for valItem in val:
                    if not valueTypeAllowed(valItem, validTypes):
                        raise TypeMismatchError(
                            'List entry %s for %s is invalid. Valid types: %s' %
                    ...
                    if isinstance(val, dict):
                        for dKey, dVal in val.items():
                            if not valueTypeAllowed(
                                    dKey, validTypes) or not valueTypeAllowed(
                                    dVal, validTypes):
                                raise TypeMismatchError(
                                    'Dictionary entry %s:%s for attribute %s is invalid. Valid types for key/value pairs: %s' %
                                    (dKey, dVal, name, validTypes))
                    else:  # new value is not a dict
                        if val == []:
                            return
                        raise TypeMismatchError(
                            'Attribute "%s" expects a dictionary. Found: "%s".' %
                            (name, type(val)))
                    return
        ...
        if self._meta['defvalue'] is None:
            logger.warning(
                'type-checking disabled: type information not provided for %s, contact plugin developer',
                name)
        else:
            if self._meta['sequence']:
                logger.warning(
                    'type-checking is incomplete: type information not provided for a sequence %s, contact plugin developer',
                    name)
            else:

                logger.debug("valType: %s defValueType: %s name: %s" %
                             (type(val), type(self._meta['defvalue']), name))
                self.__check(
                    self.__actualCheck(
                        val, self._meta['defvalue']), name, type(
                        self._meta['defvalue']), val)
