...
from GangaCore.GPIDev.Lib.Registry.JobRegistry import RegistryKeyError
from GangaCore.GPIDev.Lib.Registry.JobRegistry import JobRegistrySlice, JobRegistrySliceProxy, _wrap
from GangaCore.GPIDev.Base.Proxy import stripProxy, GPIProxyObject
...
class JobTree(GangaObject):
    ...
    def __folder_cd(self, path):
        ...
        for _dir in path:
            ...
            if _dir not in returnable_folder:
                ...
                raise TreeError(
                    1, "Directory %s does not exist in folder %s, accessing: %s" %
    ...
    def _display(self, interactive=0):
        from GangaCore.Utility.ColourText import ANSIMarkup, NoMarkup
        if interactive:
            markup = ANSIMarkup()
        else:
            markup = NoMarkup()
