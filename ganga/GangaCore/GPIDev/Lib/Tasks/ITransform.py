...
# Public methods
        ...
        for trf in self._getParent().transforms:
            for u2 in trf.units:
                for req in u2.req_units:
                    if req == "%d:%d" % (self.getID(),
                                         u.getID()) or req == "%d:ALL" % (self.getID()):
    ...
    def update(self):
        ...
        info_str = "Unit overview: %i units, %i new, %i hold, %i running, %i completed, %i bad. to_sub %i" % (len(
            self.units),
            unit_status["new"],
            unit_status["hold"],
            unit_status["running"],
...
# Information methods
    ...
    def createUnitCopyOutputDS(self, unit_id):
        ...
        self.units[unit_id].copy_output.local_location = os.path.join(
            ...
            self.units[unit_id].name.replace(
                ":",
                "_").replace(
                " ",
                "").replace(
