...
class ICredentialRequirement(GangaObject):
        ...
        for key, value in kwargs.items():
            if key not in self._schema.allItemNames():
                raise GangaAttributeError(
                    '{class_name} does not have attribute called "{attr}"'.format(
    ...
    def __repr__(self):
        ...
        explicit_items = ((name, value)
                          for name, value in items if value)  # Filter out any default values
        item_strings = ((name, repr(value))
                        for name, value in explicit_items)  # Stringify the values
    ...
    def __eq__(self, other):
        ...
        if other is None:
            return False
        return isinstance(self, type(other)) and self.encoded() == other.encoded()
