...
class IBackend(GangaObject):
    ...
    def master_prepare(self, masterjobconfig):
        ...
        if masterjobconfig:
            if hasattr(
                    job.application,
                    'is_prepared') and isType(
                    job.application.is_prepared,
                    ShareDir):
                ...
                nonsharedir_files = itertools.filterfalse(
        ...
        tmpDir = None
        files = []
        if len(job.inputfiles) > 0 or (len(job.subjobs) == 0 and job.inputdata and isType(
    ...
    def master_updateMonitoringInformation(jobs):
        ...
        try:
            ...
            logger.debug(
                "Problem with PollThread Config, defaulting to block size of 5 in master_updateMon...")
        ...
        for j in jobs:
            ...
            if len(j.subjobs) > 0:
                ...
                for this_block in monitorable_blocks:
                    ...
                    # If the monitoring function was running at the start of the function but
                    # has since stopped, break.
                    if was_monitoring_running and monitoring_component and not monitoring_component.isEnabled(
                            False) or not monitoring_component:
                        break
                    ...
                    try:
                        ...
                        if multiThreadMon:
                            if queues.totalNumIntThreads() < getConfig("Queues")[
                                    'NumWorkerThreads']:
                                queues._addSystem(
                                    j.backend.updateMonitoringInformation, args=(
                                        subjobs_to_monitor,), name="Backend Monitor")
                        else:
        ...
        if len(simple_jobs) > 0:
            for this_backend in simple_jobs.keys():
                logger.debug('Monitoring jobs: %s',
                             repr([jj._repr() for jj in simple_jobs[this_backend]]))
                if multiThreadMon:
                    if queues.totalNumIntThreads() < getConfig("Queues")['NumWorkerThreads']:
                        queues._addSystem(
                            stripProxy(
                                simple_jobs[this_backend][0].backend).updateMonitoringInformation, args=(
                                simple_jobs[this_backend],), name="Backend Monitor")
                else:
                    stripProxy(
                        simple_jobs[this_backend][0].backend).updateMonitoringInformation(
