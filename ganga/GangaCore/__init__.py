...
# If environment variable GANGA_SWAN_INTEGRATION is present enable this
# mechanism.
...
def getLCGRootPath():
    ...
    # CAUTION This could be sensitive to mixed AFS/CVMFS running but I doubt
    # this setup is common or likely
...
# config module - write protection is done in bootstrap at present though
...
sys_config = makeConfig(
    'System',
    "parameters of this ganga session (read-only)",
    cfile=False)
...
conf_config.addOption('autoGenerateJobWorkspace', False,
...
conf_config.addOption(
    'ServerUserScript',
    "",
    "Full path to user script to call periodically. The script will be executed as if called within Ganga by 'execfile'.")
conf_config.addOption(
...
conf_config.addOption(
    'AutoStartReg',
    True,
    'AutoStart the registries, needed to access any jobs in registry therefore needs to be True for 99.999% of use cases')
# ------------------------------------------------
# IPython
ipython_config = makeConfig(
    'TextShell_IPython',
...
poll_config.addOption(
    'autostart',
    True,
    'enable monitoring automatically at startup, in script mode monitoring is disabled by default, in interactive mode it is enabled',
    type=type(True))  # enable monitoring on startup
poll_config.addOption(
    'autostart_monThreads',
    True,
...
poll_config.addOption(
    'HeartBeatTimeOut',
    sys.maxsize,
    'Time before the user gets the warning that a thread has locked up due to failing to update the heartbeat attribute')
...
root_config.addOption(
    'location',
    '%s/ROOT/6.04.02/x86_64-slc6-gcc48-opt' %
    getLCGRootPath(),
    'Location of ROOT')
root_config.addOption(
    'path',
...
root_config.addOption(
    'pythonhome',
    '%s/Python/2.7.9.p1/x86_64-slc6-gcc48-opt' %
    getLCGRootPath(),
...
lcg_config.addOption('GLITE_WMS_WMPROXY_ENDPOINT', '',
...
lcg_config.addOption(
    'ArcWaitTimeBeforeStartingMonitoring',
    240,
...
# ------------------------------------------------
# GridSimulator
gridsim_config = makeConfig('GridSimulator',
...
lsf_config.addOption(
    'submit_res_pattern',
    r'^Job <(?P<id>\d*)> is submitted to .*queue <(?P<queue>\S*)>',
...
lsf_config.addOption(
    'kill_res_pattern',
    r'(^Job <\d+> is being terminated)|(Job <\d+>: Job has already finished)|(Job <\d+>: No matching job found)',
...
sge_config.addOption(
    'kill_res_pattern',
    r'(has registered the job +\d+ +for deletion)|(denied: job +"\d+" +does not exist)',
    "String pattern for replay from the kill command")
...
slurm_config.addOption(
    'submit_res_pattern',
    r'^Submitted batch job (?P<id>\d+)\s*',
    "String pattern for replay from the submit command")
...
merge_config.addOption('associate',
                       {'log': 'TextMerger',
                        'root': 'RootMerger',
                        'text': 'TextMerger',
                        'txt': 'TextMerger'},
...
# ------------------------------------------------
# Preparable
preparable_config = makeConfig('Preparable',
...
# FIXME Sometimes the wrong user is set gere for the unittests, I've added
# this to correct for it - rcurrie
try:
...
disp_config.addOption(
    'tasks_show_help',
    True,
    'change this to False if you do not want to see the help screen if you first type "tasks" in a session')
