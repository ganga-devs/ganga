...
class TestStructure(GangaUnitTest):
    ...
    def runJobSlice(self):
        ...
        for j in self.jobslice:
            assert run_until_completed(
                j, timeout=10), 'Timeout on job submission: job is still not finished'
    ...
    def testStructureCreated(self):
        ...
        for j in self.jobslice:
            ...
            assert os.path.exists(os.path.join(j.outputdir, 'subdir', 'out.txt')
                                  ), 'File in directory must exist'
