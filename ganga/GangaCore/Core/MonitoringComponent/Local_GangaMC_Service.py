...
def checkHeartBeat(global_count):
    ...
    for this_thread in ThreadPool:
        ...
        if (latest_timeNow - last_time) > dead_time and this_thread.isAlive()\
            ...
            log.warning(
                "Thread: %s Has not updated the heartbeat in %ss!! It's possibly dead" %
                (thread_name, str(dead_time)))
...
class UpdateDict(object):
    ...
    def addEntry(self, backendObj, backendCheckingFunction, jobList, timeoutMax=None):
        ...
        if backend in self.table:
            ...
            log.debug("**Adding %s to new %s backend entry." %
                      ([stripProxy(x).getFQID('.') for x in jobList], backend))
            return True
        ...
        # purchases as well! ;p
        log.debug("*: backend=%s, isOwner=%s, joblist=%s, queue=%s" %
                  (backend, lock._is_owned(), [x.id for x in jobList], Qin.qsize()))
        if lock.acquire(False):
            try:
                ...
                if jSetSize:  # jSet not cleared
                    log.debug("%s backend job set exists. Added %s to it." %
                              (backend, [stripProxy(x).getFQID('.') for x in jobList]))
                else:
                    Qin.put(
                        JobAction(
                            backendCheckingFunction,
                            self.table[backend].updateActionTuple()))
                    log.debug("Added new %s backend update action for jobs %s." % (
                        backend, [stripProxy(x).getFQID('.') for x in self.table[backend].updateActionTuple()[1]]))
            ...
            log.debug(
                "**: backend=%s, isOwner=%s, joblist=%s, queue=%s" %
                (backend, lock._is_owned(), [
                    stripProxy(x).getFQID('.') for x in jobList], Qin.qsize()))
            return True
            ...
            if entry.timeoutCounter == entry.timeoutCounterMax and entry.entryLock.acquire(False):
                with release_when_done(entry.entryLock):
                    ...
                    if entry.timeoutCounter <= 0.0:
                        ...
                        entry.timeLastUpdate = time.time()
                        log.debug(
                            "%s backend counter timeout. Resetting to %s." %
    ...
    def isBackendLocked(self, backend):
        if backend in self.table:
            return self.table[backend].entryLock._is_owned()
        else:
            return False
...
def autoKill_if_required(jobList_fromset):
    for j in jobList_fromset:
        if j.master:
            ...
            n_completed = len([sj for sj in j.master.subjobs if (
                sj.status == 'completed' or sj.status == 'completing')])
    ...
    for j in jobList_fromset:
        ...
        if len(j.subjobs) == 0:
            ...
            if n_failed > 0 and (
                ...
                float(
                    n_completed +
                    n_failed)) >= config['MaxFracForResubmit']:
...
class JobRegistry_Monitor(GangaThread):
    ...
    def __init__(self, registry_slice):
        ...
        for afsToken in credential_store.get_all_matching_type(AfsToken()):
            ...
            self.setCallbackHook(
                JobRegistry_Monitor.makeCredCheckJobInsertor, {
                    'thisMonitor': self, 'credObj': afsToken}, True, timeout=config['creds_poll_rate'])

        # Add low disk-space checking to monitoring loop
        log.debug("Setting callback hook for disk space checking")
        self.setCallbackHook(
            JobRegistry_Monitor.diskSpaceCheckJobInsertor, {
                'thisMonitor': self}, True, timeout=config['diskspace_poll_rate'])
    ...
    def run(self):
        ...
        while self.alive:
            if GANGA_SWAN_INTEGRATION:
                ...
                new_jobs = stripProxy(
                    self.registry_slice).objects.repository.update_index(
                    True, True)
    ...
    def __monStep(self):
        ...
        for cbHookFunc in self.callbackHookDict.keys():
            ...
            log.debug("(time.time() - cbHookEntry._lastRun): %s" %
                      str((time.time() - cbHookEntry._lastRun)))
            ...
            if cbHookEntry.enabled and (time.time() - cbHookEntry._lastRun) >= cbHookEntry.timeout:
                log.debug(
                    "Running monitoring callback hook function %s(**%s)" %
    ...
    def setCallbackHook(self, func, argDict, enabled, timeout=0):
        ...
        if func_name in self.callbackHookDict:
            log.debug('Replacing existing callback hook function %s with %s' %
                      (str(self.callbackHookDict[func_name]), func_name))
        self.callbackHookDict[func_name] = [
            func,
            CallbackHookEntry(
                argDict=argDict,
                enabled=enabled,
    ...
    def __defaultActiveBackendsFunc(self, jobSlice=None):
        ...
        for i in fixed_ids:
            try:
                ...
                if job_status in ['submitted', 'running'] or (
                        j.master and (job_status in ['submitting'])):
    ...
    def _checkBackend(self, backendObj, jobListSet, lock):
        ...
        try:
            log.debug(
                "[Update Thread %s] Lock acquired for %s" %
                (currentThread, getName(backendObj)))
            ...
            # rcurrie
            alljobList_fromset = list([x for x in jobListSet if x.status in [
            ...
            try:
                ...
                for this_job_list in all_job_bunches:
                    ...
                    # ALL ERRORS AND EXCEPTIONS ARE REPORTED VIA log.error SO NO INFORMATION
                    # IS LOST/IGNORED HERE!
                    job_ids = ''
                    ...
                    try:
                        ...
                        # This would allow us to continue in the case of errors due to bad
                        # job/backend combinations
            ...
            lock.release()
            log.debug(
                "[Update Thread %s] Lock released for %s." %
                (currentThread, getName(backendObj)))
            self._runningNow = False
    ...
    def makeUpdateJobStatusFunction(self, makeActiveBackendsFunc=None, jobSlice=None):
        ...
        self.setCallbackHook(
            self._checkActiveBackends, {
                'thisMonitor': self, 'activeBackendsFunc': makeActiveBackendsFunc, 'jobSlice': jobSlice}, True)
        log.debug("Returning")
    ...
    def makeCredCheckJobInsertor(thisMonitor, credObj):
        def credCheckJobInsertor():
            ...
            def cb_Failure():
                thisMonitor.enableCallbackHook(credCheckJobInsertor)
                thisMonitor._handleError(
                    '%s checking failed!' %
                    getName(credObj), getName(credObj), False)
    ...
    def diskSpaceCheckJobInsertor(thisMonitor):
        ...
        def cb_Failure():
            ...
            thisMonitor._handleError(
                'Available disk space checking failed and it has been disabled!',
                'DiskSpaceChecker',
                False)
    ...
    def _handleError(self, x, backend_name, show_traceback):
        ...
        if self.errors[bn] == 0:
            ...
            if not config['repeat_messages']:
                log.info(
                    'Further error messages from %s handler in the monitoring loop will be skipped.' %
                    bn)
        else:
    ...
    try:
        ...
        for worker in ThreadPool:
            ...
            if hasattr(worker, '_frame'):
                ...
                if frame:
                    status = status + "    stack:\n"
                    for frame, filename, line, function_name, context, index in inspect.getouterframes(
                            frame):
