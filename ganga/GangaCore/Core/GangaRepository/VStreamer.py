...
class VStreamer(object):
    ...
    def showAttribute(self, node, name):
        return (self.level > 1 or name not in self.selection) and not node._schema.getItem(
            name)['transient']
...
class Loader(object):
    ...
    def parse(self, s):
        ...
        def start_element(name, attrs):
            ...
            assert self.stack is not None, "missing <root> element"
        ...
        def end_element(name):
            ...
            if name == 'value':
                try:
                    ...
                    raise GangaException(
                        "ERROR in loading XML, failed to correctly parse attribute value: \'%s\'" % str(
                            self.value_construct))
