...
class PrimeFactorizer(IApplication):
    """
    PrimeFactorizer application -->
        factorize any arbitrary number into prime numbers.
    """
    _schema = Schema(
        Version(
            1, 1), {
            'number': SimpleItem(
                defvalue=1, sequence=0, typelist=[
                    'int', 'long'], doc='The number to be factorized.'), })
...
# FIXME: a cleaner solution, which is integrated with type information
# in schemas should be used automatically
...
class RTHandler(IRuntimeHandler):
    def prepare(self, app, appconfig, appmasterconfig, jobmasterconfig):
        from GangaCore.GPIDev.Adapters.StandardJobConfig \
            import StandardJobConfig
