...
class PrimeTableDataset(Dataset):
    ...
    _schema = Schema(
        Version(
            ...
            0),
        {
            'table_id_lower': SimpleItem(
                defvalue=1,
                doc='The lower bound id of the lookup tables (inclusive).'),
            'table_id_upper': SimpleItem(
                defvalue=1,
                doc='The upper bound id of the lookup tables (inclusive).'),
            'table_id_max': SimpleItem(
                defvalue=15,
                hidden=1,
                doc='The upper bound id of the lookup tables (inclusive).'),
            'table_location': SimpleItem(
                defvalue='http://cern.ch/diane/download/primes',
                doc='The location of the lookup tables.')})
    ...
    def get_dataset(self):
        ...
        if self.table_id_upper > self.table_id_max:
            logger.warning(
                'table_id_upper is restricted to up to %d' %
                self.table_id_max)
