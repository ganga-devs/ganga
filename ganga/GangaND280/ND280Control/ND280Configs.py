...
inputfile =
    ...
    def __init__(self, cfgtype, extra_opts):
        ...
        if self.cfgtype == 'gnsetup':
            ...
            self.options['genie_setup_script'] = self.t2ksoftdir + \
                '/GENIE/setup.sh'
    ...
    # Raw Data Options: Dictionary of options specific to Raw data processing
    # cfg files
    ...
    # GENIE Setup Options: Dictionary of options specific to GENIE setup cfg
    # files
    ...
    # GRID options are not considered compulsory but I include here for
    # totality.
    ...
    def CheckOptions(self):
        allOK = 1
        # Quick check to see if there are any blank options, only allowed for
        # comment.
    ...
    def CreateRawCF(self):
        ...
        if self.options['db_time']:
            configfile += "database_rollback_date = " + \
                self.options['db_time'] + "\n"
            configfile += "dq_database_rollback_date = " + \
                self.options['db_time'] + "\n"
        ...
        if self.options['midas_file']:
            ...
            configfile += "event_select = " + \
                self.options['event_select'] + "\n"

            if self.options['process_truncated']:
                configfile += "process_truncated = " + \
                    self.options['process_truncated'] + "\n"
                configfile += "\n"
            if self.options['num_events']:
                configfile += "num_events = " + \
                    self.options['num_events'] + "\n"
            configfile += "\n"
        ...
        if 'use_grid' in self.options and self.options['use_grid'] == '1':
            ...
            configfile += "storage_address = " + \
                self.options['storage_address'] + "\n"
            configfile += "register_files = " + \
                self.options['register_files'] + "\n"
            configfile += "register_catalogue_files = " + \
                self.options['register_catalogue_files'] + "\n"
            configfile += "register_address = " + \
                self.options['register_address'] + "\n"
    ...
    def CreateCosmicMCCF(self):
        ...
        if self.options['stage'] != 'base':
            ...
            configfile += "module_list = " + \
                self.options['module_list'] + "\n\n"
        ...
        configfile += "replace_comment = " + \
            self.options['replace_comment'] + "\n"
        configfile += "comment = " + \
            self.options['stage'] + "cosmiccorsika5F\n\n"

        # Geometry
        configfile += "[geometry]\n"
        configfile += "baseline = " + self.options['baseline'] + "\n"
        configfile += "p0d_water_fill = " + \
            self.options['p0d_water_fill'] + "\n\n"

        # nd280mc
        configfile += "[nd280mc]\n"
        configfile += "mc_type = " + self.options['mc_type'] + "\n"
        configfile += "random_seed = " + \
        ...
        configfile += "randomize_kinfile = " + \
            self.options['randomize_kinfile'] + "\n\n"
    ...
    def CreateSandMCCF(self):
        ...
        # Software Setup
        configfile += "[software]\n"
        configfile += "neut_setup_script = " + \
            self.options['neut_setup_script'] + "\n\n"
        ...
        configfile += "force_volume_name = " + \
            self.options['force_volume_name'] + "\n"
        configfile += "random_start = " + self.options['random_start'] + "\n"
        configfile += "random_seed = " + \
            str(random.randint(1, 999999999)) + "\n"
        configfile += "neut_seed1 = " + \
            str(random.randint(1, 999999999)) + "\n"
        configfile += "neut_seed2 = " + \
            str(random.randint(1, 999999999)) + "\n"
        configfile += "neut_seed3 = " + \
            str(random.randint(1, 999999999)) + "\n\n"

        # Dead channels
        configfile += "[dead_channels]\n"
        configfile += "tpc_periods_to_activate = " + \
            self.options['tpc_periods_to_activate'] + "\n"
        configfile += "ecal_periods_to_activate = " + \
