...
class ND280Transform_CSVEvtList(ITransform):
    ...
    def createUnits(self):
        ...
        # Look at the application schema and check if there is a csvfile
        # variable
        try:
            csvfile = self.application.csvfile
        except AttributeError:
            logger.error(
                'This application doesn\'t contain a csvfile variable. Use another Transform !')
            return
        ...
        for s, sub in enumerate(subsets):
            ...
            # check if this data is being run over by checking all the names
            # listed
            ok = False
        ...
        if not use_copy_output or not copy_output_ok:
            ...
            for parent in parent_units:
                ...
                for f in job.outputfiles:
                    ...
                    # This will be A LOT easier with Ganga 6.1 as you can
                    # easily map outputfiles -> inputfiles!
