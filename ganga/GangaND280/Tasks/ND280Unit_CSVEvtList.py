...
class ND280Unit_CSVEvtList(IUnit):
    _schema = Schema(
        ...
        dict(
            IUnit._schema.datadict.items() + {
                'subpartid': SimpleItem(
                    defvalue=-1,
                    typelist=['int'],
                    doc='Index of this unit which is important when the original CSV file was split by the transform. Otherwise leave it at -1'),
                'eventswanted': SimpleItem(
                    defvalue='',
                    typelist=[
                        'str',
                        'list'],
                    doc='CSV list of run, subrun, and event numbers wanted.'),
            }.items()))
    ...
    def createNewJob(self):
        ...
        if isinstance(self.eventswanted, type('')):
        ...
        j._impl.getInputWorkspace().writefile(
            FileBuffer(thiscsv, subLines), executable=0)
