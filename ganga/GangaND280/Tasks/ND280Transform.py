...
class ND280Transform(ITransform):
    _schema = Schema(
        ...
        dict(
            ...
            {
                'nbinputfiles': SimpleItem(
                    defvalue=1,
                    doc='The max number of files assigned to each unit. Use 0 to put all the available files in each given inputdata in one unit (i.e. N inputdata => N units).'),
                'inputdatasubsets': SimpleItem(
                    defvalue=[],
                    hidden=1,
                    doc='List of subsets of files. The number of files in each subset is equal to nbinputfiles'),
            }.items()))
        ...
        for idx, inds in enumerate(self.inputdata):
            ...
            if inds._name == "ND280LocalDataset" or inds._name == "ND280DCacheDataset":
                ...
                if len(self.inputdatasubsets) == idx:
                    ...
                    raise Exception(
                        'ND280Transform: The inputdata and inputdatasubsets are out of sync. This should not happen.')

                ok = True

                for subset in self.inputdatasubsets[idx]:
                    # check if this data is being run over by checking all the
                    # names listed
                    ok = False
    ...
    def createChainUnit(self, parent_units, use_copy_output=True):
        ...
        # all parent units must be completed so the outputfiles are filled
        # correctly
        for parent in parent_units:
        ...
        if not use_copy_output or not copy_output_ok:
            ...
            unit.inputdata = ND280LocalDataset()
            for parent in parent_units:
                # loop over the output files and add them to the
                # ND280LocalDataset - THIS MIGHT NEED SOME WORK!
                job = GPI.jobs(parent.active_job_ids[0])

                # if TaskChainInput.include_file_mask is not used go old way (see below)
                # otherwise add all file matching include_file_mask(s) to the
                # unit.inputdata. DV.
                inc_file_mask = False
                ...
                if not inc_file_mask:
                    for f in job.outputfiles:
                        ...
                        try:
                            ...
                            # WARNING: The following will work only if the
                            # SharedFile puts the files in local directories !
                            inputdir = '/'.join(
                                [
                                    os.path.expanduser(
                                        os.path.expandvars(
                                            getConfig('Output')['SharedFile']['uploadOptions']['path'])),
                                    f.outputfilenameformat.replace(
                                        '{fname}',
                                        '')])
            ...
            for parent in parent_units:
                # unit needs to have completed and downloaded before we can get
                # file list
                if parent.status != "completed":
                    return None
                ...
                for f in parent.copy_output.files:
                    unit.inputdata.names.append(os.path.join(
                        parent.copy_output.local_location, f))
