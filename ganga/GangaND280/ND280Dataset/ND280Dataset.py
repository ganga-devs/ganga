...
class ND280Dataset(Dataset):
    ...
    _schema = Schema(Version(1, 0), {'names': SimpleItem(defvalue=[], typelist=[
                     'str'], sequence=1, doc='List of input files with full path'), })
    ...
    def get_filenames(app):
        ...
        if not classnamematch:
            logger.warning(
                'Dataset is not a class inheriting from ND280Dataset.')
            return []
...
class ND280LocalDataset(ND280Dataset):
    ...
    _schema = Schema(Version(1, 1), {'names': SimpleItem(defvalue=[], typelist=[
                     'str'], sequence=1, doc='List of input files with full path'), })
    ...
    def get_dataset(self, directory, filter=None):
        ...
        if filter:
            new_names = [
                os.path.join(
                    directory,
                    name) for name in fnmatch.filter(
                    sorted(
    ...
    def get_kin_range(self, fr, to):
        ...
        logger.info(
            'Producing a list of kin file numbers in the range from %s to %s.',
            fr,
            to)
...
class ND280DCacheDataset(ND280Dataset):
    ...
    _schema = Schema(
            ...
            0),
        {
            ...
            'server': SimpleItem(
                defvalue="TRIUMF",
                doc='Name of the dcache server used'),
        })
    ...
    def get_dataset(self, directory, filter=None):
        ...
        if self.server not in self._commandstr.keys():
