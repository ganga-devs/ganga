...
class VFT_make_ana(IPrepareApp):
    ...
    _schema = Schema(
        Version(1, 1),
        {
            'cmtsetup': SimpleItem(
                defvalue=None,
                doc='Setup script in bash to set up cmt and the cmt package of the executable.',
                typelist=['str', 'type(None)'],
            ),
            'tree': SimpleItem(
                defvalue=None,
                doc='Name of the TTree to analyze in the input file(s). The default name is ND280ValidTree.',
                typelist=['str', 'type(None)'],
            ),
            'ana_custom': SimpleItem(
                defvalue=None,
                doc='Name of the make_ana.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            'ana_output': SimpleItem(
                defvalue=None,
                doc='Output filename or filenames. Takes a string or a list of strings.',
                typelist=['str', 'type(None)'],
            ),
            'ana_useropt': SimpleItem(
                defvalue=[],
                typelist=['str'],
                sequence=1,
                strict_sequence=0,
                doc="List of user options to pass to make_ana.py using -0. No need to provide '-O'.",
            ),
            'run_pdf': SimpleItem(
                defvalue=True,
                doc='Switch to run make_pdf.py on the make_ana.py output.',
                typelist=['bool'],
            ),
            'pdf_custom': SimpleItem(
                defvalue=None,
                doc='Name of the make_pdf.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            'pdf_title': SimpleItem(
                defvalue=None,
                doc='Name of the make_ana.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            ...
            'pdf_oldrdp': SimpleItem(
                defvalue=None,
                doc='Name of the make_ana.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            'pdf_oldmcp': SimpleItem(
                defvalue=None,
                doc='Name of the make_ana.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            'pdf_rdptitle': SimpleItem(
                defvalue=None,
                doc='Name of the make_ana.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            'pdf_mcptitle': SimpleItem(
                defvalue=None,
                doc='Name of the make_ana.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            'pdf_oldrdptitle': SimpleItem(
                defvalue=None,
                doc='Name of the make_ana.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            'pdf_oldmcptitle': SimpleItem(
                defvalue=None,
                doc='Name of the make_ana.py module to use for this analysis.',
                typelist=['str', 'type(None)'],
            ),
            'pdf_output': SimpleItem(
                defvalue=None,
                doc='Output filename or filenames. Takes a string or a list of strings.',
                typelist=['str', 'type(None)'],
            ),
            'pdf_options': SimpleItem(
                defvalue=[],
                typelist=['str'],
                sequence=1,
                strict_sequence=0,
                doc="List of user options to pass to make_pdf.py.",
            ),
    ...
    def configure(self, masterappconfig):
        ...
        args = []
        args.append(
            '${ND280ANALYSISTOOLSROOT:-${RECONUTILSROOT}}/macros/grtf_VFT/make_ana.py'
        )
        ...
        if self.ana_custom is not None:
            args.append('-c')
            args.append(self.ana_custom)

        if self.ana_useropt is not None:
        ...
        if self.run_pdf:
            ...
            args.append(
                '${ND280ANALYSISTOOLSROOT:-${RECONUTILSROOT}}/macros/grtf/pdfgen/make_pdf.py'
            )

            if 'ana_output' not in [
                self.pdf_rdp,
                self.pdf_mcp,
                self.pdf_oldrdp,
                self.pdf_oldmcp,
                ...
                raise ApplicationConfigurationError(
                    'None of the pdf inputs is set to use the make_ana.py output. \
                    Please set "pdf_rdp", "pdf_mcp", "pdf_oldrdp", or "pdf_oldmcp" to the value "ana_output"'
                )
            ...
            # argDict = {
            #     '--custom': self.pdf_custom,
            #     '--title': self.pdf_title,
            #     '--rdp': self.pdf_rdp,
            #     '--mcp': self.pdf_mcp,
            #     '--oldrdp': self.pdf_oldrdp,
            #     '--oldmcp': self.pdf_oldmcp,
            #     '--rdptitle': self.pdf_rdptitle,
            #     '--mcptitle': self.pdf_mcptitle,
            #     '--oldrdptitle': self.pdf_oldrdptitle,
            #     '--oldmcptitle': self.pdf_oldmcptitle,
            #     '--out': self.pdf_output,
            # }
            ...
            for opt in self.pdf_options:
                for key in argDict:
                    if opt.find(key) > -1 and getattr(self, argDict[key]) is not None:
                        raise ApplicationConfigurationError(
                            f'The make_pdf.py command line argument {key}'
                            f'was set through both the ganga application variable "{argDict[key]}" and '
                            f'pdf_options "{opt}". Use only one of them.'
                        )
                args.append(opt)
