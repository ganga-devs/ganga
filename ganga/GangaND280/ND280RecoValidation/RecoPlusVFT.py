...
class RecoPlusVFT(IPrepareApp):
            ...
            1),
        {
            ...
            'reco_exe': SimpleItem(
                ...
                typelist=[
                    ...
                    'type(None)'],
                comparable=1,
                doc='A path (string) or a File object specifying the reconstruction executable. For example oaRecon.exe.'),
            'reco_args': SimpleItem(
                defvalue=[],
                typelist=[
                    'str',
                    'GangaCore.GPIDev.Lib.File.File.File',
                    'int'],
                sequence=1,
                strict_sequence=0,
            ...
            'vft_only': SimpleItem(
                defvalue=True,
                doc='Run only VFT on the input files, i.e. don\'t call a reconstruction software.',
                typelist=['bool']),
            'vft_exe': SimpleItem(
                defvalue=None,
                typelist=[
                    ...
                    'type(None)'],
                comparable=1,
                doc='A path (string) or a File object specifying the validation flat tree executable. For example OAReconValidTree.exe.'),
            'vft_args': SimpleItem(
                defvalue=[],
                typelist=[
                    'str',
                ...
                strict_sequence=0,
                doc="List of arguments for the executable. Arguments may be strings, numerics or File objects."),
            'filenamesubstr': SimpleItem(
                defvalue=None,
                doc='This string will be substituted by recnewstr variable in the reco output filename and vftnewstr variable in the VFT output filename.',
                typelist=[
                    'str',
                    'type(None)']),
            'reconewstr': SimpleItem(
                defvalue='newreco',
                doc='This string will substitute filenamesubstr in the input filename to create the reco output filename.',
                typelist=['str']),
            'vftnewstr': SimpleItem(
                defvalue='validtree',
                doc='This string will substitute filenamesubstr in the input filename to create the VFT output filename.',
                typelist=['str']),
            'env': SimpleItem(
                defvalue={},
                typelist=['str'],
    ...
    def configure(self, masterappconfig):
        ...
        # So get the list of filenames get_dataset_filenames() and create a
        # file containing the list of files and put it in the sandbox
        ...
        if self.filenamesubstr is None:
            ...
            reco_outputfile = firstFile.replace(
                self.filenamesubstr, self.reconewstr)

        reco_args.append(reco_outputfile)

        # Just to define the output before the potentially long list of input
        # files
        ...
        if self.filenamesubstr is None:
            ...
            vft_outputfile = firstFile.replace(
                self.filenamesubstr, self.vftnewstr)
