...
class ND280Kin_Checker(IFileChecker):
    ...
    """
    _schema = IFileChecker._schema.inherit_copy()
    _schema.datadict['prfx'] = SimpleItem(
        defvalue=None,
        typelist=[
            'str',
            'type(None)'],
        doc='Path prefix to store output files')
    _schema.datadict['path'] = SimpleItem(
        defvalue=None,
        typelist=[
            'str',
            'type(None)'],
        doc='Middle path to store output files')
    _category = 'postprocessor'
    ...
    def check(self, job):
        ...
        if len(filepaths) != 1:
            logger.error(
                'Something wrong with kin file(s) ' +
                filepaths +
                '. CANNOT CONTINUE')
...
class ND280RDP_Checker(IFileChecker):
    ...
    _schema.datadict['path'] = SimpleItem(
        ...
        typelist=[
            ...
            'type(None)'],
        doc='Middle path to store output files')
    _schema.datadict['trig'] = SimpleItem(
        defvalue=None,
        typelist=[
            'str',
            'type(None)'],
        doc='Trigger type, e.g. SPILL or COSMIC')
    _schema.datadict['site'] = SimpleItem(
        defvalue=None,
        typelist=[
            'str',
            'type(None)'],
        doc='Processing site, e.g. wg-bugaboo')
    _schema.datadict['post_status'] = SimpleItem(
        defvalue=None,
        typelist=[
            'bool',
            'type(None)'],
        doc='Post job information to processingstatus db')
    _category = 'postprocessor'
    ...
    def send_status(self):
        ...
        logger.info(
            'Result for %s %s %s %s is: %s, %s, %s, %s, %s' %
            (self.RUN,
             self.SUBRUN,
             self.TRIGTYPE,
             self.STAGE,
        ...
        if not self.path:
            logger.error(
                "No monitoring info sent because MONDIR is not defined")
            return
    ...
    def check(self, job):
        ...
        filepaths = self.findFiles(job)
        if len(filepaths) != 1:
            # print 'Something wrong with logfile(s) '+filepaths+'. CANNOT
            # CONTINUE'
            logger.error(
                'Something wrong with logfile(s) ' +
                filepaths +
                '. CANNOT CONTINUE')
            self.move_output(job, ok=False)
            return False
        ...
        # print "for run %s, subrun %s, type %s" %
        # (self.RUN,self.SUBRUN,self.trig)
        logger.info("Starting to scan file " + filename)
        logger.info(
            "for run %s, subrun %s, type %s" %
            (self.RUN, self.SUBRUN, self.trig))
        ...
        for self.line in inlogf:
            ...
            if self.find('Midas File') and self.find('has been truncated'):
                ...
                # neutMC logs are filled by Fluka, they are huge and seems
                # useless
                ...
                if self.find('Segmentation fault'):
                    ...
                    if self.line == '"oaCherryPicker-geo_v5mr.bat: line 7"':
                        ...
                        logger.warning(
                            "This is an acceptable error - ignore it")
                    else:
                        self.ReturnCode = -2
                        self.InStage = 0
                        self.send_status()
                    ...
                    if IsMC == 1:
                        ...
                        if self.find(
                                'Disabling module GRooTrackerVtx') and IsGenie:
                    ...
                    if self.STAGE == 'COSMICTRIG':
                        # print "Not sure what this stage is yet - no call to
                        # post_status"
                        logger.warning(
                            "Not sure what this stage is yet - no call to post_status")
                    else:
                        self.send_status()
        ...
        if self.InStage == 1:
            # print "The stage "+self.STAGE+" has not completed succesfully,
            # Error is unknown"
            logger.error(
                "The stage " +
                self.STAGE +
                " has not completed succesfully, Error is unknown")
            self.ReturnCode = 0
            self.send_status()

        inlogf.close()
        # print "\nFinished scanning the log file. Last check return code
        # posted is "+str(self.ReturnCode)
        logger.info(
