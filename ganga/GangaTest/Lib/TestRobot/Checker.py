...
#    NewRelease = False
    ...
    def execute(self, runid):
        ...
        self._getconfig()
        # Email on Startup
        if (self.EmailOnStartup):
            subject = "Ganga - TestRobot automatic restart by crontab"
            text = "GangaTestRobot restarted on: %s" % (
        ...
        while not self.NewRelease:
            ...
            if (CurrentTime - self.LastCheckedTime) < SleepDelta:
                ...
                config.setSessionValue('LastCheckedTime',
                                       self.LastCheckedTime.strftime("%d %b %Y %H:%M:%S"))
            if self.NewRelease:
                config.setSessionValue('VersionNumber', self.VersionNumber)
                config.setSessionValue('VersionTime',
                                       self.VersionTime.strftime("%d %b %Y %H:%M:%S"))
                break
    ...
    def _getconfig(self):
        ...
        if (self.LastCheckedTime == '0'):
            ...
            self.LastCheckedTime = datetime.datetime(
                *(time.strptime(self.LastCheckedTime, "%d %b %Y %H:%M:%S")[0:6]))
        ...
        for h in inf.headers:
            if (h.find(label) != -1):
                if (h.find(type) != -1):
                    ...
                    msg = "File not found or of wrong type at '%s'" % (
                        self.DownloadURL + "VERSIONS")
