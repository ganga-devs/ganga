...
class StoreTestApplication(IApplication):
    _schema = Schema(
        Version(
            1, 0), {
            'factor': SimpleItem(
                defvalue=10), 'shared_config': SimpleItem(
                    defvalue=0, protected=True, copyable=False), 'specific_config': SimpleItem(
                        defvalue=0, protected=True, copyable=False)})
...
class StoreTestBackend(IBackend):
    _schema = Schema(
        Version(
            1, 0), {
            'shared_config': SimpleItem(
                defvalue=0, protected=True, copyable=True), 'specific_config': SimpleItem(
                    defvalue=0, protected=True, copyable=True), })
...
# This runtime handler just passes on the application configuration
# objects (adding a necessary wrapper around)
