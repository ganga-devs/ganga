...
class CrashType(GangaObject):
    _schema = Schema(Version(1, 0), {'expr': SimpleItem(defvalue=''),
                     'method': SimpleItem(defvalue=''), 'condition': SimpleItem(defvalue='')})
    ...
    def trigger(self, val=None):
        ...
        if sys._getframe(1).f_code.co_name == self.method:
            if val is None or not self.condition or eval(self.condition):
                logger.debug("triggering error at <method=%s, condition=%s, val=%s>, error expr=%s" %
                             (repr(self.method), repr(self.condition), repr(val), repr(self.expr)))
                exec(self.expr)
