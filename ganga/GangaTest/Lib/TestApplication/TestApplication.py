...
class TestApplication(IPrepareApp):
    _schema = Schema(Version(1, 0), {'exe': SimpleItem(defvalue='/usr/bin/env'),
                                     ...
                                     'args': SimpleItem(defvalue=["Hello World"], typelist=['str', 'GangaCore.GPIDev.Lib.File.File'], sequence=1, doc="List of arguments for the executable. Arguments may be strings or File objects."),
                                     # introduced for RTHandler compatibility
...
class TestAdvancedProperties(IApplication):
    _schema = Schema(Version(1, 0), {'exe': SimpleItem(
        defvalue='/usr/bin/env'), 'exe2': SimpleItem(defvalue='')})
...
class TestAdvancedFileProperties(IApplication):
    _schema = Schema(
        Version(
            1, 0), {
            'file_or_files': FileItem(
                sequence=1, defvalue=[], strict_sequence=0)})
