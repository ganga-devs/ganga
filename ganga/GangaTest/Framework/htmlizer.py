...
def appendLinesToSummaryReport(
        out,
        columns,
        data,
        render_links=True,
        append_total=True,
        append_coverage_report=False):
    totals = {}
    for line in sorted(data):
        ...
        cell = getHTMLTable(
            cells=(
                (('width=60%', text), ('width=40% align=right', coverage_link)),))
        print("""<tr>\n<td>%s</td>\n""" % cell, file=out)
...
def printTestCase(out, testcase, config=None):
    ...
    if resultNode:
        ...
        if gpip_type:
            # if the test type is "gpip", the link of standard output will be
            # indicated to the same standard output file.
            stdout = name.split()[0].replace("/", ".").split(":")[0] + ".ALL__" + config
        else:
            if config != 'Schema':
                stdout = name.split()[0].replace("/", ".").replace(":", ".") + "__" + config
            else:
                stdout = name.split()[0].replace("/", ".").replace(":", ".") + "_" + testcase.getAttribute(
                    'ganga_schema_version') + "_" + testcase.getAttribute('ganga_schema_userid') + "_" + config
        ...
        info = '%s<a class="small" href="../output/%s">View full output</a>' % (
            info, stdout + ".out")
        if name.lower().find('stats') >= 0:
            info = '%s &nbsp; <a class="small" href="../output/%s">Statistics</a>' % (
                info, stdout + ".stats")
        # link to code:

        if config != 'Schema':
            name = '<strong>%s</strong><br><a class="small" href="%s">[Source Code]</a>' % (
                name, code_repository_prefix + testcase_src + ext + code_repository_suffix)
        else:
            name = '<strong>%s: %s</strong><br><a class="small" href="%s">[Source Code]</a>' % (testcase.getAttribute(
                'ganga_schema_version'), testcase.getAttribute('ganga_schema_userid'), code_repository_prefix + testcase_src + ext + code_repository_suffix)
        ...
        if matcher is not None:
            ...
            savannah_page = r'https://its.cern.ch/jira/issues/?jql=\'External\ issue ID\' ~ \'bugs%s\'' % matcher.group(
                1)
...
def generate1stLevelReports(reports, categories=[]):
    ...
    appendSummaryHeader(
        out, title='Summarized results of tests performed on %s' %
        now, header='Package', columns=columns)
...
def generateSlowestTestsReport(out, group_testcases):
    ...
    print(getCSSStyles(), file=out)
    print(
        '<h3>List with top 25 testcases that took longest time to execute <a href="summary_top25.html">here</a></h3>',
        file=out)
    ...
    appendDetailedHeader(
        file,
        title='List with top 25 testcases that took longest time to execute performed on %s &nbsp;<a href="index.html">[BACK]</a>' %
        now)
...
def generateSchemaTestsReport(schema_reports):
    ...
    appendDetailedHeader(
        file,
        title='Historical schema compatibility tests performed on %s &nbsp;<a href="index.html">[BACK]</a>' %
        now)
                    ...
                    for (name, value) in testcase.attributes.items():
                        if name == 'name':
                            ...
                            if len(testcase_name) > 2:
                                category = testcase_name[2]
                            break
    all_testcases.sort(key=lambda s: list(
        map(int, s.getAttribute('ganga_schema_version').split('-')[0].split('.'))))
...
def generate2ndLevelReports(reports, categories=[]):
    ...
    for package in packages:
        ...
        appendSummaryHeader(
            file, title='%s : summarized results of tests performed on %s' %
            (package, now), navigation=True, header='Package', columns=columns)
    ...
    for category in category_lines:
        ...
        appendSummaryHeader(
            file, title='%s : summarized results of tests performed on %s' %
            (category, now), header='Package', columns=columns)
    ...
    for group in tests:
        ...
        appendDetailedHeader(
            file, title='%s : results of tests performed on %s &nbsp;<a href="summary_%s.html">[BACK]</a>' %
            (package, now, group.split('/')[0]))
...
def generateCoverageReports(packages, detailed=False):
    ...
    for package in packages:
        ...
        try:
            ...
            myLogger.error(
                "Problem in creating coverage report for package %s. Figleaf was called with the arguments %s" %
                (package, repr(args_list)))
...
def start(cmd_args=None):
    ...
    try:
        opts, args = getopt.getopt(
            cmd_args, "hd:p:s:o:", [
                "help", "dest-dir=", "code-repository-prefix=", "code-repository-suffix", "stdout-dir="])
    ...
    categories = ['Bugs', 'GPI']
    # generate statistics page for the 1st level (with 'Bugs' selected as
    # cross category between all top level packages)
