...
def printTestCase(out, testcase, config=None):
    ...
    if resultNode:
        ...
        if matcher is not None:
            ...
            savannah_page = 'https://its.cern.ch/jira/issues/?jql=\'External\ issue ID\' ~ \'bugs%s\'' % matcher.group(
                1)
...
def generate1stLevelReports(reports, categories=[]):
    ...
    appendSummaryHeader(out, title='Summarized results of tests performed on %s' %
...
def generateSlowestTestsReport(out, group_testcases):
    ...
    appendDetailedHeader(
        file, title='List with top 25 testcases that took longest time to execute performed on %s &nbsp;<a href="index.html">[BACK]</a>' % now)
...
def generateSchemaTestsReport(schema_reports):
    ...
    appendDetailedHeader(
        file, title='Historical schema compatibility tests performed on %s &nbsp;<a href="index.html">[BACK]</a>' % now)
...
def generate2ndLevelReports(reports, categories=[]):
    ...
    for package in packages:
        ...
        appendSummaryHeader(file, title='%s : summarized results of tests performed on %s' %
    ...
    for group in tests:
        ...
        appendDetailedHeader(
            file, title='%s : results of tests performed on %s &nbsp;<a href="summary_%s.html">[BACK]</a>' % (package, now, group.split('/')[0]))
...
def generateCoverageReports(packages, detailed=False):
    ...
    for package in packages:
        ...
        try:
            ...
            myLogger.error("Problem in creating coverage report for package %s. Figleaf was called with the arguments %s" % (
...
def start(cmd_args=None):
    ...
    if cmd_args is None:
        cmd_args = sys.argv[1:]
    try:
        opts, args = getopt.getopt(cmd_args, "hd:p:s:o:", [
