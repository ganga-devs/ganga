...
class GangaGPIPTestCase(unittest.TestCase):
    ...
    def _initializeTestCmd(self):
        self.testCmd = '%s --output-path=%s --description=\'%s\'' % (
            self.testCmd, self.output_path_template, self.description.replace('ALL', '%s'))
...
class GPIPPreparationTestCase(unittest.TestCase):
    ...
    def getCheckTest(self):
        try:
            ...
            raise Exception(
                'Failed to get the instance of "CheckTest".(Should get this after running the testcase.)')
...
class GPIPCheckTestCase(unittest.TestCase):
    ...
    def runCheckTest(self):
        try:
            ...
            assert(
                self.checkTest is not None), 'No instance of checktest, this should happen while the preparation of test is failed.'
...
class SimpleRunnerControl(object):
    ...
    def runPreparationTestCase(self):
        ...
        try:
            ...
            logger.warning(
                "Failed to get the instance of check test, because of the fail of the preparation of test, won't do the check test.")
            self.checkTest = None
            if run_status:
                logger.warning(
                    'The test preparation is done successfully, did you forget to return the instance of check test?')
            # Mark the run status as fail, in order to set to finish by driver.py.
            run_status = False
    ...
    def runCheckTest(self):
        ...
        checkTestCase = GPIPCheckTestCase(
            self.checkTest, "CheckTest", self.error, self.timeoutError)
