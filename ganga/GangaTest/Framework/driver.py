...
class UnitRunner:
    ...
    def load(self):
        ...
        try:
            ...
            # getattr(GangaCore.Runtime._prog.local_ns.__dict__, testClass)
            clazz = GangaCore.Runtime._prog.local_ns[testClass]
...
class GPIPRunner:
    def __init__(
        ...
        # description template - 'GangaDummy/old_test/Root/TestRoot/%s [GPIP]'
    ...
    def run(self):
        ...
        for fixture in test_extractor(self.testsuite):
            ...
            runner_control = SimpleRunnerControl(
                testName, runner, decorated_fixture, fixture_decorators, self.timeout)
            ...
            if not run_control.isFinished():
                if not run_control.runCheckTest():
                    if run_control.error:
                        ...
                        failedTests.append(
                            ["%s:%s" % (self.gpiptest_prefix, run_control.testName), 'Failed in running check test somehow.'])
    ...
    def _generate_runner(self, testName):
        ...
        if self.releaseTest:
            runner.xml_filename = self._xml_report_ext % os.path.join(
                self.report_outputpath, '%s_%s_%s' % (self.parent_report_name, self.pytest_name, testName))
    ...
    def _getTestSuite(self):
        ...
        for test in attrs:
            ...
            if stest.startswith('test'):
                testcase = GPIPPreparationTestCase(
                    self._get_tester_instance(),
                    self.description %
                    stest,
                    stest,
                    self.timeout,
...
def cleanup():
    if myconfig['AutoCleanup']:
        ...
        try:
            ...
            logger.exception(
                "Cleanup failed (%s)... will delete repository and workspace dirs manually " %
                str(e))
