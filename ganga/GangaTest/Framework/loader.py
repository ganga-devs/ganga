...
"""
    ...
    def __generateTestCaseFromGPITest(self, test_path, ganga_config_path, test_relpath=None):
        ...
        if self.releaseTesting:
            ...
            if self.schema_test is not '':
                output_path = os.path.join(self.outputPrefix, test_relpath.replace(
                    "/", ".") + "_" + self.schema_test + "_" + self.testID + "_" + self.configAlias)
            else:
        ...
        if os.path.isfile(test_cleanup):
            ...
            #    cleanUpPyFile)
        try:
            descr = os.path.join(test_dir[test_dir.index(self.testsTopDir) +
                                 len(self.testsTopDir) + 1:], test_filename) + " [GPI]"
        except ValueError:
    ...
    def __generateTestCaseFromGMPTest(self, test_path, ganga_config_path, test_relpath=None):
        ...
        try:
            descr = os.path.join(test_dir[test_dir.index(self.testsTopDir) +
                                 len(self.testsTopDir) + 1:], test_filename) + " [GPIM]"
        except ValueError:
            ...
            #print("sclazz: %s" % str(sclazz))
                if isinstance(clazz, type) and sclazz.split('.')[-1].find(test_filename) >= 0:
                    ...
                    for attrname in clazz_dict:
                        try:
                            descr = os.path.join(test_dir[test_dir.index(self.testsTopDir) +
                                                 len(self.testsTopDir) + 1:], test_filename, attrname) + " [PY]"
                        except ValueError:
        ...
        for name in dir(module):
            ...
            if isinstance(clazz, type) and sclazz.split('.')[-1].split("'")[0] == test_filename:
                ...
                if test is not None and attrname == test:
                    ...
                    try:
                        descr = os.path.join(test_dir[test_dir.index(self.testsTopDir) +
                                             len(self.testsTopDir) + 1:], test_filename, 'ALL') + " [GPIP]"
                    except ValueError:
                    ...
                    # print '%s *********' % descr
                    testcase = GangaGPIPTestCase(testCmd, descr, output_path,
                                                 self.report_outputpath, self.current_report_name, self.releaseTesting)
                    if testcase is not None:
                        tests.append(testcase)
        # print tests
        return tests
