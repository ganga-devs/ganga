...
class GangaTestLoader:
    ...
    def loadTests(self, patterns):
        ...
        if self.enableReleaseTests:
            ...
            pattern = _patterns[0]
            _log(
                self.logger, 'info', "Searching release tests matching [%s] in %s " %
                (pattern, self.testsTopDir))
            atoms = pattern.split(".")
    ...
    def filterTests(self, testcases, pattern):
        ...
        for key in testcases:
            tests = testcases[key]
            for t in tests:
                if key.startswith(pattern) or fnmatch.fnmatch(
                        t.getDescription().split()[0], pattern):
    ...
    def _loadReleaseTests(self, pattern):
        ...
        for config in self.config:
            ...
            if self.schema_test is not '':
                report_name = "%s_%s_%s__%s" % (pattern.replace(
                    "*", "ALL"), self.schema_test, self.testID, self.configAlias)
                ...
                for file in files:
                    ...
                    if file.endswith(".gpi") or file.endswith(".gpim"):
                        ...
                        if file.endswith(".gpi"):
                            test = self.__generateTestCaseFromGPITest(
                                os.path.join(root, file), config)
                            if test is not None:
                                list.append(test)
                        elif file.endswith(".gpim"):
                            test = self.__generateTestCaseFromGMPTest(
                                os.path.join(root, file), config)
                            if test is not None:
                                list.append(test)
                        tests[key] = list
                        ...
                        pys = self.__generateTestCasesFromPyUnitTest(
                            os.path.join(root, file), config)
                        if isinstance(pys, type([])) and len(pys) > 0:
                            list += pys
                        tests[key] = list
                        ...
                        list = tests.get(newroot, [])
                        #list.append("%s tests"%file)
                        gpips = self.__generateTestCasesFromGPIPTest(
                            os.path.join(root, file), config)
                        if isinstance(gpips, type([])) and len(gpips) > 0:
                            list += gpips
                        tests[key] = list
    ...
    def __loadLocalTest(self, file, gangaPackage='Ganga'):
        ...
        for config in self.config:
            ...
            config_path = "%s/ganga/%s/old_test/config/%s" % (
                self.releaseTopDir, gangaPackage, config)
            ...
            if ext in (".gpi", ".gpim"):
                ...
                if ext == ".gpi":
                    test = self.__generateTestCaseFromGPITest(
                        file, ganga_config_path, test_relpath=os.path.dirname(file))
                elif ext == ".gpim":
                    test = self.__generateTestCaseFromGMPTest(
                        file, ganga_config_path, test_relpath=os.path.dirname(file))
                if test is not None:
                ...
                gpips = []
                gpips = self.__generateTestCasesFromGPIPTest(
                    file, ganga_config_path, test=None, test_relpath=os.path.dirname(file))
                if isinstance(gpips, type([])) and len(gpips) > 0:
                    list += gpips
...
"""
    ...
    def __generateTestCaseFromGPITest(self, test_path, ganga_config_path, test_relpath=None):
        ...
        if test_relpath is None:  # relative to release
            test_relpath = test_path[len(self.testsTopDir) + 1:-
                                     len(os.path.splitext(test_filepart)[1])]
        ...
        if self.releaseTesting:
            ...
            if self.schema_test is not '':
                ...
                output_path = os.path.join(
                    self.outputPrefix, test_relpath.replace(
                        "/", ".") + "__" + self.configAlias)
        ...
        if os.path.isfile(test_cleanup):
            ...
            #    cleanUpPyFile)
        try:
            descr = os.path.join(test_dir[test_dir.index(
                self.testsTopDir) + len(self.testsTopDir) + 1:], test_filename) + " [GPI]"
        except ValueError:
    ...
    def __generateTestCaseFromGMPTest(self, test_path, ganga_config_path, test_relpath=None):
        ...
        # testCmdPrefix = "cd %s ; PYTHONUNBUFFERED=1 PYTHONPATH=%s %s/bin/ganga
        # -o[Configuration]RUNTIME_PATH=GangaTest --config= --config-path=%s"%(
        ...
        try:
            ...
            print(
                "[WARNING] Cannot parse Multipass test (invalid format) %s [skipped]-> %s" %
                (test_path, e))
            return
        ...
        for name in dir(module):
            ...
            if isinstance(
                    ...
                    type) and issubclass(
                    clazz,
                    MultipassTest) and sclazz.find(test_filename) >= 0:
        ...
        try:
            descr = os.path.join(test_dir[test_dir.index(
                self.testsTopDir) + len(self.testsTopDir) + 1:], test_filename) + " [GPIM]"
        except ValueError:
    ...
    def __generateTestCasesFromPyUnitTest(
            self,
            test_path,
            ganga_config_path,
            test=None,
            ...
            #print("sclazz: %s" % str(sclazz))
                if isinstance(clazz, type) and sclazz.split('.')[-1].find(test_filename) >= 0:
                    ...
                    for attrname in clazz_dict:
                        try:
                            descr = os.path.join(test_dir[test_dir.index(
                                self.testsTopDir) + len(self.testsTopDir) + 1:], test_filename, attrname) + " [PY]"
                        except ValueError:
    ...
    def __generateTestCasesFromGPIPTest(
            self,
            test_path,
            ganga_config_path,
            test=None,
        ...
        for name in dir(module):
            ...
            if isinstance(clazz, type) and sclazz.split('.')[-1].split("'")[0] == test_filename:
                ...
                if test is not None and attrname == test:
                    ...
                    try:
                        descr = os.path.join(test_dir[test_dir.index(
                            self.testsTopDir) + len(self.testsTopDir) + 1:], test_filename, 'ALL') + " [GPIP]"
                    except ValueError:
                    ...
                    # print '%s *********' % descr
                    testcase = GangaGPIPTestCase(
                        testCmd,
                        descr,
                        output_path,
                        self.report_outputpath,
