...
def main():
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "", ["version=", "interactive", "show-all", "experiment="])
    ...
    for o, a in opts:
        ...
        if o == '--experiment':
            if a not in ['atlas', 'lhcb', 'generic']:
    ...
    print >> sys.stderr
    print >> sys.stderr, 'Setting up Ganga %s (%s,%s)' % (
        versionStr, shellStr, experiment)

    setupcmds = {
        'lhcb': {
            'sh': """export PATH=%(PATH)s ; export GANGA_CONFIG_PATH=%(INSTALL_PREFIX)s/config/GangaLHCbRoot.ini:%(INSTALL_PREFIX)s/config/GangaLHCb.ini:GangaLHCb/LHCb.ini""",
        ...
        'atlas': {
            'sh': """alias ganga=%(BIN_DIR)s/ganga ; export GANGA_CONFIG_PATH=%(INSTALL_PREFIX)s/config/GangaAtlas%(NEWER44)s.ini:GangaAtlas/Atlas.ini""",
        ...
        'generic': {
            'sh': """alias ganga=%(BIN_DIR)s/ganga""",
            'csh': """alias ganga %(BIN_DIR)s/ganga"""}}

    vars = {
        'PATH': prepend_search_path(
            os.environ['PATH'],
            '%s/%s/bin' % (prefixStr,
                           versionStr),
            prefixStr),
        'INSTALL_PREFIX': prefixStr,
        'BIN_DIR': '%s/%s/bin' % (prefixStr,
                                  versionStr)}
    ...
    if _relcmp(versionStr, '4.4.0-beta1') < 0:
        ...
        # use %(INSTALL_PREFIX)s/config/GangaAtlas-v44.ini
        vars['NEWER44'] = '-v44'

    print setupcmds[experiment][shellStr] % vars

    # if versionStr contains hotfix: strip the hotfix part (use public
    # release) and replace GANGA_CONFIG_PATH=GangaLHCb/LHCb.ini by an absolute
    # path to your hotfix configuration file
...
def _cmpver(a, b):
    ...
    import re
    a = map(_stoi, re.findall(r"\d+|\w+", a))
    b = map(_stoi, re.findall(r"\d+|\w+", b))
...
def get_versions(prefixStr):
    ...
    public = filter(lambda x: re.compile(
        "(alpha|pre|beta)").search(x) is None, all)
    defaultVer = public[-1]
    return public, all
