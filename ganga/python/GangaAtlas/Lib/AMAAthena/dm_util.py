...
def resolve_dq2_local_site_id(ds_locations, site_domain, se_hostname, force_siteid_domain={}, force_siteid_se={}):
    ...
    # if enforcement is never applied, do the check via ToA
    if toa_check:
...
def make_FileStager_sampleFile(pfns, sampleFileName='sampleFile', gridCopy=True, override=True):
    '''
    creates the sampleFile for the Athena/FileStager module.

    @param pfns is a list of physical file locations
    @param sampleFileName is the name of the sampleFile output
    @param gridCopy indicates if grid copy is used in FileStager
    @param override indicates if overriding the existing sampleFileName

    @return True if sampleFile is finally produced

    by setting override to False, the PFNs are appended to the sampleList file.
    '''

    f = None

    if override:
        f = open(sampleFileName,'a')
    else:
        f = open(sampleFileName,'w')

    pfn_fs_prefix = ''
    if gridCopy:
        pfn_fs_prefix = 'gridcopy://'
...
def make_FileStager_jobOption(sampleFileName='sampleFile', optionFileName='input_stager.py'):
    '''
    creates the Athena job option file for FileStager.

    @param sampleFileName specifies the location of the FileStager sample file
...
stagetool = FileStagerTool(sampleFile=###SAMPLEFILEPATH###)
...
"""
    ...
    if os.path.exists(sampleFileName):
        jOption = jOption.replace('###SAMPLEFILEPATH###', repr(sampleFileName))
        f = open(optionFileName, 'w')
        f.write(jOption)
        f.close()
        ick = True
    else:
        print >> sys.stderr, 'sample file not available: %s' % sampleFileName
        ick = False
...
if __name__ == '__main__':
    ...
    # produce the sampleFile for Athena/FileStager module
    make_FileStager_sampleFile(pfn_list, sampleFileName='sampleFile')
    f = open('sampleFile','r')
    print >> sys.stdout, '##### File Stager Sample File #####'
    for l in f.readlines():
        print >> sys.stdout, l.strip()
    f.close()

    # produce the job option file for Athena/FileStager module
    make_FileStager_jobOption(sampleFileName='sampleFile', optionFileName='input_stager.py')
