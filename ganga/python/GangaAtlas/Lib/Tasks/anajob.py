...
class AnaJob(abstractjob.AbstractJob):
   ...
   def prepare(self, number = 1, backend = "Grid"):
      ...
      if sister_job_id>0:
         ...
         logger.info("Belonging to the same [Ana]Task object (%s): Setting application of job %d as in job %d "%(p.name,j.id,sister_job_id))
         ...
         if p.application_group_area:
            ...
            j.application.group_area=p.application_group_area 
         else:
      ...
      if isinstance(p.abstract_jobs[self.name]['sites_to_run'],str):self.sites=""
...
################## find out if sister jobs exist
   def get_sister_job(self):
      """ checks if any abstract jobs of the task already exist"""
      from Ganga.GPI import jobs
      p = self.get_task()
      for sis_j  in jobs:
...
################## set job applications
   def get_output_dataset(self,thisjob_id):
      """returns the dataset where the output results are installed"""
      from Ganga.GPI import jobs
      p = self.get_task()
      this_task_jobs=[]
      for sis_j  in jobs:
      ...
      if p.outputdata_datasetname:
         ...
         p.outputdata_datasetname="users."+username+".ganga."+jid_date
...
############################
   def register_dataset(self, datasetname=None):
      """registers the resulted dataset to qd2"""
      d=DQ2Dataset()
...
############################
   ...
   def check_appl_opt_fle(self,fle,protected_cont):
      """ ensures that the application option file stays unchanged during tasks's running time"""
      ...
      for i in range(len(fle_cont)):
         if fle_cont[i] != protected_cont[i]:
            ...
            logger.info("Compare the lines to your application file")
            return False
      return True
##################################
