...
class MDParser:
    ...
    def parseWhereClause(self):
        if DEBUG: print "parsing where clause: ", self.q
        if len(self.q) == 0:
            return True
        self.__qp = 0
        try:
    ...
    def parseStatement(self):
        if DEBUG: print "parsing statement: ", self.q
        self.__qp = 0
        try:
            token, v = self.__pStatement()
        except EmptyTableException:          
    ...
    def __isLiteral(self, start, literal):
        if self.__qp+len(literal) <= len(self.q) \
            ...
            if DEBUGS: 'Found literal ', literal
    ...
    def __readToken(self):
        if DEBUGS: print 'Reading token at ' + str(self.__qp)
        # Check whether it is in our parser dictionary
        t, v, next = self.parseDict[self.__qp]
        if t:
            if DEBUGS: print 'Returning ', [self.__qp], t, ' ', v
            self.__qp = next
            return t, v
        start = self.__qp
        ...
        if DEBUGS: print 'Reading token 2 '
        
        if self.__qp>= len(self.q):
            if DEBUGS: print "Token " + str(self.__qp) + " END"
        ...
        if t!= None:
            ...
            if DEBUGS: print "Token " + str(self.__qp) + " CONST " + t
        ...
        if DEBUGS: print 'Reading token 3 '
        ...
        if DEBUGS: print 'Reading token 4 '
        t=self.reTABLEREF.match(self.q[self.__qp:])
        if t != None:
            t = t.group()
            self.__qp=self.__qp+len(t)
            if DEBUGS: print "Token ", self.__qp, " TABLREF ", t
    ...
    def __pToken(self):
        ...
        if DEBUG: print 'Got ', token, " ", v 
        if token == 'CONST':
            if DEBUG: print 'Token: CONST ', v
            return 'CONST', v
    ...
    def __pEvaluateOp(self, op, exp1, exp2):
        if DEBUG: print 'Evaluating operation: ', exp1, op, exp2
        if op == '=':
    ...
    def __evalVar(self, token, v):
        if DEBUG: print "Evaluating variable ", token, " : ", v
        ...
        if token == 'TABLEREF':
            ...
            if mdtable.typeDict[v] == 'int':
                ...
                if DEBUG: print 'CONST ', row[mdtable.attributeDict[v]+1]
            else:
    ...
    def __pExpression(self):
        if DEBUG: print "Expression: Trying x and/or  y"
        myi=self.__qp
        if DEBUG: print 'Expression: was ' + str(self.__qp)
        token, v = self.__pComparison()
        if DEBUG: print 'Expression: now ' +str(self.__qp)
        if token:
            if DEBUG: print 'Comp ' + str(v)
            myi2 = self.__qp
            DEBUGS = True
            op, opv = self.__readToken()
            DEBUGS = False
            if DEBUG: print 'Expression token: ' + str(op) + str(opv)
            if op == 'and' or op == 'or':
                ntoken, nv = self.__pExpression()
                if not ntoken:
                    raise CommandException (8, 'Illegal query: expecting exp')
                if DEBUG: print 'Expression: reducing a op b'
                return self.__pEvaluateOp(op, v, nv)
            self.__qp = myi2
        self.__qp=myi
        
        if DEBUG: print 'Expression: Trying comp' + str(self.__qp)
        token, v = self.__pComparison()
        if not token:
            raise CommandException (8, 'Illegal query: expecting comp')
        if DEBUG: print "Expression: Found comp " + str(v)
        return token, v


    def __pComparison(self):
        if DEBUG: print "Comparison: Trying x =><  y"
        myi=self.__qp
        if DEBUG: print 'Comparison: was ' + str(self.__qp)
        token, v = self.__pSum()
        if DEBUG: print 'Comparison: now ' +str(self.__qp)
        if token:
            myi2 = self.__qp
            op, opv = self.__readToken()
            if DEBUG: print 'Comparison: got op ' + op + ' '
            if op == '=' or op == '>' or op == '<' \
                   or op == '<=' or op == '>=' or op == '!=':
                ntoken, nv = self.__pComparison()
                if not ntoken:
                    raise CommandException (8, 'Illegal query: expecting comp')
                if DEBUG: print 'Comparison: reducing a op b'
                return self.__pEvaluateOp(op, v, nv)
            self.__qp = myi2
        self.__qp=myi
        
        if DEBUG: print 'Comparison: Trying sum' + str(self.__qp)
        token, v = self.__pSum()
        if not token:
            raise CommandException (8, 'Illegal query: expecting sum')
        if DEBUG: print "Comparison: Found sum " + str(v)
        return token, v


    def __pSum(self):
        if DEBUG: print "Sum: Trying x +- y"
        myi=self.__qp
        if DEBUG: print 'Sum: was ' + str(self.__qp)
        token, v = self.__pTerm()
        if DEBUG: print 'Sum: now ' +str(self.__qp)
        if token:
            if DEBUG: print 'Sum ' + str(v)
            myi2 = self.__qp
            op, opv = self.__readToken()
            if op == '+' or op == '-':
        ...
        if DEBUG: print 'Sum: Trying term' + str(self.__qp)
        token, v = self.__pTerm()
        if not token:
            raise CommandException (8, 'Illegal query: expecting term')
        if DEBUG: print "Sum: Found term " + str(v)
        return token, v


    def __pTerm(self):
        if DEBUG: print "Term: Trying x */ y"
        myi=self.__qp
        if DEBUG: print 'Term: I was ' + str(self.__qp)
        token, v = self.__pFactor()
        if DEBUG: print 'Term: I now ' +str(self.__qp)
        if token:
            if DEBUG: print "Term: " +str(v)
            myi2 = self.__qp
            op, opv = self.__readToken()
            if op == '*' or op == '/':
        ...
        if DEBUG: print 'Term: Trying faktor' + str(self.__qp)
        token, v = self.__pFactor()
        if not token:
            raise CommandException (8, 'Illegal query: expecting factor')
        if DEBUG: print "Term: Found factor " + str(v)
        return token, v


    def __pFactor(self):
        if DEBUG: print "Factor: Trying ()"
        myi=self.__qp
        if DEBUG: print 'Factor: I was ' + str(self.__qp)
        token, v = self.__readToken()
        if DEBUG: print "Factor: now " + str(self.__qp)
        if token == '(':
            token, v = self.__pExpression()
            ntoken, nv = self.__readToken()
            if ntoken != ')':
...
#        if DEBUG:
        if DEBUG: print "Factor: Trying Token"
        token, v = self.__pToken()
        
        if not token:
            raise EmptyTableException
        if DEBUG: print "Factor: Found token " + str(v)
        return token, v
