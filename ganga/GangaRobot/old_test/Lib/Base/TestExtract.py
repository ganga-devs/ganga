...
class TestExtract(GangaGPITestCase):
    ...
    def test_fromdom(self):
        ...
        assert equal(
            expected, actual), 'node does not convert from dom as expected'
    ...
    def test_fromxml(self):
        ...
        assert equal(
            expected, actual), 'node does not convert from xml as expected'
    ...
    def test_addnode(self):
        ...
        assert equal(
            expected, actual), 'result of adding node is not as expected'

    def test_addnode_value(self):
        """Test addnode() adds a node with a value."""
        expected = Node.fromxml(
            '<my-node><my-sub-node>my-value</my-sub-node></my-node>')
        actual = Node('my-node')
    ...
    def test_getnodes(self):
        ...
        assert str(expected) == str(
            actual), 'getnodes() does not return expected list of current node.'
        actual = self.testnode.getnodes('')
        assert equal(
            expected, actual), "getnodes('') does not return expected list of current node."

    def test_getnodes_name(self):
        """Test getnodes('job') returns list of job subnodes."""
        expected = [n for n in self.testnode.nodes if n.name ==
                    'job']  # job subnodes
        actual = self.testnode.getnodes('job')
        assert equal(
            expected, actual), "getnodes('job') does not return expected list of job subnodes."
    ...
    def test_getnodes_wild(self):
        ...
        actual = self.testnode.getnodes('*')
        assert equal(
            expected, actual), "getnodes('*') does not return expected list of all subnodes."

    def test_getnodes_path(self):
        """Test getnodes('job.core') returns list of core subnodes of job subnodes."""
        expected = []
        for jobnode in [
                n for n in self.testnode.nodes if n.name == 'job']:  # job subnodes
            # core subnodes
            expected.extend([n for n in jobnode.nodes if n.name == 'core'])
        actual = self.testnode.getnodes('job.core')
        assert equal(
            expected, actual), "getnodes('job.core') does not return expected list of core subnodes of job subnodes."
    ...
    def test_getnodes_path_wild(self):
        ...
        for jobnode in [
                n for n in self.testnode.nodes if n.name == 'job']:  # job subnodes
            for corenode in [
                    n for n in jobnode.nodes if n.name == 'core']:  # core subnodes
                expected.extend(corenode.nodes)  # all subnodes
        actual = self.testnode.getnodes('job.core.*')
        assert equal(
            expected, actual), "getnodes('job.core.*') does not return expected list of all subnodes of job.core subnodes."
    ...
    def test_getnodes_path_wild_path(self):
        ...
        for jobnode in [
            ...
            for subnode in jobnode.nodes:  # all subnodes
                # id subnodes
                expected.extend([n for n in subnode.nodes if n.name == 'id'])
        actual = self.testnode.getnodes('job.*.id')
        assert equal(
            expected, actual), "getnodes('job.*.id') does not return expected list of id subnodes of job.* subnodes."
    ...
    def test_getnodes_wild_path(self):
        ...
        for subnode in self.testnode.nodes:  # all subnodes
            for corenode in [
                    n for n in subnode.nodes if n.name == 'core']:  # core subnodes
                # id subnodes
                expected.extend([n for n in corenode.nodes if n.name == 'id'])
        actual = self.testnode.getnodes('*.core.id')
        assert equal(
            expected, actual), "getnodes('*.core.id') does not return expected list of id subnodes of *.core subnodes."
    ...
    def test_getnodes_wild_wild(self):
        ...
        actual = self.testnode.getnodes('*.*')
        assert equal(
            expected, actual), "getnodes('*.*') does not return expected list of all subnodes of all subnodes."
    ...
    def test_getnode(self):
        ...
        assert equal(
            expected, actual), "getnode('job.core.id') does not return first job.core.id subnode as expected."
