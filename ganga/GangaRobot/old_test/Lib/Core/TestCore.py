...
class TestCore(GangaGPITestCase):
    ...
    def setUp(self):
        ...
        self.submitter.options = {'CoreSubmitter_Patterns': [
    ...
    def test_extractor(self):
        ...
        assert runnode.getvalue(
            'core.id') == self.runid, 'extracted runnode does not contain correct run id'
        # check runnode contains 3 job nodes
        assert len(runnode.getnodes('job')
                   ) == 3, 'extracted runnode does not contain 3 job nodes'
