...
def test_outputfiles_iterator():
    ...
    test_job = TestJob(
        outputfiles=[
            TestFileA(
                'A1', subfiles=[
                    TestFileA('AS1')]), TestFileA('A2'), TestFileB(
                'B1', subfiles=[
                    TestFileB('BS1')]), TestFileA('A3')], nc_outputfiles=[
            TestFileB('B2'), TestFileA('A4'), TestFileB(
                'B3', subfiles=[
                    TestFileB('BS2'), TestFileB('BS3')]), TestFileB('B4')])
    ...
    assert [
        f.name for f in outputfiles_iterator(
            test_job,
            TestFile,
            selection_pred=pred_a)] == ['A2']
    assert [
        f.name for f in outputfiles_iterator(
            test_job,
            TestFile,
            selection_pred=pred_b)] == ['BS2']
    assert [
        f.name for f in outputfiles_iterator(
            test_job,
            TestFile,
            selection_pred=pred_b,
            include_subfiles=False)] == []
