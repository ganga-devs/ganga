...
class NA62MC(IPrepareApp):
    ...
    _schema = Schema(
        Version(
            ...
            0),
        {
            'run_number': SimpleItem(
                defvalue=-1,
                typelist=['int'],
                doc="Run Number to pass to the scripts"),
            'seed': SimpleItem(
                defvalue=-1,
                typelist=['int'],
                doc="Random seed to pass to the scripts - overwritten by run_number if not given"),
            'num_events': SimpleItem(
                defvalue=-1,
                typelist=['int'],
                doc="Number of events to generate"),
            'decay_type': SimpleItem(
                defvalue=-1,
                typelist=['int'],
                doc="Decay type to generate"),
            'decay_name': SimpleItem(
                defvalue="",
                typelist=['str'],
                doc="String of the Decay type"),
            'mc_version': SimpleItem(
                defvalue=8,
                typelist=['int'],
                doc="MC version to use"),
            'revision': SimpleItem(
                defvalue=0,
                typelist=['int'],
                doc="MC revision to use"),
            'script_name': SimpleItem(
                defvalue="",
                typelist=['str'],
                doc="The name of the script to download and use on submission"),
            'radcor': SimpleItem(
                defvalue=False,
                typelist=['bool'],
                doc="Use radiatve corrections or not"),
            'file_prefix': SimpleItem(
                defvalue='pluto',
                typelist=['str'],
            ...
            'job_type': SimpleItem(
                defvalue='prod',
                typelist=['str'],
                doc="Job type (e.g. prod or test)"),
            'is_prepared': SharedItem(
                defvalue=None,
                strict_sequence=0,
                visitable=1,
                copyable=1,
    ...
    def getNextRunNumber(self):
        ...
        nec_str = open(os.path.join(work_dir, nec_file)
                       ).read().strip().strip('#')
        mysqlc = "mysql -hhughnon.ppe.gla.ac.uk -ugridbot -p%s -s GridJobs" % nec_str
        rc, out = getstatusoutput(
            "echo \"SELECT run FROM jobs ORDER BY run DESC LIMIT 1;\" | %s" %
            mysqlc)
        if rc != 0:
            logger.error(out)
        run_num = int(out) + 1
    ...
    def prepare(self, force=False):
        ...
        scr_path = os.path.join(
            ...
            self.script_name)
        os.system(
            "wget -q http://na62.gla.ac.uk/scripts/jw1v8.sh.txt -O %s" %
            scr_path)
        ...
        inp_path = os.path.join(
            ...
            "input_files.tgz")
        os.system(
            "wget -q http://na62.gla.ac.uk/input/input_files.tgz -O %s" %
            inp_path)
...
class RTHandler(IRuntimeHandler):
    def prepare(self, app, appconfig, appmasterconfig, jobmasterconfig):
        ...
        args = [app.getMACFileName(), "%s/r%d/%s" %
                (app.job_type, app.getRevision(), app.getDecayString())]
