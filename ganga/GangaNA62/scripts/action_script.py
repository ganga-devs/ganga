...
while not os.path.exists(os.path.join(work_dir, "GangaRun", "ganga.stop")):
    ...
    for ln in os.listdir(os.path.join(work_dir, "actions")):
        if ln.find("ganga-action") != -1:
            ...
            # check for other actions first - odd numbered clone/submit actions
            # means to wait
            rc, out = getstatusoutput(
                "echo \"SELECT COUNT(*) FROM events WHERE attr2 LIKE 'c%%-action-2%%';\" | %s" %
                mysqlc)
            num_clone = int(out)
            rc, out = getstatusoutput(
                "echo \"SELECT COUNT(*) FROM events WHERE attr2 LIKE 's%%-action-2%%';\" | %s" %
                mysqlc)
            num_submit = int(out)

            if (num_clone % 2 == 1) or (num_submit % 2 == 1):
                print(
                    "num. Clone (%d), Num. Submit (%d). Pending requests. Waiting." %
                    (num_clone, num_submit))
                continue
            ...
            os.system(
                "mv %s %s" %
                (os.path.join(
                    work_dir, "actions", ln), os.path.join(
                    work_dir, "actions", "done", ln)))

            # update the DB
            rc, out = getstatusoutput(
                "echo \"INSERT INTO events (type, attr1, attr2, attr3) VALUES ('action', 'ganga', '%s', '%d')\" | %s" %
