...
config = GangaCore.Utility.Config.makeConfig(
    'SAGA', 'parameters of the SAGA backend')
config.addOption(
    'remove_workdir',
    True,
...
class SAGA(IBackend):
    ...
    def prestagesandbox(self, jobconfig):
        ...
        if self.enable_compression:
            logger.info(
                "  * adding %s user defined files to input sandbox",
                len(sandbox_files))
            ...
            try:
                ...
                logger.error(
                    'exception caught while transfering file: %s',
        ...
        # if SAGA is using the Globus GridFTP adaptor.
            ...
            logger.info(
                "prestaging %s input sandbox files",
                len(sandbox_files))
            for f in sandbox_files:
                try:
                    ...
                    if(f.subdir != '.'):  # create subdirectory & copy
                        ...
                        sd = saga.filesystem.directory(
                            target, saga.filesystem.Create)
                        sf = saga.filesystem.file(source)

                        logger.info("  * copying %s -> %s ", source, target)
                        sf.copy(
                            target, saga.filesystem.Overwrite)  # copy the file
    ...
    def submit(self, jobconfig, master_input_sandbox):
        ...
        try:
            ...
            logger.error(
                'exception caught while submitting job: %s',
                e.get_all_exceptions())
    ...
    def resubmit(self):
        try:
            ...
            logger.error(
                'exception caught while re-submitting job: %s',
                e.get_all_exceptions())
            self.getJobObject().updateStatus("failed")
            return False
    ...
    def run(self, js_contact, jd):
        ...
        try:
            ...
            logger.info(
                "job submitted with internal job id: %s",
                self.saga_job_id)
    ...
    def makesagajobdesc(self, job, jobconfig):
        ...
        from GangaCore.GPIDev.Lib.File import FileBuffer
        jobscript = job.getInputWorkspace().writefile(
            FileBuffer('__jobscript__', text), executable=1)
        ...
        if len(job.backend.workdir) != 0:
            ...
            logger.debug(
                "  * backend.workdir -> saga.workdir: %s",
                jd.working_directory)
        else:  # default to the remote filesystem path component
            jd.working_directory = saga.url(
                self.filesystem_url + "/" + self.workdir_uuid + "/").path
            logger.debug(
        ...
        logger.debug(
            "  * application.exe -> saga.executable: %s",
            jd.executable)
        ...
        if len(argList) != 0:
            ...
            logger.debug(
                "  * application.args -> saga.arguments: %s",
                jd.arguments)
        ...
        if len(envList) != 0:
            ...
            logger.debug(
                "  * application.env -> saga.environment: %s",
                jd.environment)
        ...
        if len(job.backend.allocation) != 0:
            ...
            logger.debug(
                "  * backend.allocation -> saga.job_project: %s",
                jd.job_project)
    ...
    def updateMonitoringInformation(jobs):
        ...
        for j in jobs:
            ...
            try:
                ...
                # ((j.status != 'new') and (js_url.scheme == 'fork')):
                if False:
                    ...
                    if job_state == saga.job.Done:
                        downloader = get_saga_file_transfer_manager()
                        downloader.addTask(j)
                        # download will set the status either to complete of
                        # failed
                ...
                logger.error(
                    'exception caught while updating job: %s',
                    e.get_all_messages())
