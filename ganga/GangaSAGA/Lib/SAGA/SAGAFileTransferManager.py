    ...
    def process(self, item):
        ...
        try:
            ...
            if len(job.outputsandbox) > 0:
                output_sandbox = saga.url(
                    job.backend.filesystem_url +
                    "/" +
                    job.backend.workdir_uuid +
                    "/_output_sandbox.tgz")
            ...
            logger.error(
                'exception caught while poststaging: %s',
                e.get_full_message())
            job.updateStatus('failed')
...
class SAGAFileTransferManager(MTRunner):
    ...
    def __init__(self, numThread=10):
        ...
        logger.debug(
            'starting new MTRunner instance: %s.',
