...
if not _after_bootstrap:
    config = makeConfig(
        'SuperB',
        'Configuration parameters for SuperB ganga plugin')
    # /storage/gpfs_superb/users/ganga/ will be replaced by $VO_SUPERBVO_ORG_SW_DIR
    config.addOption(
        'severus_dir',
        '/storage/gpfs_superb/users/ganga_util/GangaSuperB/severus',
        'Local UI directory where severus.tgz is located.')
    config.addOption(
        'submission_site',
        'INFN-T1',
        'Target site where job analysis output will be transfered. Must be set to local UI site.')
