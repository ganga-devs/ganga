...
from Ganga.GPIDev.Lib.Registry.RegistrySliceProxy import RegistrySliceProxy, _wrap
...
class TransientRegistrySlice(RegistrySlice):
    def __init__(self,name):
        super(TransientRegistrySlice,self).__init__(name,display_prefix="box")
    ...
    def __call__(self,id):
        ...
        if type(id) is str:
            if id.isdigit(): 
                ...
                matches = [o for o in self.objects if fnmatch.fnmatch(o.name,id)]
                if len(matches)>1:
                    ...
                    raise RegistryKeyError("Multiple matches for id='%s':%s"%(id, str(map(lambda x:x.name, matches))))
                if len(matches) <1:
